...
> Принципы CREDO остаются в силе, но теперь они воплощены в двухуровневой
> стене линтов (`[workspace.lints]` в корневом `Cargo.toml`: forbid-floor +
> deny-band) и в актуальных соглашениях [`CLAUDE.md`](CLAUDE.md).
> Авторитетное поведение — в коде. Этот документ — исторический
> дизайн-рекорд, суперсёднутый `CLAUDE.md` + кодом; читайте его ради
> обоснований, не как инструкции. Тело ниже — архив и не редактируется.
