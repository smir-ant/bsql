...
## End-to-end / competitor / peak-RSS harness (this branch)

The codegen tools below (`asm-diff.sh` etc.) measure the *engine* with the socket
removed. The whole-driver comparison against a real PostgreSQL over loopback TCP —
side by side with `tokio-postgres` and `sqlx` — is the harness at the **root of
this branch**. It depends on `bsql` as a git dependency on `main`, so it carries no
copy of the library, and its competitor deps never touch the shipped crate's
dependency graph.
...
The measured standing (bsql wins every latency scenario; ~1.7 MB peak RSS,
~3.9× smaller than the field), the methodology, and how to reproduce it all live
