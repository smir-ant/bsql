> **HISTORICAL — историческая запись, НЕ текущий источник истины.**
> Это оригинальный blueprint перезапуска v1.0. Его ядро уже реализовано:
> компилируемо-проверяемый флагман `query!`, драйверы PostgreSQL (async +
> sync) и встроенный SQLite доступны через единый крейт `bsql`. Актуальные
> соглашения, раскладка воркспейса и команды сборки/тестов живут в
> [`CLAUDE.md`](CLAUDE.md); авторитетное поведение — в коде. Этот файл
> суперсёднут `CLAUDE.md` + кодом — читайте его только ради проектных
> обоснований, не как инструкции. Тело ниже (включая строку «Status:
> master») — архив и не редактируется.
...
- `NoticeResponse` (tag `'N'`): пре-диспатч filter silently consumes (DEF-062, shipped `cccc909` в Phase-1b hardening). Будущий `Action::EmitNotice(...)` — Phase 1c+, когда wrapper surfaces notices.
...
- 1c — Query / Execute / post-auth chain / basic codec. Pre-1c binding commitments ALREADY CLOSED в Phase-1b hardening waves: ✅ **DEF-059** compute/apply split, ✅ **DEF-060** typed error enums (kills silent-truncation class), ✅ **DEF-061** `Errored(ErrorKind)` (zero-clone on subsequent pushes), ✅ **DEF-062** `NoticeResponse` pre-dispatch filter, ✅ **DEF-063** substantively met by DEF-097 + DEF-112 (handshake typestate chain via `ProtoState` variants + exhaustive dispatch), ✅ **DEF-064** bounded-iterations loop in `parse_error_response`, ✅ **DEF-065** superseded by DEF-107 (deferred). Остаются open: **DEF-067** (`cargo asm` verification milestone), **DEF-068** superseded by DEF-102 (`base64ct` swap shipped `3129d1e`). **Phase-1c work surface reduced to the core domain: Query / Execute flows.**
  - **1c-1 Simple Query landed (2026-04-20).** ✅ `PgCommand::SimpleQuery { sql: Sql, reply: ReplyId<QueryKind> }` end-to-end with row streaming: `Action<'w, 'r>` two-lifetime refactor (`'r` for inbound `StreamRow` slices), `StagedAction::StreamRowRange` (absolute `ReadBuf::populated` coords — survives per-frame advance), four `ProtoState` variants modelling the T/D*/C/Z sequence incl. connection-survivable query-level errors (E emits FailReply, trailing Z drains to Idle — no socket close). 12 integration tests green. `Truncating` sealed marker trait now uniformly serves `BoundedStrTag` + `SqlTag`. Commits `eaffe4e` (1c-0 skeletons) → `4e3896b` (1c-1a) → `14d386d` (1c-1b) → `4c33eb0` (1c-1c). Self-audit pass (commit `6bc1744`) closed **DEF-121** — silent-reply-loss hazard on `feed_bytes` loop overflow: per-iter budget gate before dispatch (tier-4 → tier-2 structural), `MAX_ACTIONS_PER_CALL` 4 → 8, three shared dispatch helpers centralising invariants previously duplicated across AwaitFirstResponse / StreamingRows arms. Test count 83 → 96. See `deferred.md` §18 "1c-1 landed" + "DEF-121" sections for the detailed inventory.
  - **1c-2 Column decoder landed (2026-04-20).** ✅ Schema propagation + text-format decoding end-to-end. Public surface: `RowDesc` (Copy POD, 32-column cap) + `ColumnDesc { type_oid, format_code }` + `FormatCode { Text, Binary }` from `RowDescription` parse; `Action::StreamRow.desc: &'r RowDesc` and `Reply::QueryComplete.row_desc: Option<RowDesc>` thread schema through streaming and terminal delivery; `DataRowRef` + `ColumnsIter` zero-copy row parser with SQL NULL (`len = -1 → Ok(None)`), fused-after-error semantics, `ExactSizeIterator` / `FusedIterator` impls; `FromPgText<'a>` trait + impls for `i16` / `i32` / `i64` / `u32` / `bool` / `&str` (PG-strict `"t"`/`"f"` booleans, UTF-8 validation); `oids` module with catalog-pinned type OIDs. **Round-4 finding #5 closed** — `UnexpectedFormatCode { code }` classifies non-`{0,1}` format codes at parse time. Commits `8d3c5df` (1c-2a) → `7513a09` (1c-2b) → `3c3c0b2` (1c-2c). Test count 96 → 131 (+35). See `deferred.md` §18 "1c-2 landed" for the detailed commit-by-commit table.
  - **Tier-1 uplift wave (2026-04-21, 9 commits).** ✅ Aggressive tier-1 hunt on the 1c-1+1c-2 baseline. Six major uplifts (`Inbound`/`Outbound` typed tag newtypes, `TxStatus` enum, `FrameCoords` typed construction, `AuthSubCode` enum, wire-tag + OID value drift-pins, SimpleQuery WriteBuf overflow closure) landed as commits `b1cb052` → `0f2cbb2` → `81478ff` → `1be0a29`. Then `rust-senior-architect` deep audit (75 findings, triple-checked) produced 14 "TAKE"-rated follow-ups across 3 batches (`c54b6b9` visibility + Encoding overflow fidelity; `f8e5e62` `Option<Severity>`; `c46802d` `AuthSubCodeClass` + `column_idx: u8` + `application_name` cap 128). I rejected 2 agent recommendations after triple-check (`StagedActions` POD would trade heapless's `MaybeUninit` zero-cost init for a 2.5KB stack memset; `payload_len usize→u16` added fallible-narrowing complexity for marginal packing). Net: **~100 individual compile-time shield points** — 48 const-asserted drift-pins (17 OIDs + 26 wire tags + 4 auth sub-codes + protocol version), 26 tags with compile-enforced direction, 3 `TxStatus` variants, 4 `AuthSubCode` variants, 3 `FrameCoords` typed constructor args, plus `Option<Severity>`, `AuthSubCodeClass`, `UnexpectedFrame.tag: InboundTag`, SimpleQuery sizing invariant, and visibility-surface shields. Test count 131 → 124 → 125 (the −7 came from `OutActions::new` consolidation + OID test removal + FromPgText matrix tests; +1 regression for TxStatus invalid-byte shield). Forbid-bundle intact, clippy -D warnings clean, no fragility introduced. See `deferred.md` §18 "Architect-agent deep audit (2026-04-21)" for the detailed batch-by-batch table.
