...
async fn batch_vs_serial() {
    ...
    for &n in &[2_usize, 8, 32, 128, 512, 2000] {
        ...
        c.execute_batch::<EbbUpdQuery, _>((0..n as i64).map(|_| (0_i64, 0_i64)))
            .await
        ...
        c.execute_batch::<EbbUpdQuery, _>(sets.clone()).await.expect("warm batch");

        // Time execute_batch (best of 5).
        let mut batch_ns = u128::MAX;
        for _ in 0..5 {
            let t = Instant::now();
            c.execute_batch::<EbbUpdQuery, _>(sets.clone()).await.expect("batch");
        ...
        for _ in 0..5 {
            let t = Instant::now();
            for &(id, inc) in &sets {
                c.execute_batch::<EbbUpdQuery, _>(vec![(id, inc)]).await.expect("serial");
            }
