...
async fn grouping_is_preserved_per_command() {
    let mut c = Connection::connect(&cfg()).await.expect("connect");
    fresh(&mut c).await;
    c.query_batch::<QbInsQuery, _>((1..=5).map(|i| (i, "x")))
        .await
        .expect("seed 5");
    // Three SELECT commands with $1 = 1, 3, 5 → row counts 1, 3, 5 respectively.
    let grouped = c
        .query_batch::<QbSelQuery, _>(vec![(1_i64,), (3,), (5,)])
        .await
        .expect("select batch");
...
async fn mid_batch_failure_returns_zero_results_and_applies_nothing() {
    ...
    assert_eq!(c.query_one::<QbSevenQuery>(()).await.expect("reuse").n, 7);
    c.close().await.expect("close");
}
...
async fn zero_and_one() {
    ...
    fresh(&mut c).await;
    let empty = c
        .query_batch::<QbInsQuery, _>(Vec::<(i64, &str)>::new())
        .await
        .expect("N=0");
    assert!(empty.is_empty(), "N=0 → empty Vec, no I/O");
    let one = c
        .query_batch::<QbInsQuery, _>(vec![(1_i64, "solo")])
        .await
        .expect("N=1");
...
async fn large_n_windowed_is_correct_and_deadlock_free() {
    ...
    const N: i64 = 20_000;
    let grouped = c
        .query_batch::<QbInsQuery, _>((0..N).map(|i| (i, "bulk")))
        .await
        .expect("large batch");
    ...
    let result = c.query_batch::<QbInsQuery, _>(sets).await;
...
async fn oid_guard_drift_is_a_classified_batch_column_oid_mismatch() {
    ...
    match c.query_batch::<QbTagQuery, _>(vec![(), ()]).await {
    ...
    assert_eq!(c.query_one::<QbSevenQuery>(()).await.expect("recovers").n, 7);
    c.close().await.expect("close");
}
...
async fn oid_guard_matching_shadow_decodes_correctly() {
    ...
    c.execute_sql("INSERT INTO oidguard (tag, vc, bp, n) VALUES ('hello', 'v', 'b', 1)")
        .await
        .expect("seed match");
    let grouped = c.query_batch::<QbTagQuery, _>(vec![(), ()]).await.expect("matching batch runs");
...
async fn cancel_mid_batch_is_57014_and_connection_recovers() {
    ...
    let result = c.query_batch::<QbSleepQuery, _>(vec![(), ()]).await;
    ...
    assert_eq!(c.query_one::<QbSevenQuery>(()).await.expect("reuse after cancel").n, 7);
    c.close().await.expect("close");
}
