...
//!
//! `execute_batch<'p, Q>(params: impl IntoIterator<Item = Q::Params<'p>>)` binds the
//! argument-position iterator's `Item` to the carrier's `Q::Params<'p>`.
...
fn probe(c: &mut Connection) {
    ...
    let _ = c.execute_batch::<EbMismatch>(vec![("hostile", 1_i64)]);
}
