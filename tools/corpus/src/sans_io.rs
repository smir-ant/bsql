...
use bsql_postgres_proto::{
    Action, ActiveState, ColEvent, ConnectionStatus, NoticeRef, NotificationRef, PreparedQuery,
    SessionParams, StmtName, prepared,
};
...
/// [`ObservedRun`] is observable; the `last_*` fields are driving bookkeeping.
struct RunState {
    client_bytes: Vec<u8>,
...
impl RunState {
    ...
    fn into_observed(self, params: Vec<(String, String)>) -> ObservedRun {
        ObservedRun {
        ...
        loop {
            ...
            match rs.col_next() {
                ...
                ColEvent::EndQuery { outcome, .. } => {
                    if outcome.is_err() && step_err.is_none() {
                        *step_err = Some(ObservedErr::Protocol(ProtocolFailureKind::Unclassified));
                    }
                    return;
                }
                ColEvent::NeedMore => {
...
fn run_sync(transcript: &Transcript) -> ObservedRun {
    ...
    let mut session = match sync_setup(transcript, &mut run) {
        ...
        SetupResult::Terminal => return run.into_observed(Vec::new()),
        SetupResult::ActiveNoSession(params) => return run.into_observed(params),
    };
    ...
    let params = observe_param_statuses(session.proto.session_params());
    run.into_observed(params)
}

/// Result of driving a transcript's setup.
#[allow(
    clippy::large_enum_variant,
    reason = "the Active(Session) variant is the dominant case (every active-phase transcript) and the value lives briefly on the stack before its steps run; boxing it to shrink the rare no-session variants would add an allocation to the common path for no benefit"
)]
enum SetupResult {
    /// An active session is ready for steps.
    Active(Session),
    /// No session was produced and observation is complete (Disconnected, or a
    /// failed handshake) — but its session parameters were captured.
    ActiveNoSession(Vec<(String, String)>),
    /// No session and no further observation (terminal-only).
    Terminal,
}
            ...
            match sync_handshake(&mut ChunkQueue::new(chunks), run, true) {
                Ok(session) => {
                    if transcript.steps.is_empty() {
                        let params = observe_param_statuses(session.proto.session_params());
                        SetupResult::ActiveNoSession(params)
                    } else {
                        SetupResult::Active(session)
                    }
                }
                Err(()) => SetupResult::ActiveNoSession(Vec::new()),
            }
        }
        Setup::Disconnected => {
            // Server supplies no bytes; the handshake cannot complete.
            match sync_handshake(&mut ChunkQueue::new(Vec::new()), run, true) {
                Ok(_session) => SetupResult::Terminal,
                Err(()) => SetupResult::ActiveNoSession(Vec::new()),
            }
        }
    }
        ...
        PushOutcome::Drive(bytes) => {
            ...
            loop {
                match drain_buffered(session, run, &mut step_err) {
                    ...
                    DrainStep::Streaming => {
                        let rows = collect_rows(session, &mut step_err, || queue.next_chunk());
...
fn run_async(transcript: &Transcript) -> ObservedRun {
    let runtime = match tokio::runtime::Builder::new_current_thread().build() {
        ...
        Err(_) => {
            ...
            return run.into_observed(Vec::new());
        }
    };
...
async fn run_async_inner(transcript: &Transcript) -> ObservedRun {
    ...
    let mut session = match async_setup(transcript, &mut run, &mut writer).await {
        SetupResult::Active(s) => s,
        SetupResult::Terminal => {
            run.client_bytes = writer.captured;
            return run.into_observed(Vec::new());
        }
        SetupResult::ActiveNoSession(params) => {
            run.client_bytes = writer.captured;
            return run.into_observed(params);
        }
    };
            ...
            loop {
                match drain_buffered(session, run, &mut step_err) {
                    ...
                    DrainStep::Streaming => {
                        ...
                        let rows = collect_rows(session, &mut step_err, || pending.pop_front());
                        run.pending_rows = rows;
                    }
