...
async fn mixed_async_side(conn: &mut bsql::pg::Connection) {
    conn.execute_sql("CREATE TABLE mixed_shared (v int)").await.unwrap();
    conn.execute_sql("INSERT INTO mixed_shared (v) VALUES (1)").await.unwrap();

    let only = conn.query_one_sql("SELECT v FROM mixed_shared").await.unwrap();
    assert_eq!(only.get_i32(0), Ok(Some(1)), "async side sees only its own row");

    let count = conn.query_one_sql("SELECT count(*) FROM mixed_shared").await.unwrap();
...
fn mixed_sync_side(conn: &mut bsql::pg_sync::Connection) {
    conn.execute_sql("CREATE TABLE mixed_shared (v int)").unwrap();
    conn.execute_sql("INSERT INTO mixed_shared (v) VALUES (2)").unwrap();

    let only = conn.query_one_sql("SELECT v FROM mixed_shared").unwrap();
    assert_eq!(only.get_i32(0), Ok(Some(2)), "sync side sees only its own row");

    let count = conn.query_one_sql("SELECT count(*) FROM mixed_shared").unwrap();
    assert_eq!(count.get_i64(0), Ok(Some(1)), "isolation: exactly one row visible");
