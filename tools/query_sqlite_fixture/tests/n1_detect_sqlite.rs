...
fn meas_sql() -> &'static str {
    <MeasById as SqliteTypedQuery>::SQL
}
...
fn n_plus_one_is_flagged_with_source_and_count() {
    ...
    for _ in 0..LOOP {
        call_line = line!() + 1;
        let rows = conn.query::<MeasById>((1i64,)).unwrap();
...
fn a_single_query_is_not_flagged() {
    let conn = seed();
    let a = conn.query::<MeasById>((1i64,)).unwrap();
    assert_eq!(a.into_owned().unwrap().len(), 1);
    let b = conn.query::<AllIds>(()).unwrap();
...
fn distinct_call_sites_are_not_conflated() {
    ...
    for _ in 0..20 {
        let a = conn.query::<MeasById>((1i64,)).unwrap(); // site A
        assert_eq!(a.into_owned().unwrap().len(), 1);
    }
    for _ in 0..20 {
        let b = conn.query::<MeasById>((1i64,)).unwrap(); // site B (distinct line)
        assert_eq!(b.into_owned().unwrap().len(), 1);
    }
    assert!(
        conn.n1_report().is_empty(),
...
fn query_one_loop_is_flagged_once() {
    ...
    for _ in 0..LOOP {
        call_line = line!() + 1;
        let one = conn.query_one::<MeasById>((1i64,)).unwrap();
        assert_eq!(one.id, 1);
    }
    let report = conn.n1_report();
...
fn manual_rollback_resets_the_window_like_pg() {
    ...
    for tx_idx in 0..2 {
        conn.begin().unwrap();
        for _ in 0..20 {
            let rows = conn.query::<MeasById>((1i64,)).unwrap(); // ONE site
            assert_eq!(rows.into_owned().unwrap().len(), 1);
        }
...
fn n_plus_one_through_the_transaction_guard_is_flagged_at_the_consumer_line() {
    ...
    conn.transaction(|tx| {
        for _ in 0..LOOP {
            call_line = line!() + 1;
            let rows = tx.query::<MeasById>((1i64,))?;
