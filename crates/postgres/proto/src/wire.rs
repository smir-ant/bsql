...
/// socket.write_all(&bsql_postgres_proto::FLUSH_WIRE_BYTES).await?;
...
/// // Driver pseudocode:
/// async fn close(self) {
///     self.socket.write_all(&bsql_postgres_proto::TERMINATE_WIRE_BYTES).await?;
...
///     socket.write_all(&bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES).await?;
...
///
/// ```ignore
/// match bsql_postgres_proto::wire::classify_ssl_response_byte(byte) {
...
///     socket.write_all(&bsql_postgres_proto::cancel_request_bytes(pid, secret)).await?;
