...
fn ping_id(proto: &mut PgProtocol<bsql_postgres_proto::ActivePhase>) -> bsql_postgres_proto::reply_id::ReplyId<bsql_postgres_proto::reply_id::PingKind> {
    proto.next_reply_id()
}

fn query_id(proto: &mut PgProtocol<bsql_postgres_proto::ActivePhase>) -> bsql_postgres_proto::reply_id::ReplyId<bsql_postgres_proto::reply_id::QueryKind> {
    proto.next_reply_id()
}

fn parse_id(proto: &mut PgProtocol<bsql_postgres_proto::ActivePhase>) -> bsql_postgres_proto::reply_id::ReplyId<bsql_postgres_proto::reply_id::ParseKind> {
    proto.next_reply_id()
}
...
fn push_ping(proto: &mut PgProtocol<bsql_postgres_proto::ActivePhase>, wb: &mut WriteBuf) {
...
fn parse_complete_with_non_zero_body_classifies() {
    ...
    let stmt = match bsql_postgres_proto::ident::StmtName::try_from_str("s") {
        Ok(s) => s,
        Err(_) => return,
    };
    let sql = "SELECT 1";
...
fn parse_header_boundary_cap_is_exact() {
    use bsql_postgres_proto::frame::{parse_header, HeaderParse, MAX_FRAME_LEN_FIELD};
...
fn scram_max_iterations_is_pinned_at_100k() {
    assert_eq!(
        bsql_postgres_proto::scram::wire::MAX_SCRAM_ITERATIONS,
        100_000,
...
fn parameter_description_zero_params_is_legal() {
    use bsql_postgres_proto::frame::{parse_header, HeaderParse};
