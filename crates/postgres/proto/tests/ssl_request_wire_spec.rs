...
//! `bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES` (top-level re-export)
//! and `bsql_postgres_proto::wire::SSL_REQUEST_WIRE_BYTES` (module path),
...
const _ASSERT_LEN: () = assert!(
    bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES.len() == 8,
    "SSLRequest packet is 8 bytes: length(4) + version(4)",
);
const _ASSERT_LENGTH_FIELD: () = assert!(
    bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[0] == 0
        && bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[1] == 0
        && bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[2] == 0
        && bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[3] == 8,
    "Length field is BE u32 = 8 (length includes self + version code)",
);
const _ASSERT_VERSION_BYTES: () = assert!(
    bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[4] == 0x04
        && bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[5] == 0xd2
        && bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[6] == 0x16
        && bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES[7] == 0x2f,
...
fn ssl_request_top_level_and_module_paths_agree() {
    ...
    assert_eq!(
        bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES,
        bsql_postgres_proto::wire::SSL_REQUEST_WIRE_BYTES,
    );
}

#[test]
fn ssl_request_version_const_matches_byte_literal() {
    let v = bsql_postgres_proto::wire::SSL_REQUEST_VERSION;
    assert_eq!(v, 80_877_103u32, "SSL_REQUEST_VERSION constant value");
    let v_bytes = v.to_be_bytes();
    let wire_v = bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES.get(4..).unwrap_or_default();
    assert_eq!(
        wire_v,
        v_bytes.as_slice(),
...
fn ssl_request_length_field_includes_self() {
    ...
    let len_bytes: [u8; 4] = bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES
        .get(..4)
    ...
    assert_eq!(
        ...
        Some(bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES.len()),
...
fn ssl_request_distinct_from_terminate_and_sync() {
    ...
    let real_proto_version = bsql_postgres_proto::wire::PROTOCOL_VERSION_3_0;
    assert_ne!(
        bsql_postgres_proto::wire::SSL_REQUEST_VERSION, real_proto_version,
...
fn ssl_request_first_four_bytes_decode_to_eight() {
    ...
    let packet = bsql_postgres_proto::SSL_REQUEST_WIRE_BYTES;
...
fn classify_ssl_byte_s_is_accepted() {
    use bsql_postgres_proto::SslNegotiationOutcome;
    assert!(matches!(
        bsql_postgres_proto::classify_ssl_response_byte(b'S'),
...
fn classify_ssl_byte_n_is_refused() {
    use bsql_postgres_proto::SslNegotiationOutcome;
    assert!(matches!(
        bsql_postgres_proto::classify_ssl_response_byte(b'N'),
...
fn classify_ssl_byte_e_is_error_incoming() {
    use bsql_postgres_proto::SslNegotiationOutcome;
    assert!(matches!(
        bsql_postgres_proto::classify_ssl_response_byte(b'E'),
...
fn classify_ssl_byte_unknown_bytes_preserve_payload() {
    use bsql_postgres_proto::SslNegotiationOutcome;
    for byte in 0..=255u8 {
        let outcome = bsql_postgres_proto::classify_ssl_response_byte(byte);
        match byte {
...
fn classify_ssl_byte_outcome_equality_semantics() {
    let s = bsql_postgres_proto::classify_ssl_response_byte(b'S');
    let s2 = bsql_postgres_proto::classify_ssl_response_byte(b'S');
    let n = bsql_postgres_proto::classify_ssl_response_byte(b'N');
    let invalid_a = bsql_postgres_proto::classify_ssl_response_byte(0xab);
    let invalid_a2 = bsql_postgres_proto::classify_ssl_response_byte(0xab);
    let invalid_b = bsql_postgres_proto::classify_ssl_response_byte(0xcd);
...
const _PIN_CONST_FN: () = {
    let _ = bsql_postgres_proto::classify_ssl_response_byte(b'S');
    let _ = bsql_postgres_proto::classify_ssl_response_byte(b'N');
    let _ = bsql_postgres_proto::classify_ssl_response_byte(b'E');
    let _ = bsql_postgres_proto::classify_ssl_response_byte(0xff);
};
...
fn classify_ssl_byte_top_level_and_module_paths_agree() {
    let via_top = bsql_postgres_proto::classify_ssl_response_byte(b'S');
    let via_mod = bsql_postgres_proto::wire::classify_ssl_response_byte(b'S');
...
fn outcome_non_exhaustive_requires_catchall_externally() {
    use bsql_postgres_proto::SslNegotiationOutcome;
    let outcome = bsql_postgres_proto::classify_ssl_response_byte(b'S');
