...
//! `bsql_postgres_proto::TERMINATE_WIRE_BYTES` (top-level re-export) and
//! `bsql_postgres_proto::wire::TERMINATE_WIRE_BYTES` (module path), and
...
const _ASSERT_LEN: () = assert!(
    bsql_postgres_proto::TERMINATE_WIRE_BYTES.len() == 5,
    "Terminate frame is 5 bytes: tag (1) + length-field (4)",
);
const _ASSERT_TAG: () = assert!(
    bsql_postgres_proto::TERMINATE_WIRE_BYTES[0] == b'X',
    "Terminate tag is 'X' per PG §55.7",
);
const _ASSERT_LENGTH_FIELD: () = assert!(
    bsql_postgres_proto::TERMINATE_WIRE_BYTES[1] == 0
        && bsql_postgres_proto::TERMINATE_WIRE_BYTES[2] == 0
        && bsql_postgres_proto::TERMINATE_WIRE_BYTES[3] == 0
        && bsql_postgres_proto::TERMINATE_WIRE_BYTES[4] == 4,
...
fn terminate_wire_bytes_top_level_and_module_paths_agree() {
    ...
    assert_eq!(
        bsql_postgres_proto::TERMINATE_WIRE_BYTES,
        bsql_postgres_proto::wire::TERMINATE_WIRE_BYTES,
    );
}
...
fn terminate_wire_bytes_distinct_from_sync() {
    ...
    assert_ne!(
        bsql_postgres_proto::TERMINATE_WIRE_BYTES, sync_bytes,
