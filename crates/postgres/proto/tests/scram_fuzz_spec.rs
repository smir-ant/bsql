...
fn init_scram_protocol(
    _seed: u64,
) -> Option<(PgProtocol<bsql_postgres_proto::ConnectingPhase>, WriteBuf)> {
    let mut proto = PgProtocol::<bsql_postgres_proto::DisconnectedPhase>::new();
    ...
    for action in actions {
        if let bsql_postgres_proto::Action::SendBytes(b) = action {
