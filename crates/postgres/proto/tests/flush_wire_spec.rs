...
//! `bsql_postgres_proto::FLUSH_WIRE_BYTES` (top-level re-export) and
//! `bsql_postgres_proto::wire::FLUSH_WIRE_BYTES` (module path), and that
...
const _ASSERT_LEN: () = assert!(
    bsql_postgres_proto::FLUSH_WIRE_BYTES.len() == 5,
    "Flush frame is 5 bytes: tag (1) + length-field (4)",
);
const _ASSERT_TAG: () = assert!(
    bsql_postgres_proto::FLUSH_WIRE_BYTES[0] == b'H',
    "Flush tag is 'H' per PG §55.7",
);
const _ASSERT_LENGTH_FIELD: () = assert!(
    bsql_postgres_proto::FLUSH_WIRE_BYTES[1] == 0
        && bsql_postgres_proto::FLUSH_WIRE_BYTES[2] == 0
        && bsql_postgres_proto::FLUSH_WIRE_BYTES[3] == 0
        && bsql_postgres_proto::FLUSH_WIRE_BYTES[4] == 4,
...
fn flush_wire_bytes_match_pg_spec() {
    assert_eq!(
        bsql_postgres_proto::FLUSH_WIRE_BYTES,
        [b'H', 0, 0, 0, 4],
...
fn flush_wire_bytes_top_level_and_module_paths_agree() {
    ...
    assert_eq!(
        bsql_postgres_proto::FLUSH_WIRE_BYTES,
        bsql_postgres_proto::wire::FLUSH_WIRE_BYTES,
    );
}
...
fn flush_wire_bytes_distinct_from_sibling_parameterless_frames() {
    ...
    assert_ne!(
        bsql_postgres_proto::FLUSH_WIRE_BYTES, sync_bytes,
