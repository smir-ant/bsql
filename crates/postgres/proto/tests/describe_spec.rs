...
fn describe_stmt_setup(
    ...
    proto.push_or_panic(
        bsql_postgres_proto::push_command::DescribeStatement { stmt_name, reply },
        wb,
    );
...
fn describe_portal_setup(
    ...
    proto.push_or_panic(
        bsql_postgres_proto::push_command::DescribePortal { portal_name, reply },
        wb,
    );
    let (d_frame, _sync) = split_frame_plus_sync(wb.as_bytes());
    assert_eq!(
...
fn describe_statement_while_bind_execute_in_flight_blocked() {
    ...
    use bsql_postgres_proto::QueryKind as QK;
