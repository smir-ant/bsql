...
//! pull surface.
//!
//! Drives the new [`ConnectingEngine`] the way the flush/ingest pump will:
//! seat the startup for some credentials, feed scripted server auth frames
//! through `read_slot`/`commit`, and pull the connecting events. Covers the
//! trust success path, the cleartext / MD5 password-response paths (the engine
//! builds the `PasswordMessage` into its write buffer and surfaces the auth
//! event), the SCRAM initial-response path, `ParameterStatus` surfacing during
//! post-auth, the `into_active` consuming move (success + still-connecting),
//! and the rejection paths (a trust client cannot satisfy a SASL challenge).
//!
...
/// which any outbound auth response begins in the client buffer.
...
/// packet), or `None` when no response has been written yet.
fn first_response_tag(engine: &ConnectingEngine) -> Option<u8> {
    engine.client_bytes().get(STARTUP_LEN).copied()
}

/// Extract the client nonce from the `SASLInitialResponse` the engine wrote
...
fn trust_handshake_reaches_ready() {
    let mut engine = ConnectingEngine::start(&user(), None, None, Credentials::Trust).unwrap();
    // Startup packet is built immediately; no auth response yet.
    assert_eq!(engine.client_bytes().len(), STARTUP_LEN);

    feed(&mut engine, &auth_ok());
    feed(&mut engine, &backend_key(4321, 8765));
    feed(&mut engine, &ready_for_query(b'I'));

    assert!(matches!(engine.next_auth_event(), AuthEvent::Ready));
    // Trust sends nothing back: client bytes stay the startup packet.
    assert_eq!(engine.client_bytes().len(), STARTUP_LEN);
...
fn cleartext_builds_password_message_and_completes() {
    ...
    let mut engine = ConnectingEngine::start(&user(), None, None, creds).unwrap();

    feed(&mut engine, &auth(3, &[]));
    assert!(matches!(engine.next_auth_event(), AuthEvent::AuthCleartext));
    // The PasswordMessage frame (tag 'p') was appended to the write buffer.
    assert_eq!(first_response_tag(&engine), Some(TAG_SASL_RESPONSE.byte()));
...
fn md5_wrong_length_salt_is_classified_fail() {
    ...
    assert!(matches!(engine.next_auth_event(), AuthEvent::Fail(_)));
}
...
fn scram_initial_response_built() {
    ...
    assert!(matches!(engine.next_auth_event(), AuthEvent::NeedMore));
    assert_eq!(first_response_tag(&engine), Some(TAG_SASL_RESPONSE.byte()));
}
...
fn scram_continue_builds_sasl_response() {
    ...
    feed(&mut engine, &auth(10, b"SCRAM-SHA-256\0\0"));
    assert!(matches!(engine.next_auth_event(), AuthEvent::NeedMore));
    let init_len = engine.client_bytes().len();
    assert!(init_len > STARTUP_LEN, "SASLInitialResponse must be written");

    // Simulate the server: echo the client nonce, add a server nonce part, a
    // base64 salt, and an RFC-7677-legal iteration count.
    let nonce = extract_client_nonce(engine.client_bytes());
    ...
    // appended after the SASLInitialResponse.
    assert!(engine.client_bytes().len() > init_len);
    assert_eq!(
        engine.client_bytes().get(init_len).copied(),
...
fn into_active_before_ready_returns_still_connecting() {
    let engine = ConnectingEngine::start(&user(), None, None, Credentials::Trust).unwrap();
    ...
    match engine.into_active() {
        ...
        Err(still) => {
            // The returned connecting engine is intact and re-drivable.
            assert_eq!(still.client_bytes().len(), STARTUP_LEN);
        }
    }
}
