...
use bsql_postgres_proto::{AdvancePastEnd, READ_BUF_CAP, ReadBuf, ReadBufFull, WriteBuf, WriteBufFull};
...
fn write_buf_overflow_returns_full() {
    ...
    let big = vec![0u8; bsql_postgres_proto::MAX_OWNED_SEND_LEN];
    ...
    // Contents unchanged.
    assert_eq!(wb.len(), bsql_postgres_proto::MAX_OWNED_SEND_LEN);
}
