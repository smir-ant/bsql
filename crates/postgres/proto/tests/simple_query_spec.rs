...
fn dml_no_rows_end_to_end() {
    ...
    assert_eq!(proto.terminal_tx_status(), bsql_postgres_proto::TxStatus::InTransaction);
    assert!(matches!(proto.state(), ActiveState::Idle));
}
...
/// `self.row_desc = None` (in `bsql_postgres_proto::push_command::SimpleQuery` branch).
///
    ...
    proto.iter_rows(&mut wb, |stream| {
        ...
        for _ in 0..32 {
            match stream.col_next() {
                bsql_postgres_proto::ColEvent::EndQuery { outcome: Ok(_), .. } => {
                    saw_end_query = true;
                    break;
                }
                bsql_postgres_proto::ColEvent::EndQuery { outcome: Err(cause), .. } => {
                    panic!("unexpected EndQuery::Err on Q1: {cause:?}");
                }
                bsql_postgres_proto::ColEvent::Got { .. }
                | bsql_postgres_proto::ColEvent::Null { .. }
                | bsql_postgres_proto::ColEvent::EndRow
                | bsql_postgres_proto::ColEvent::Chunk { .. }
                | bsql_postgres_proto::ColEvent::ChunkEnd { .. }
