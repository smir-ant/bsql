...
trait BenchPushOrPanic {
    fn bench_push_or_panic<C: bsql_postgres_proto::push_command::PushCommand>(
        &mut self,
        cmd: C,
        wb: &mut WriteBuf,
    ) -> Result<(), PushFailure>;
...
impl BenchPushOrPanic for PgProtocol<bsql_postgres_proto::ActivePhase> {
    #[inline]
    fn bench_push_or_panic<C: bsql_postgres_proto::push_command::PushCommand>(
        &mut self,
        cmd: C,
...
fn bench_iter_rows_per_row_throughput(c: &mut Criterion) {
    use bsql_postgres_proto::ColEvent;
        ...
        b.iter_batched_ref(
            ...
            |(proto, wb)| {
                ...
                let out = proto.bench_push_or_panic(
                    bsql_postgres_proto::push_command::Ping::new(reply),
                    wb,
                );
                let _ = black_box(out);
            },
...
fn bench_validate_utf8_swar(c: &mut Criterion) {
    use bsql_postgres_proto::decode::validate_utf8_swar;
...
fn bench_parse_pg_bool_swar(c: &mut Criterion) {
    use bsql_postgres_proto::decode::parse_pg_bool_swar;
    ...
    group.bench_function("push_feed_collect_10rows", |b| {
        b.iter_batched_ref(
            ...
            |(proto, wb)| {
                ...
                let _result: Result<(), bsql_postgres_proto::ProtocolError> = proto.iter_rows(wb, |stream| {
                    if stream.feed(&server_bytes).is_err() {
                        return Err(bsql_postgres_proto::ProtocolError::InternalCrateBug {
...
fn bench_readbuf_compact_pressure(c: &mut Criterion) {
    ...
    group.bench_function("1000x_small_67B_append_advance", |b| {
        b.iter_batched_ref(
            bsql_postgres_proto::ReadBuf::new,
            |buf| {
                let chunk = [0u8; 67];
                for _ in 0..1000_u32 {
