...
use bsql_postgres_proto::{prepared, PreparedQuery, RowDecode};
...
fn t1_happy_path_per_type_compiles() {
    ...
    assert_eq!(Q_INT2.param_oids(), &[bsql_postgres_proto::oids::INT2]);
    assert_eq!(Q_INT4.param_oids(), &[bsql_postgres_proto::oids::INT4]);
    assert_eq!(Q_INT8.param_oids(), &[bsql_postgres_proto::oids::INT8]);
    assert_eq!(Q_OID.param_oids(), &[bsql_postgres_proto::oids::OID]);
    assert_eq!(Q_BOOL.param_oids(), &[bsql_postgres_proto::oids::BOOL]);
    assert_eq!(Q_TEXT.param_oids(), &[bsql_postgres_proto::oids::TEXT]);
}
...
fn t2_multi_parameter() {
    assert_eq!(
        ...
        &[
            ...
            bsql_postgres_proto::oids::TEXT,
            bsql_postgres_proto::oids::BOOL,
        ],
    );
    assert_eq!(Q_MULTI_PARAM.row_oids(), &[bsql_postgres_proto::oids::INT4]);
}
...
#[test]
fn t4_cast_form() {
    assert_eq!(Q_CAST_FORM.param_oids(), &[bsql_postgres_proto::oids::INT4]);
    assert_eq!(Q_CAST_FORM.row_oids(), &[bsql_postgres_proto::oids::INT4]);
}
...
#[test]
fn t7_comments() {
    assert_eq!(Q_LINE_COMMENT.param_oids(), &[bsql_postgres_proto::oids::INT4]);
    assert_eq!(Q_BLOCK_COMMENT.param_oids(), &[bsql_postgres_proto::oids::INT4]);
}
...
fn row_decode_oids_per_arity() {
    ...
    assert_eq!(
        ...
        &[bsql_postgres_proto::oids::INT4, bsql_postgres_proto::oids::TEXT],
    );
}
