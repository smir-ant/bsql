...
use bsql_postgres_proto::Pristine; // brings BOTH trait (type ns) and derive (macro ns) into scope
...
fn trait_method_and_const_inherent_agree() {
    ...
    for (case, expected) in cases {
        ...
        let via_trait = <OneInt as bsql_postgres_proto::pristine::Pristine>::is_pristine(&case);
