...
fn explain(conn: &mut Connection, sql: &str) -> String {
    let result = conn.query_raw(sql).expect("EXPLAIN runs");
...
fn plan_counts(conn: &mut Connection, name: &str) -> (i64, i64) {
    ...
    let result = conn.query_raw(&sql).expect("read plan counts");
...
fn run_execs(conn: &mut Connection, execute_raw: &str, n: usize) {
    for _ in 0..n {
        let _ = conn.query_raw(execute_raw).expect("execute prepared form");
    }
}
...
fn dynamic_filter_plan_mode_evidence() {
    ...
    conn.execute_raw("INSERT INTO toggle_demo SELECT g, g, g % 4 FROM generate_series(1, 100000) g")
        .expect("seed rows");
    conn.execute_raw("CREATE INDEX toggle_demo_v_idx ON toggle_demo (v)")
        .expect("create v index");
    conn.execute_raw("CREATE INDEX toggle_demo_bucket_idx ON toggle_demo (bucket)")
        .expect("create bucket index");
    conn.execute_raw("ANALYZE toggle_demo").expect("analyze");
    ...
    conn.execute_raw("PREPARE tog(int4) AS SELECT id FROM toggle_demo WHERE ($1 IS NULL OR v = $1)")
    ...
    //          reproduces. -----------------------------------------------
    conn.execute_raw("SET plan_cache_mode = force_generic_plan")
    ...
    conn.execute_raw("RESET plan_cache_mode").expect("reset mode");
    conn.execute_raw("DEALLOCATE tog").expect("deallocate toggle form");
    ...
    conn.execute_raw("PREPARE tog2(int4) AS SELECT id FROM toggle_demo WHERE ($1 IS NULL OR v = $1)")
    ...
    conn.execute_raw("DEALLOCATE tog2").expect("deallocate null-warmup form");
    ...
    conn.execute_raw("DEALLOCATE tog3").expect("deallocate low-selectivity form");
    ...
    conn.execute_raw("DEALLOCATE togm").expect("deallocate multi-toggle form");
    ...
    conn.execute_raw("PREPARE pk(int4) AS SELECT v FROM toggle_demo WHERE id = $1")
    ...
    conn.execute_raw("DEALLOCATE pk").expect("deallocate plain lookup");

    conn.execute_raw("SET plan_cache_mode = force_custom_plan")
        .expect("force custom");
    conn.execute_raw("PREPARE pk2(int4) AS SELECT v FROM toggle_demo WHERE id = $1")
    ...
    conn.execute_raw("RESET plan_cache_mode").expect("reset mode");
    conn.execute_raw("DEALLOCATE pk2").expect("deallocate plain lookup");
