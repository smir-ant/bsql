...
/// whose inherent methods the scenarios call (`query_raw`, `execute_raw`,
...
fn sql_join_types() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE t1(id int, v text)").expect("t1");
    c.execute_raw("CREATE TEMP TABLE t2(id int, label text)").expect("t2");
    c.execute_raw("INSERT INTO t1 VALUES (1,'a'),(2,'b'),(3,'c')").expect("ins t1");
    c.execute_raw("INSERT INTO t2 VALUES (2,'x'),(3,'y'),(4,'z')").expect("ins t2");

    // INNER JOIN
    let r = c.query_raw("SELECT t1.v, t2.label FROM t1 INNER JOIN t2 ON t1.id = t2.id ORDER BY t1.id").expect("inner");
    assert_eq!(r.len(), 2);
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("b")));

    // LEFT JOIN
    let r = c.query_raw("SELECT t1.v, t2.label FROM t1 LEFT JOIN t2 ON t1.id = t2.id ORDER BY t1.id").expect("left");
    assert_eq!(r.len(), 3);
    assert!(r.get(0).expect("row 0").is_null(1)); // t1.id=1 has no match

    // RIGHT JOIN
    let r = c.query_raw("SELECT t1.v, t2.label FROM t1 RIGHT JOIN t2 ON t1.id = t2.id ORDER BY t2.id").expect("right");
    assert_eq!(r.len(), 3);

    // CROSS JOIN
    let r = c.query_raw("SELECT count(*) FROM t1 CROSS JOIN t2").expect("cross");
...
fn sql_subqueries_and_cte() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE emp(id int, name text, dept_id int, salary int)").expect("create");
    c.execute_raw("INSERT INTO emp VALUES (1,'alice',1,100),(2,'bob',1,120),(3,'charlie',2,90),(4,'dave',2,110)").expect("insert");

    // Subquery in WHERE
    let r = c.query_raw("SELECT name FROM emp WHERE salary > (SELECT AVG(salary) FROM emp) ORDER BY name").expect("subq");
    ...
    // EXISTS
    let r = c.query_raw("SELECT name FROM emp WHERE EXISTS (SELECT 1 FROM emp e2 WHERE e2.dept_id = emp.dept_id AND e2.salary > 110) ORDER BY name").expect("exists");
    assert!(r.len() >= 1);
...
fn sql_window_functions() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE sales(id int, region text, amount int)").expect("create");
    c.execute_raw("INSERT INTO sales VALUES (1,'east',100),(2,'east',200),(3,'west',150),(4,'west',300),(5,'east',50)").expect("ins");

    // ROW_NUMBER
    let r = c.query_raw("SELECT id, ROW_NUMBER() OVER (ORDER BY amount DESC) as rn FROM sales").expect("row_number");
    assert_eq!(r.len(), 5);

    // RANK + PARTITION BY
    let r = c.query_raw("SELECT region, amount, RANK() OVER (PARTITION BY region ORDER BY amount DESC) as rnk FROM sales ORDER BY region, rnk").expect("rank");
    assert_eq!(r.len(), 5);
    assert_eq!(r.get(0).expect("row 0").get_i64(2), Ok(Some(1))); // top of east partition

    // SUM OVER (running total)
    let r = c.query_raw("SELECT id, SUM(amount) OVER (ORDER BY id) as running FROM sales ORDER BY id").expect("running sum");
    assert_eq!(r.get(0).expect("row 0").get_i64(1), Ok(Some(100)));
    assert_eq!(r.get(1).expect("row 1").get_i64(1), Ok(Some(300)));

    // LAG / LEAD
    let r = c.query_raw("SELECT id, amount, LAG(amount) OVER (ORDER BY id) as prev FROM sales ORDER BY id").expect("lag");
...
fn sql_aggregates_and_grouping() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE agg_data(cat text, val int)").expect("create");
    c.execute_raw("INSERT INTO agg_data VALUES ('a',10),('a',20),('b',30),('b',40),('a',50)").expect("ins");

    // GROUP BY + HAVING
    let r = c.query_raw("SELECT cat, SUM(val) as s FROM agg_data GROUP BY cat HAVING SUM(val) > 50 ORDER BY cat").expect("having");
    ...
    // COUNT, MIN, MAX, AVG
    let r = c.query_raw("SELECT COUNT(*), MIN(val), MAX(val), AVG(val)::int FROM agg_data").expect("agg");
    ...
    // DISTINCT
    let r = c.query_raw("SELECT DISTINCT cat FROM agg_data ORDER BY cat").expect("distinct");
    assert_eq!(r.len(), 2);

    // GROUP BY ROLLUP (PG extension)
    let r = c.query_raw("SELECT cat, SUM(val) FROM agg_data GROUP BY ROLLUP(cat) ORDER BY cat NULLS LAST").expect("rollup");
...
fn sql_string_and_type_operations() {
    ...
    c.execute_raw(&format!("CREATE TEMP TABLE \"{long_name}\"(v int)")).expect("long table name");
    c.execute_raw(&format!("INSERT INTO \"{long_name}\" VALUES (1)")).expect("insert long");
    let r = c.query_raw(&format!("SELECT v FROM \"{long_name}\"")).expect("select long");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1)));

    // String functions
    let r = c.query_raw("SELECT LENGTH('hello'), UPPER('hello'), TRIM('  hi  '), CONCAT('a','b','c')").expect("str funcs");
    ...
    // Type casts
    let r = c.query_raw("SELECT '42'::int, 3.14::text, true::int, NULL::text").expect("casts");
    ...
    // CASE WHEN
    let r = c.query_raw("SELECT CASE WHEN 1=1 THEN 'yes' ELSE 'no' END").expect("case");
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("yes")));

    // COALESCE + NULLIF
    let r = c.query_raw("SELECT COALESCE(NULL, NULL, 'found'), NULLIF(1, 1), NULLIF(1, 2)").expect("coalesce");
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("found")));
    assert!(r.get(0).expect("row 0").is_null(1));
    assert_eq!(r.get(0).expect("row 0").get_i32(2), Ok(Some(1)));

    // Array (PG-specific)
    let r = c.query_raw("SELECT ARRAY[1,2,3]::text").expect("array");
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("{1,2,3}")));

    // JSON (PG-specific)
    let r = c.query_raw("SELECT '{\"key\": \"value\"}'::json->>'key'").expect("json");
...
fn sql_where_and_ordering() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE wh_items(id int, name text, price int, active bool)").expect("create");
    c.execute_raw("INSERT INTO wh_items VALUES (1,'pen',10,true),(2,'book',50,true),(3,'eraser',5,false),(4,'ruler',15,true),(5,'pencil',8,false)").expect("ins");

    // WHERE with AND/OR
    let r = c.query_raw("SELECT name FROM wh_items WHERE active AND price > 10 ORDER BY name").expect("and");
    assert_eq!(r.len(), 2); // book(50), ruler(15)
    let r = c.query_raw("SELECT name FROM wh_items WHERE price < 10 OR price > 40 ORDER BY name").expect("or");
    assert!(r.len() >= 2); // eraser(5), pencil(8), book(50)

    // IN, NOT IN
    let r = c.query_raw("SELECT name FROM wh_items WHERE id IN (1, 3, 5) ORDER BY id").expect("in");
    assert_eq!(r.len(), 3);

    // BETWEEN
    let r = c.query_raw("SELECT name FROM wh_items WHERE price BETWEEN 8 AND 15 ORDER BY price").expect("between");
    assert_eq!(r.len(), 3); // pencil(8), pen(10), ruler(15)

    // LIKE / ILIKE
    let r = c.query_raw("SELECT name FROM wh_items WHERE name LIKE 'p%' ORDER BY name").expect("like");
    assert_eq!(r.len(), 2); // pen, pencil
    let r = c.query_raw("SELECT name FROM wh_items WHERE name ILIKE 'P%' ORDER BY name").expect("ilike");
    assert_eq!(r.len(), 2);

    // ORDER BY + LIMIT + OFFSET
    let r = c.query_raw("SELECT name FROM wh_items ORDER BY price DESC LIMIT 2 OFFSET 1").expect("limit offset");
    assert_eq!(r.len(), 2);

    // IS NULL / IS NOT NULL
    let r = c.query_raw("SELECT name FROM wh_items WHERE active IS NOT NULL").expect("is not null");
    assert_eq!(r.len(), 5);
...
fn sql_keyset_pagination() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE pages(id int PRIMARY KEY, val text)").expect("create");
    for i in 1..=20 {
        c.execute_raw(&format!("INSERT INTO pages VALUES ({i}, 'row_{i}')")).expect("ins");
    }

    // Keyset pagination (seek method)
    let page1 = c.query_raw("SELECT id, val FROM pages ORDER BY id LIMIT 5").expect("page1");
...
fn sql_upsert_and_returning() {
    let mut c = $config_fn();
    c.execute_raw("CREATE TEMP TABLE kv(key text PRIMARY KEY, val int)").expect("create");
    c.execute_raw("INSERT INTO kv VALUES ('a', 1)").expect("ins");

    // ON CONFLICT (upsert)
    c.execute_raw("INSERT INTO kv VALUES ('a', 10) ON CONFLICT (key) DO UPDATE SET val = EXCLUDED.val").expect("upsert");
    let r = c.query_raw("SELECT val FROM kv WHERE key = 'a'").expect("check");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(10)));

    // INSERT ... RETURNING
    let r = c.query_raw("INSERT INTO kv VALUES ('b', 20) RETURNING key, val").expect("returning");
    assert_eq!(r.len(), 1);
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("b")));
...
fn sql_generate_series_and_lateral() {
    ...
    // generate_series
    let r = c.query_raw("SELECT * FROM generate_series(1, 5)").expect("series");
    assert_eq!(r.len(), 5);

    // LATERAL join
    c.execute_raw("CREATE TEMP TABLE depts(id int, name text)").expect("create");
    c.execute_raw("CREATE TEMP TABLE emps(id int, dept_id int, name text)").expect("create2");
    c.execute_raw("INSERT INTO depts VALUES (1,'eng'),(2,'sales')").expect("ins d");
    c.execute_raw("INSERT INTO emps VALUES (1,1,'a'),(2,1,'b'),(3,2,'c')").expect("ins e");
...
fn sql_system_and_meta_queries() {
    ...
    // SELECT version()
    let r = c.query_raw("SELECT version()").expect("version");
    ...
    let r = c.query_raw("SELECT current_database(), current_user, now()::text").expect("meta");
    ...
    // pg_catalog queries
    let r = c.query_raw("SELECT count(*) FROM pg_catalog.pg_tables WHERE schemaname = 'pg_catalog'").expect("pg_tables");
    assert!(r.get(0).expect("row 0").get_i64(0).expect("count decodes").is_some_and(|n| n > 0));

    // information_schema
    let r = c.query_raw("SELECT count(*) FROM information_schema.tables WHERE table_schema = 'pg_catalog'").expect("info_schema");
    assert!(r.get(0).expect("row 0").get_i64(0).expect("count decodes").is_some_and(|n| n > 0));
...
fn sql_edge_cases() {
    ...
    c.execute_raw("CREATE TEMP TABLE edge(v text)").expect("create");
    c.execute_raw("INSERT INTO edge VALUES (''), (NULL), (' '), ('hello')").expect("ins");
    let r = c.query_raw("SELECT v FROM edge ORDER BY v NULLS FIRST").expect("q");
    ...
    c.execute_raw("CREATE TEMP TABLE longval(v text)").expect("create");
    c.execute_params("INSERT INTO longval VALUES ($1)", &(long.as_str(),)).expect("ins long");
    let r = c.query_raw("SELECT LENGTH(v) FROM longval").expect("len");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1_000)));

    // Unicode edge cases
    let r = c.query_raw("SELECT '🦀🐘'::text, E'tab\\there'::text, E'new\\nline'::text").expect("unicode");
    ...
    // Numeric edge cases
    let r = c.query_raw("SELECT 2147483647::int, (-2147483647-1)::int, 9223372036854775807::bigint").expect("nums");
    ...
    // Boolean
    let r = c.query_raw("SELECT true, false, NULL::bool").expect("bools");
    ...
    // Empty result set
    let r = c.query_raw("SELECT 1 WHERE false").expect("empty");
    assert_eq!(r.len(), 0);

    // Multiple columns same name (PG allows this)
    let r = c.query_raw("SELECT 1 AS x, 2 AS x").expect("dup cols");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1)));
...
fn sql_date_and_interval() {
    ...
    let r = c.query_raw("SELECT '2024-01-15'::date::text, '13:45:00'::time::text").expect("date/time");
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("2024-01-15")));
    assert_eq!(r.get(0).expect("row 0").get_str(1), Ok(Some("13:45:00")));

    let r = c.query_raw("SELECT '2024-01-15'::date + interval '1 month' + interval '2 days'").expect("interval");
    assert!(r.get(0).expect("row 0").get_str(0).expect("interval decodes").is_some());

    // EXTRACT
    let r = c.query_raw("SELECT EXTRACT(YEAR FROM '2024-06-15'::date)::int").expect("extract");
...
fn sql_error_zoo() {
    ...
    // Missing objects
    assert!(c.query_raw("SELECT * FROM table_that_does_not_exist_xyz").is_err());
    assert!(c.query_raw("SELECT nonexistent_column FROM pg_class LIMIT 1").is_err());
    assert!(c.execute_raw("DROP TABLE table_that_does_not_exist_xyz").is_err());
    assert!(c.execute_raw("ALTER TABLE nonexistent_xyz ADD COLUMN x int").is_err());
    c.ping().expect("recover after missing objects");

    // Double-drop
    c.execute_raw("CREATE TEMP TABLE dd(x int)").expect("create");
    c.execute_raw("DROP TABLE dd").expect("drop");
    assert!(c.execute_raw("DROP TABLE dd").is_err());
    c.ping().expect("recover after double drop");

    // Type mismatch
    assert!(c.query_raw("SELECT 'not_a_number'::int").is_err());
    assert!(c.query_raw("SELECT 'not_a_bool'::bool").is_err());
    assert!(c.query_raw("SELECT '99999999999999999999'::int").is_err());
    c.ping().expect("recover after type mismatch");

    // Division by zero
    assert!(c.query_raw("SELECT 1/0").is_err());
    assert!(c.query_raw("SELECT 1.0/0.0").is_err());
    c.ping().expect("recover after division by zero");

    // Constraint violations
    c.execute_raw("CREATE TEMP TABLE cv(id int PRIMARY KEY, name text NOT NULL, val int CHECK(val > 0))").expect("create");
    c.execute_raw("INSERT INTO cv VALUES (1, 'a', 1)").expect("ok");
    assert!(c.execute_raw("INSERT INTO cv VALUES (1, 'b', 2)").is_err()); // PK dup
    assert!(c.execute_raw("INSERT INTO cv VALUES (2, NULL, 2)").is_err()); // NOT NULL
    assert!(c.execute_raw("INSERT INTO cv VALUES (3, 'c', -1)").is_err()); // CHECK
    assert!(c.execute_raw("INSERT INTO cv VALUES (4, 'c', 0)").is_err()); // CHECK
    c.ping().expect("recover after constraints");
    assert_eq!(c.query_raw("SELECT count(*) FROM cv").expect("c").get(0).expect("row 0").get_i64(0), Ok(Some(1)));

    // Truncation / overflow
    assert!(c.query_raw("SELECT 2147483648::int").is_err()); // i32 overflow
    assert!(c.query_raw("SELECT 9999999999999999999::bigint").is_err()); // i64 overflow
    ...
    c.execute_raw("CREATE TEMP TABLE ghost(v int)").expect("create");
    c.execute_raw("INSERT INTO ghost VALUES (1)").expect("ins");
    c.execute_raw("DROP TABLE ghost").expect("drop");
    assert!(c.query_raw("SELECT * FROM ghost").is_err());
    assert!(c.execute_raw("INSERT INTO ghost VALUES (2)").is_err());
...
fn notice_then_command_recovers() {
    ...
    for _ in 0..16 {
        ...
        let r = c.query_raw("SELECT 1::int").expect("SELECT after NOTICE must succeed");
        assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1)));
    }
    ...
    c.execute_raw(&format!("CREATE TABLE {real}(x int)")).expect("create");
...
fn sql_extreme_values() {
    ...
    // Integer extremes
    let r = c.query_raw("SELECT (-2147483647-1)::int, 2147483647::int").expect("i32 extremes");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(i32::MIN)));
    assert_eq!(r.get(0).expect("row 0").get_i32(1), Ok(Some(i32::MAX)));

    let r = c.query_raw("SELECT (-9223372036854775807-1)::bigint, 9223372036854775807::bigint").expect("i64 extremes");
    assert_eq!(r.get(0).expect("row 0").get_i64(0), Ok(Some(i64::MIN)));
    assert_eq!(r.get(0).expect("row 0").get_i64(1), Ok(Some(i64::MAX)));

    let r = c.query_raw("SELECT (-32767-1)::smallint, 32767::smallint").expect("i16 extremes");
    ...
    // Zero
    let r = c.query_raw("SELECT 0::int, 0::bigint, 0::smallint, 0.0::float4, 0.0::float8").expect("zeros");
    ...
    // Float specials
    let r = c.query_raw("SELECT 'NaN'::float8, 'Infinity'::float8, '-Infinity'::float8").expect("float specials");
    ...
    // Empty string vs NULL
    let r = c.query_raw("SELECT ''::text, NULL::text, ' '::text").expect("empty vs null");
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("")));
    assert!(r.get(0).expect("row 0").is_null(1));
    ...
    c.execute_raw("CREATE TEMP TABLE longtext(v text)").expect("create");
    let long = "x".repeat(8000);
    c.execute_params("INSERT INTO longtext VALUES ($1)", &(long.as_str(),)).expect("insert 8000");
    let r = c.query_raw("SELECT length(v), v FROM longtext").expect("select");
    ...
    c.execute_raw(&format!("INSERT INTO longtext VALUES ('{big_literal}')")).expect("insert 50K literal");
    let r = c.query_raw("SELECT length(v) FROM longtext WHERE v LIKE 'y%'").expect("select big");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(50_000)));

    // Unicode stress
    let r = c.query_raw("SELECT '🎭🎪🎨'::text, '中文测试'::text, 'العربية'::text, '日本語テスト'::text").expect("unicode");
    assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("🎭🎪🎨")));
    ...
    // All NULLs
    let r = c.query_raw("SELECT NULL::int, NULL::text, NULL::bool, NULL::float8, NULL::bigint").expect("all null");
    for i in 0..5 { assert!(r.get(0).expect("row 0").is_null(i), "col {i} should be null"); }

    // Booleans
    let r = c.query_raw("SELECT true, false, NOT true, true AND false, true OR false").expect("bools");
    assert_eq!(r.get(0).expect("row 0").get_bool(0), Ok(Some(true)));
    assert_eq!(r.get(0).expect("row 0").get_bool(1), Ok(Some(false)));
...
fn sql_empty_and_boundary_results() {
    ...
    // Empty result set
    c.execute_raw("CREATE TEMP TABLE empty_t(v int)").expect("create");
    let r = c.query_raw("SELECT * FROM empty_t").expect("empty");
    assert_eq!(r.len(), 0);
    assert!(r.column_names.len() > 0);

    // WHERE that matches nothing
    c.execute_raw("INSERT INTO empty_t VALUES (1),(2),(3)").expect("ins");
    let r = c.query_raw("SELECT * FROM empty_t WHERE v > 999").expect("no match");
    assert_eq!(r.len(), 0);

    // Single row, single column
    let r = c.query_raw("SELECT 42::int").expect("scalar");
    assert_eq!(r.len(), 1);
    assert_eq!(r.column_names.len(), 1);

    // Single row, many NULLs
    let r = c.query_raw("SELECT NULL::int, NULL::text, NULL::bool, NULL::float8, NULL::int, NULL::text, NULL::bool, NULL::float8, NULL::int, NULL::text").expect("10 nulls");
    assert_eq!(r.len(), 1);
    for i in 0..10 { assert!(r.get(0).expect("row 0").is_null(i)); }

    // Many rows, one column
    let r = c.query_raw("SELECT generate_series(1, 5000)").expect("5k rows");
    assert_eq!(r.len(), 5000);
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1)));
    assert_eq!(r.get(4999).expect("row 4999").get_i32(0), Ok(Some(5000)));

    // LIMIT 0
    let r = c.query_raw("SELECT * FROM empty_t LIMIT 0").expect("limit 0");
    assert_eq!(r.len(), 0);

    // OFFSET past end
    let r = c.query_raw("SELECT * FROM empty_t OFFSET 999").expect("offset past end");
    assert_eq!(r.len(), 0);

    // SELECT with no FROM (synthetic row)
    let r = c.query_raw("SELECT 1 as a, 'b' as b, true as c").expect("no from");
    assert_eq!(r.len(), 1);
    assert_eq!(&*r.column_names, &["a", "b", "c"]);

    // Duplicate column names
    let r = c.query_raw("SELECT 1 as x, 2 as x, 3 as x").expect("dup names");
    assert_eq!(r.column_names.len(), 3);
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1)));
    assert_eq!(r.get(0).expect("row 0").get_i32(2), Ok(Some(3)));

    // Reserved words as identifiers
    c.execute_raw("CREATE TEMP TABLE \"select\"(\"where\" int, \"from\" text)").expect("reserved");
    c.execute_raw("INSERT INTO \"select\" VALUES (1, 'a')").expect("ins");
    let r = c.query_raw("SELECT \"where\", \"from\" FROM \"select\"").expect("select reserved");
    assert_eq!(r.get(0).expect("row 0").get_i32(0), Ok(Some(1)));
...
fn sql_concurrent_ddl_and_rapid_ops() {
    ...
    for i in 0..20u32 {
        ...
        c.execute_raw(&format!("CREATE TEMP TABLE {name}(v int)")).expect("create");
        c.execute_raw(&format!("INSERT INTO {name} VALUES ({i})")).expect("ins");
        assert_eq!(c.query_raw(&format!("SELECT v FROM {name}")).expect("q").get(0).expect("row 0").get_i32(0), Ok(Some(i as i32)));
        c.execute_raw(&format!("DROP TABLE {name}")).expect("drop");
    }
    ...
    for _ in 0..10 {
        assert!(c.query_raw("SELECT * FROM nonexistent_table_zzz").is_err());
        assert_eq!(c.query_raw("SELECT 1::int").expect("ok").get(0).expect("row 0").get_i32(0), Ok(Some(1)));
    }

    // Multiple temp tables at once
    for i in 0..10u32 {
        c.execute_raw(&format!("CREATE TEMP TABLE mt_{i}(v int)")).expect("create");
        c.execute_raw(&format!("INSERT INTO mt_{i} VALUES ({i})")).expect("ins");
    }
    for i in 0..10u32 {
        let r = c.query_raw(&format!("SELECT v FROM mt_{i}")).expect("q");
    ...
    let r = c.query_raw(&sql).expect("mega join");
...
fn sql_transaction_edge_cases() {
    ...
    c.execute_raw("CREATE TEMP TABLE txe(id int PRIMARY KEY, v text)").expect("create");

    // Empty transaction — commit nothing
    c.transaction(|tx| Ok(())).expect("empty tx");

    // Transaction with only reads
    c.execute_raw("INSERT INTO txe VALUES (1, 'a')").expect("seed");
    c.transaction(|tx| {
        let r = tx.query_raw("SELECT v FROM txe WHERE id = 1")?;
        assert_eq!(r.get(0).expect("row 0").get_str(0), Ok(Some("a")));
        Ok(())
    }).expect("read-only tx");
    ...
    let result: Result<(), _> = c.transaction(|tx| {
        tx.execute_raw("INSERT INTO txe VALUES (2, 'b')")?;
        tx.execute_raw("INSERT INTO txe VALUES (2, 'dupe')")?; // PK violation
        Ok(())
    });
    assert!(result.is_err());
    assert_eq!(c.query_raw("SELECT count(*) FROM txe").expect("c").get(0).expect("row 0").get_i64(0), Ok(Some(1)));

    // Rollback on user error
    let _: Result<(), _> = c.transaction(|tx| {
        tx.execute_raw("INSERT INTO txe VALUES (3, 'c')")?;
    ...
    c.transaction(|tx| {
        tx.execute_raw("INSERT INTO txe VALUES (10, 'x')")?;
        tx.execute_raw("INSERT INTO txe VALUES (11, 'y')")?;
        tx.execute_raw("UPDATE txe SET v = 'z' WHERE id = 10")?;
        tx.execute_raw("DELETE FROM txe WHERE id = 1")?;
        Ok(())
    }).expect("multi-stmt tx");
    assert_eq!(c.query_raw("SELECT count(*) FROM txe").expect("c").get(0).expect("row 0").get_i64(0), Ok(Some(2)));
    assert_eq!(c.query_raw("SELECT v FROM txe WHERE id = 10").expect("q").get(0).expect("row 0").get_str(0), Ok(Some("z")));
