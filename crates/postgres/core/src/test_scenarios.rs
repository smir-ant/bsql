...
fn sql_error_zoo() {
    ...
    assert_eq!(c.query_sql("SELECT count(*) FROM cv").expect("c").rows[0].get_i64(0), Some(1));
...
fn sql_concurrent_ddl_and_rapid_ops() {
    ...
    for i in 0..20u32 {
        ...
        assert_eq!(c.query_sql(&format!("SELECT v FROM {name}")).expect("q").rows[0].get_i32(0), Some(i as i32));
    ...
    for _ in 0..10 {
        ...
        assert_eq!(c.query_sql("SELECT 1::int").expect("ok").rows[0].get_i32(0), Some(1));
    }
...
fn sql_transaction_edge_cases() {
    ...
    assert_eq!(c.query_sql("SELECT count(*) FROM txe").expect("c").rows[0].get_i64(0), Some(1));
    ...
    assert_eq!(c.query_sql("SELECT count(*) FROM txe").expect("c").rows[0].get_i64(0), Some(2));
    assert_eq!(c.query_sql("SELECT v FROM txe WHERE id = 10").expect("q").rows[0].get_str(0), Some("z"));
