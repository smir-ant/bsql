...
fn sql_join_types() {
    ...
    assert_eq!(r.rows[0].get_str(0), Ok(Some("b")));
    ...
    assert_eq!(r.rows[0].get_i64(0), Ok(Some(9)));
...
fn sql_window_functions() {
    ...
    assert_eq!(r.rows.len(), 5);
    assert_eq!(r.rows[0].get_i64(2), Ok(Some(1))); // top of east partition

    // SUM OVER (running total)
    let r = c.query_sql("SELECT id, SUM(amount) OVER (ORDER BY id) as running FROM sales ORDER BY id").expect("running sum");
    assert_eq!(r.rows[0].get_i64(1), Ok(Some(100)));
    assert_eq!(r.rows[1].get_i64(1), Ok(Some(300)));
...
fn sql_aggregates_and_grouping() {
    ...
    assert_eq!(r.rows[0].get_str(0), Ok(Some("a")));
    assert_eq!(r.rows[0].get_i64(1), Ok(Some(80)));
    assert_eq!(r.rows[1].get_str(0), Ok(Some("b")));
    assert_eq!(r.rows[1].get_i64(1), Ok(Some(70)));

    // COUNT, MIN, MAX, AVG
    let r = c.query_sql("SELECT COUNT(*), MIN(val), MAX(val), AVG(val)::int FROM agg_data").expect("agg");
    assert_eq!(r.rows[0].get_i64(0), Ok(Some(5)));
    assert_eq!(r.rows[0].get_i32(1), Ok(Some(10)));
    assert_eq!(r.rows[0].get_i32(2), Ok(Some(50)));
...
fn sql_keyset_pagination() {
    ...
    let last_id = page1.rows[4].get_i32(0).expect("last id decodes").expect("last id present");
    ...
    assert_eq!(page2.rows[0].get_i32(0), Ok(Some(last_id + 1)));
...
fn sql_system_and_meta_queries() {
    ...
    let v = r.rows[0].get_str(0).expect("version decodes").expect("version present");
    ...
    assert!(r.rows[0].get_str(0).expect("meta col 0 decodes").is_some());
    assert!(r.rows[0].get_str(1).expect("meta col 1 decodes").is_some());
    assert!(r.rows[0].get_str(2).expect("meta col 2 decodes").is_some());

    // pg_catalog queries
    let r = c.query_sql("SELECT count(*) FROM pg_catalog.pg_tables WHERE schemaname = 'pg_catalog'").expect("pg_tables");
    assert!(r.rows[0].get_i64(0).expect("count decodes").is_some_and(|n| n > 0));
...
fn sql_edge_cases() {
    ...
    assert_eq!(r.rows[1].get_str(0), Ok(Some(""))); // empty string
    assert_eq!(r.rows[2].get_str(0), Ok(Some(" "))); // space
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(1_000)));

    // Unicode edge cases
    let r = c.query_sql("SELECT '🦀🐘'::text, E'tab\\there'::text, E'new\\nline'::text").expect("unicode");
    assert_eq!(r.rows[0].get_str(0), Ok(Some("🦀🐘")));
    assert!(r.rows[0].get_str(1).expect("tab decodes").is_some_and(|s| s.contains('\t')));
    assert!(r.rows[0].get_str(2).expect("newline decodes").is_some_and(|s| s.contains('\n')));

    // Numeric edge cases
    let r = c.query_sql("SELECT 2147483647::int, (-2147483647-1)::int, 9223372036854775807::bigint").expect("nums");
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(i32::MAX)));
    assert_eq!(r.rows[0].get_i32(1), Ok(Some(i32::MIN)));
    assert_eq!(r.rows[0].get_i64(2), Ok(Some(i64::MAX)));

    // Boolean
    let r = c.query_sql("SELECT true, false, NULL::bool").expect("bools");
    assert_eq!(r.rows[0].get_bool(0), Ok(Some(true)));
    assert_eq!(r.rows[0].get_bool(1), Ok(Some(false)));
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(1)));
    assert_eq!(r.rows[0].get_i32(1), Ok(Some(2)));
...
fn sql_date_and_interval() {
    ...
    assert_eq!(r.rows[0].get_str(0), Ok(Some("2024-01-15")));
    assert_eq!(r.rows[0].get_str(1), Ok(Some("13:45:00")));

    let r = c.query_sql("SELECT '2024-01-15'::date + interval '1 month' + interval '2 days'").expect("interval");
    assert!(r.rows[0].get_str(0).expect("interval decodes").is_some());

    // EXTRACT
    let r = c.query_sql("SELECT EXTRACT(YEAR FROM '2024-06-15'::date)::int").expect("extract");
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(2024)));
...
fn sql_error_zoo() {
    ...
    assert_eq!(c.query_sql("SELECT count(*) FROM cv").expect("c").rows[0].get_i64(0), Ok(Some(1)));
...
fn sql_extreme_values() {
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(i32::MIN)));
    assert_eq!(r.rows[0].get_i32(1), Ok(Some(i32::MAX)));

    let r = c.query_sql("SELECT (-9223372036854775807-1)::bigint, 9223372036854775807::bigint").expect("i64 extremes");
    assert_eq!(r.rows[0].get_i64(0), Ok(Some(i64::MIN)));
    assert_eq!(r.rows[0].get_i64(1), Ok(Some(i64::MAX)));

    let r = c.query_sql("SELECT (-32767-1)::smallint, 32767::smallint").expect("i16 extremes");
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(i16::MIN as i32)));
    assert_eq!(r.rows[0].get_i32(1), Ok(Some(i16::MAX as i32)));

    // Zero
    let r = c.query_sql("SELECT 0::int, 0::bigint, 0::smallint, 0.0::float4, 0.0::float8").expect("zeros");
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(0)));
    assert_eq!(r.rows[0].get_i64(1), Ok(Some(0)));
    assert_eq!(r.rows[0].get_i32(2), Ok(Some(0)));

    // Float specials
    let r = c.query_sql("SELECT 'NaN'::float8, 'Infinity'::float8, '-Infinity'::float8").expect("float specials");
    let nan = r.rows[0].get_f64(0).expect("nan decodes").expect("nan present");
    assert!(nan.is_nan());
    let inf = r.rows[0].get_f64(1).expect("inf decodes").expect("inf present");
    assert!(inf.is_infinite() && inf > 0.0);
    let neg_inf = r.rows[0].get_f64(2).expect("neg_inf decodes").expect("neg_inf present");
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(50_000)));

    // Unicode stress
    let r = c.query_sql("SELECT '🎭🎪🎨'::text, '中文测试'::text, 'العربية'::text, '日本語テスト'::text").expect("unicode");
    assert_eq!(r.rows[0].get_str(0), Ok(Some("🎭🎪🎨")));
    assert_eq!(r.rows[0].get_str(1), Ok(Some("中文测试")));
    assert_eq!(r.rows[0].get_str(2), Ok(Some("العربية")));
    assert_eq!(r.rows[0].get_str(3), Ok(Some("日本語テスト")));
    ...
    assert_eq!(r.rows[0].get_bool(0), Ok(Some(true)));
    assert_eq!(r.rows[0].get_bool(1), Ok(Some(false)));
    assert_eq!(r.rows[0].get_bool(2), Ok(Some(false)));
    assert_eq!(r.rows[0].get_bool(3), Ok(Some(false)));
    assert_eq!(r.rows[0].get_bool(4), Ok(Some(true)));
...
fn sql_empty_and_boundary_results() {
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(1)));
    assert_eq!(r.rows[4999].get_i32(0), Ok(Some(5000)));
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(1)));
    assert_eq!(r.rows[0].get_i32(2), Ok(Some(3)));
    ...
    assert_eq!(r.rows[0].get_i32(0), Ok(Some(1)));
    assert_eq!(r.rows[0].get_str(1), Ok(Some("a")));
...
fn sql_concurrent_ddl_and_rapid_ops() {
    ...
    for i in 0..20u32 {
        ...
        assert_eq!(c.query_sql(&format!("SELECT v FROM {name}")).expect("q").rows[0].get_i32(0), Ok(Some(i as i32)));
    ...
    for _ in 0..10 {
        ...
        assert_eq!(c.query_sql("SELECT 1::int").expect("ok").rows[0].get_i32(0), Ok(Some(1)));
    }
    ...
    assert_eq!(r.rows[0].get_i64(0), Ok(Some(1))); // 1 row each × CROSS JOIN = 1 row
...
fn sql_transaction_edge_cases() {
    ...
    assert_eq!(c.query_sql("SELECT count(*) FROM txe").expect("c").rows[0].get_i64(0), Ok(Some(1)));
    ...
    assert_eq!(c.query_sql("SELECT count(*) FROM txe").expect("c").rows[0].get_i64(0), Ok(Some(2)));
    assert_eq!(c.query_sql("SELECT v FROM txe WHERE id = 10").expect("q").rows[0].get_str(0), Ok(Some("z")));
