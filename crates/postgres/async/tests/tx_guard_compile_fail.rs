...
//!
//! The async `transaction` hands its closure a borrowing [`Transaction`] guard
...
//!
//! [`Transaction`]: bsql_postgres_async::Transaction
//!
//! Regenerate the golden after an intentional diagnostic change:
//! ```sh
//! TRYBUILD=overwrite cargo test -p bsql-postgres-async --test tx_guard_compile_fail
//! ```
