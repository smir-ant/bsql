...
//! fault — it can never hang forever.
//!
...
//!   `with_statement_timeout` therefore arms a per-read client-liveness window
//!   (`statement_timeout` + `connect_timeout`); a black-holed in-flight query
//!   elapses into a classified `DriverError::Timeout` at that window instead of the
//!   kernel's `tcp_retries2` hang — WITHOUT cutting a query the server allows.
//!   Witnessed on a DIRECT connection
//!   (`direct_black_hole_query_is_bounded_with_statement_timeout`) AND a POOLED
//!   connection's in-flight query
//!   (`pooled_inflight_black_hole_query_is_bounded_with_statement_timeout`) — the
//!   pool bounds a CHECKOUT, this bounds the query running ON a checked-out
//!   connection.
//!
//! HONEST RESIDUAL (by design, not a gap): a connection with NO `statement_timeout`
//! (the consumer declined to bound query duration) keeps an unbounded in-flight
//! read on a live black-hole — a dead KERNEL is still caught by TCP keepalive, but
//! a live black-hole cannot be distinguished from a legitimate long query without a
//! query budget, so imposing a client deadline there would cut legitimate work.
//!
...
/// The same shape as the `pool_liveness` relay, extended with `kill`.
...
async fn direct_black_hole_query_is_bounded_with_statement_timeout() {
    let proxy = BlackHoleProxy::start();
    let mut conn = Connection::connect(&budgeted_config(proxy.port()))
        .await
        .expect("connect through the relay");
    let warm = conn
        .query_one_sql("SELECT 'warm'")
        .await
        .expect("warm query works");
    ...
    let got = conn.query_one_sql("SELECT 42").await;
    ...
    // Bounded near statement_timeout (300 ms) + connect_timeout (2 s); WAY under
    // tcp_retries2 (~15 min), with generous slack for a loaded parallel run.
    assert!(
...
/// The same closure on a POOLED connection's IN-FLIGHT query: the pool bounds a
/// CHECKOUT (the `reset_session` deadline), and this bounds the query running ON
/// the checked-out connection — the two together leave no unbounded window.
#[tokio::test]
#[ignore = "requires local PG (black-hole relay in front of 127.0.0.1:5432)"]
async fn pooled_inflight_black_hole_query_is_bounded_with_statement_timeout() {
    let proxy = BlackHoleProxy::start();
...
/// closes) is a classified disconnect in milliseconds, NOT a hang. This one uses
/// NO `statement_timeout` — a transport close is caught by the read returning EOF,
/// independent of any liveness window.
#[tokio::test]
#[ignore = "requires local PG (black-hole relay in front of 127.0.0.1:5432)"]
async fn direct_query_recovers_bounded_on_mid_query_fin() {
    let proxy = BlackHoleProxy::start();
...
//
// The window must NEVER cut a query the server allows. These prove the property
// directly: an under-budget query returns; an over-budget query gets the SERVER's
// `57014` (not the client `Timeout`); a runtime `SET` that RAISES the budget is
// observed so the following long query is NOT cut; and a migration that disables
// the timeout for a long op is NOT client-cut (the window is suppressed).
...
/// An under-budget query on a windowed connection RETURNS — the window never cuts
/// a query inside the server's own budget.
#[tokio::test]
#[ignore = "requires local PG"]
async fn under_budget_query_is_not_cut() {
    let mut conn = Connection::connect(&healthy_config(Duration::from_secs(5), 2))
        .await
        .expect("connect");
    let row = conn
...
/// NOT a disconnect), never by the client window — the `+ connect_timeout` margin
/// guarantees the server's abort always arrives first when the peer is alive.
#[tokio::test]
#[ignore = "requires local PG"]
async fn over_budget_query_hits_server_not_client_window() {
    // statement_timeout 500 ms; window = 500 ms + connect_timeout (10 s) ≈ 10.5 s.
    let mut conn = Connection::connect(&healthy_config(Duration::from_millis(500), 10))
        .await
        .expect("connect");
    let start = Instant::now();
    let err = conn
    ...
    // The server aborts near 500 ms, WAY before the ~10.5 s client window.
    assert!(
        elapsed < Duration::from_secs(3),
        "the server's 57014 must arrive first, took {elapsed:?}",
    );
    // The connection SURVIVES a statement_timeout abort — still usable.
    let row = conn.query_one_sql("SELECT 5::int4").await.expect("connection recovers");
    assert_eq!(row.get_i32(0), Ok(Some(5)));
}

/// THE headline MAJOR-1(b) witness: a runtime `SET statement_timeout` that RAISES
/// the budget is OBSERVED, so the window is re-derived and the following long
/// query is NOT cut — even though it far exceeds the CONNECT-time window.
#[tokio::test]
#[ignore = "requires local PG"]
async fn runtime_set_raising_the_budget_is_not_falsely_cut() {
    // Connect-time window ≈ 300 ms + 2 s = 2.3 s.
    let mut conn = Connection::connect(&healthy_config(Duration::from_millis(300), 2))
        .await
        .expect("connect");
    // Raise the server budget to 30 s at runtime — the window must follow.
    conn.execute_sql("SET statement_timeout = '30s'").await.expect("raise the budget");
    // A 3 s query the server now allows; the stale 2.3 s window WOULD have cut it.
    let start = Instant::now();
    let row = conn
...
/// WITHIN the same transaction is NOT client-cut — even though it far exceeds the
/// connect-time window. Before the tx-guard observation gap was closed, the stale
/// 2.3 s window would have falsely cut this 3 s query.
#[tokio::test]
#[ignore = "requires local PG"]
async fn tx_guard_set_raising_the_budget_is_not_falsely_cut() {
    let mut conn = Connection::connect(&healthy_config(Duration::from_millis(300), 2))
        .await
        .expect("connect");
    let elapsed = conn
        .transaction(async |tx| {
            ...
            tx.execute_sql("SET statement_timeout = '30s'").await?;
            ...
            let start = Instant::now();
            let row = tx.query_one_sql("SELECT pg_sleep(3), 9::int4").await?;
            assert_eq!(row.get_i32(1), Ok(Some(9)));
            Ok(start.elapsed())
        })
        .await
...
/// A `set_config` of `statement_timeout` — an opaque function call, not a `SET` —
/// that RAISES the budget is caught by disarm-on-suspicion: the window is dropped
/// (fail-safe), so a following long query the raised budget allows is NOT falsely
/// client-cut. Before the fix, `set_config` classified as `Unchanged` and left the
/// stale 2.3 s window armed, cutting the 3 s query.
#[tokio::test]
#[ignore = "requires local PG"]
async fn set_config_raising_the_budget_is_not_falsely_cut() {
    let mut conn = Connection::connect(&healthy_config(Duration::from_millis(300), 2))
        .await
        .expect("connect");
    // Raise the server budget to 30 s via `set_config` (read back through
    // `query_one_sql`, a verb that also observes) — the window must DISARM.
    let raised = conn
    ...
    // A 3 s query the server now allows; a stale 2.3 s window WOULD have cut it.
    let start = Instant::now();
    let row = conn
        .query_one_sql("SELECT pg_sleep(3), 9::int4")
        .await
...
/// the window is SUPPRESSED for the trusted bsql-controlled run.
#[tokio::test]
#[ignore = "requires local PG"]
async fn migration_long_op_is_not_client_cut() {
    ...
    // Isolate the `_bsql_migrations` ledger in a fresh schema.
    let mut mon = Connection::connect(&healthy_config(Duration::from_secs(30), 5))
        .await
        .expect("monitor connect");
    mon.execute_sql(&format!("CREATE SCHEMA {schema}")).await.expect("create schema");
    ...
    let row = conn.query_one_sql("SELECT 1::int4").await.expect("post-migration query");
    assert_eq!(row.get_i32(0), Ok(Some(1)));

    mon.execute_sql(&format!("DROP SCHEMA {schema} CASCADE")).await.expect("drop schema");
}
