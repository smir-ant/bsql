...
fn an_uncanceled_query_completes_normally() {
    ...
    let row = conn.query_one_sql("SELECT 7").expect("query completes");
...
fn cancel_with_no_query_running_is_harmless() {
    ...
    let value = match conn.query_one_sql("SELECT 1") {
