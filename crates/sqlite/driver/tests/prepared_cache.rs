...
fn cached_params_reuse_stays_correct() {
    ...
    conn.execute_sql("CREATE TABLE t (id INTEGER PRIMARY KEY, v TEXT NOT NULL)")
        .expect("create");
    for i in 1..=50_i64 {
        let v = format!("row-{i}");
        conn.execute_params(
...
fn cached_execute_reuse_applies_every_call() {
    ...
    let count = conn
        .query_one_sql("SELECT COUNT(*) FROM t")
        .expect("count")
        .get::<i64>(0)
        .expect("i64");
...
fn cached_select_star_reflects_schema_change() {
    let conn = Connection::open_in_memory().expect("open");
    conn.execute_sql("CREATE TABLE t (a INTEGER NOT NULL)").expect("create");
    conn.execute_sql("INSERT INTO t (a) VALUES (1)").expect("seed");

    // Cache "SELECT *" while the table has ONE column.
    let before = conn.query_sql("SELECT * FROM t").expect("select 1-col");
    assert_eq!(before.column_count(), 1, "one column before ALTER");

    // Widen the table. The cached compiled statement is now schema-stale.
    conn.execute_sql("ALTER TABLE t ADD COLUMN b TEXT DEFAULT 'x'").expect("alter");

    // Re-run the SAME "SELECT *": must see TWO columns, not the cached one.
    let after = conn.query_sql("SELECT * FROM t").expect("select 2-col");
    assert_eq!(
        after.column_count(),
        2,
...
fn cached_query_over_dropped_column_is_classified_error() {
    let conn = Connection::open_in_memory().expect("open");
    conn.execute_sql("CREATE TABLE t (a INTEGER NOT NULL, b INTEGER NOT NULL)")
        .expect("create");
    conn.execute_sql("INSERT INTO t (a, b) VALUES (1, 2)").expect("seed");

    // Cache a query naming column `b`.
    assert_eq!(
        conn.query_one_sql("SELECT b FROM t")
            .expect("select b")
            .get::<i64>(0)
            .expect("b"),
        2
    ...
    conn.execute_sql("ALTER TABLE t DROP COLUMN b").expect("drop b");

    // Re-run the cached "SELECT b": a classified error (unknown column), not a
    // stale 2 and not a panic.
    let err = conn.query_one_sql("SELECT b FROM t").expect_err("b is gone");
...
fn cache_capacity_knob_bounds_without_changing_results() {
    let conn = Connection::open_in_memory().expect("open");
    conn.execute_sql("CREATE TABLE t (id INTEGER PRIMARY KEY)").expect("create");
    for i in 1..=10_i64 {
    ...
    for lim in 1..=10_i64 {
        ...
        let got = conn.query_sql(&sql).expect("query").len();
