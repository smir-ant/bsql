...
fn busy_timeout_waits_then_classifies_and_zero_fails_immediately() {
    ...
    a.execute_raw("CREATE TABLE t(x INTEGER)").expect("create");

    // A takes the write lock and holds it for the rest of the test.
    a.execute_raw("BEGIN IMMEDIATE").expect("A acquires the write lock");
    ...
    let waited = b.execute_raw("INSERT INTO t VALUES (1)").expect_err("contended write must fail");
    ...
    let start = Instant::now();
    let immediate = b.execute_raw("INSERT INTO t VALUES (2)").expect_err("contended write must fail");
    ...
    a.execute_raw("ROLLBACK").expect("release lock");
}
