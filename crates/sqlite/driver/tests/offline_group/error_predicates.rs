...
fn unique_violation_is_a_constraint_violation() {
    ...
    conn.execute_raw("CREATE TABLE t(id INTEGER PRIMARY KEY, name TEXT UNIQUE)").expect("create");
    conn.execute_raw("INSERT INTO t VALUES (1, 'alice')").expect("first insert");

    let err = conn.execute_raw("INSERT INTO t VALUES (1, 'bob')").expect_err("duplicate PK must fail");
    ...
    let dup_name = conn
        .execute_raw("INSERT INTO t VALUES (2, 'alice')")
...
fn wal_write_conflict_is_busy() {
    ...
    reader.execute_raw("CREATE TABLE t(x INTEGER)").expect("create");
    reader.execute_raw("INSERT INTO t VALUES (0)").expect("seed");

    // Establish a read snapshot on `reader` (a deferred BEGIN takes the snapshot
    // at the first read).
    reader.execute_raw("BEGIN").expect("begin read txn");
    let seen = reader.query_raw("SELECT x FROM t").expect("read snapshot");
    assert_eq!(seen.len(), 1);

    // A concurrent autocommit write advances the WAL past `reader`'s snapshot.
    writer.execute_raw("INSERT INTO t VALUES (1)").expect("concurrent commit");

    // `reader` now tries to upgrade its stale snapshot to a write → BUSY_SNAPSHOT.
    let err = reader.execute_raw("INSERT INTO t VALUES (2)").expect_err("stale-snapshot write must fail");
    ...
    reader.execute_raw("ROLLBACK").expect("rollback");
}
