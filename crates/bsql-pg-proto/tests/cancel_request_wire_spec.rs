...
//! reachable through both `bsql_pg_proto::cancel_request_bytes`
//! (top-level re-export) and `bsql_pg_proto::wire::
...
const _ASSERT_RETURN_LEN: () = assert!(
    bsql_pg_proto::cancel_request_bytes(0, 0).len() == 16,
    "CancelRequest packet is 16 bytes: length(4) + version(4) + pid(4) + secret(4)",
);
const _ASSERT_LENGTH_FIELD: () = {
    let bytes = bsql_pg_proto::cancel_request_bytes(0, 0);
    assert!(
...
fn cancel_request_bytes_match_pg_spec_zero_payload() {
    assert_eq!(
        bsql_pg_proto::cancel_request_bytes(0, 0),
        [
...
fn cancel_request_bytes_match_pg_spec_nonzero_payload() {
    ...
    assert_eq!(
        bsql_pg_proto::cancel_request_bytes(pid, key),
        [
            0, 0, 0, 16,
            0x04, 0xd2, 0x16, 0x2e,
            0x12, 0x34, 0x56, 0x78,
...
fn cancel_request_top_level_and_module_paths_agree() {
    ...
    assert_eq!(
        bsql_pg_proto::cancel_request_bytes(pid, key),
        bsql_pg_proto::wire::cancel_request_bytes(pid, key),
    );
}

#[test]
fn cancel_request_version_const_matches_byte_literal() {
    let v = bsql_pg_proto::wire::CANCEL_REQUEST_VERSION;
    assert_eq!(
        v, 80_877_102u32,
...
fn cancel_request_length_field_includes_self() {
    ...
    let bytes = bsql_pg_proto::cancel_request_bytes(pid, 42);
...
fn cancel_request_distinct_from_ssl_and_terminate() {
    ...
    let cancel = bsql_pg_proto::cancel_request_bytes(0, 0);
    assert_ne!(
        &cancel[..],
...
fn cancel_request_negative_pid_be_signed_encoding() {
    ...
    let bytes_neg_one = bsql_pg_proto::cancel_request_bytes(-1, 0);
    assert_eq!(
        bytes_neg_one.get(8..12).unwrap_or_default(),
        &[0xff, 0xff, 0xff, 0xff][..],
        "pid = -1 encodes as 0xFFFFFFFF BE",
    );
    let bytes_min = bsql_pg_proto::cancel_request_bytes(i32::MIN, i32::MIN);
    assert_eq!(
...
fn cancel_request_max_pid_secret_be_encoding() {
    ...
    let bytes = bsql_pg_proto::cancel_request_bytes(i32::MAX, i32::MAX);
    assert_eq!(
...
fn cancel_request_pid_independent_of_secret_key() {
    ...
    let with_pid = bsql_pg_proto::cancel_request_bytes(payload, 0);
    let with_key = bsql_pg_proto::cancel_request_bytes(0, payload);
...
const _PIN_CONST_FN: () = {
    let _ = bsql_pg_proto::cancel_request_bytes(0, 0);
    let _ = bsql_pg_proto::cancel_request_bytes(i32::MAX, i32::MIN);
    let _ = bsql_pg_proto::cancel_request_bytes(-1, 1);
};
