...
/// The basename of the schema fingerprint file written into `OUT_DIR`.
pub const SCHEMA_FINGERPRINT_FILE_NAME: &str = "bsql_schema_fingerprint.txt";

/// The environment variable, set via `cargo:rustc-env`, that carries the
/// hexadecimal representation of the SHA-256 schema fingerprint to the consumer crate.
pub const SCHEMA_FINGERPRINT_ENV_VAR: &str = "BSQL_SCHEMA_FINGERPRINT";
...
fn write_catalog(catalog: &Catalog) -> Result<(), BuildError> {
    ...
    // Emitting the cryptographic SHA-256 schema fingerprint so consumers
    // can verify schema parity at compile time and run time.
    let fingerprint_hex = compute_schema_fingerprint_hex(catalog);
    let fingerprint_path = out_dir.join(SCHEMA_FINGERPRINT_FILE_NAME);
    std::fs::write(&fingerprint_path, &fingerprint_hex).map_err(|source| BuildError::WriteCatalog {
        path: fingerprint_path.clone(),
        source,
    })?;
    println!("cargo:rustc-env={SCHEMA_FINGERPRINT_ENV_VAR}={fingerprint_hex}");
...
impl Catalog {
    ...
    /// Compute the 32-byte SHA-256 cryptographic fingerprint of this catalog.
    #[must_use]
    pub fn fingerprint(&self) -> [u8; 32] {
        compute_schema_fingerprint(self)
    }

    /// Compute the hexadecimal string representation of the SHA-256 fingerprint of this catalog.
    #[must_use]
    pub fn fingerprint_hex(&self) -> String {
        compute_schema_fingerprint_hex(self)
    }
}
...
fn fold_ident(ident: &Ident) -> String {
    match ident.quote_style {
        ...
        // Unquoted: fold to lowercase, as PostgreSQL does at resolution (Unicode case folding).
...
/// Compute a deterministic 32-byte SHA-256 cryptographic fingerprint of a [`Catalog`].
///
/// The fingerprint hashes the deterministic text serialization of the catalog
/// tables, columns, data types, nullability, primary keys, and user-defined types.
/// Any alteration to the schema produces a completely distinct hash.
#[must_use]
pub fn compute_schema_fingerprint(catalog: &Catalog) -> [u8; 32] {
    use sha2::{Digest, Sha256};
    let mut hasher = Sha256::new();
    let serialized_tables = serialize(catalog);
    let serialized_user_types = serialize_user_types(&catalog.user_types);
    hasher.update(b"bsql-schema-v1:\n");
    hasher.update(serialized_tables.as_bytes());
    hasher.update(b"---user-types---\n");
    hasher.update(serialized_user_types.as_bytes());
...
/// Compute the hexadecimal string representation of the catalog's SHA-256 fingerprint.
#[must_use]
pub fn compute_schema_fingerprint_hex(catalog: &Catalog) -> String {
    const HEX_CHARS: &[u8; 16] = b"0123456789abcdef";
    let bytes = compute_schema_fingerprint(catalog);
...
mod tests {
    ...
    #[test]
    fn schema_fingerprint_is_deterministic_and_sensitive_to_changes() {
        let cat1 = catalog_from(&["CREATE TABLE t (a INT, b TEXT)"]);
        let cat2 = catalog_from(&["CREATE TABLE t (b TEXT, a INT)"]);
        assert_eq!(cat1.fingerprint(), cat2.fingerprint());
        assert_eq!(cat1.fingerprint_hex(), cat2.fingerprint_hex());
        assert_eq!(cat1.fingerprint_hex().len(), 64);

        // Different type -> different fingerprint
        let cat_diff_type = catalog_from(&["CREATE TABLE t (a BIGINT, b TEXT)"]);
        assert_ne!(cat1.fingerprint(), cat_diff_type.fingerprint());

        // Different nullability -> different fingerprint
        let cat_not_null = catalog_from(&["CREATE TABLE t (a INT NOT NULL, b TEXT)"]);
        assert_ne!(cat1.fingerprint(), cat_not_null.fingerprint());

        // Extra column -> different fingerprint
        let cat_extra_col = catalog_from(&["CREATE TABLE t (a INT, b TEXT, c BOOLEAN)"]);
        assert_ne!(cat1.fingerprint(), cat_extra_col.fingerprint());

        // Primary key -> different fingerprint
        let cat_pk = catalog_from(&["CREATE TABLE t (a INT PRIMARY KEY, b TEXT)"]);
        assert_ne!(cat1.fingerprint(), cat_pk.fingerprint());
    }
    ...
    #[test]
    fn fold_ident_unicode_cyrillic_case_folding() {
        let unquoted = Ident::new("ПОЛЬЗОВАТЕЛИ");
        assert_eq!(fold_ident(&unquoted), "пользователи");

        let quoted = Ident::with_quote('"', "ПОЛЬЗОВАТЕЛИ");
        assert_eq!(fold_ident(&quoted), "ПОЛЬЗОВАТЕЛИ");

        let mixed = Ident::new("ПользовательИмя");
        assert_eq!(fold_ident(&mixed), "пользовательимя");
    }
}
