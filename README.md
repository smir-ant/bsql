...
```toml
[dependencies]
bsql = { version = "0.25", features = ["time", "uuid"] }
```
...
```toml
[dependencies]
bsql = { version = "0.25", features = ["sqlite"] }
```
...
```toml
bsql = { version = "0.25", features = ["time", "uuid", "decimal"] }
```
...
```toml
bsql = { version = "0.25", features = ["explain"] }
```
...
bsql = { version = "0.25", default-features = false }
```
...
```toml
bsql = { version = "0.25", features = ["detect-n-plus-one"] }
```
