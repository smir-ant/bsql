...
> **1.0.0-alpha.2** — stable in shape, early in life; expect a few more alpha
> iterations before a full 1.0. On crates.io as a **pre-release**: pin
> `bsql = "1.0.0-alpha.2"` (a bare `cargo add bsql` resolves the older stable
...
[dependencies]
bsql = { version = "1.0.0-alpha.2", features = ["macros", "postgres-async"] }

[build-dependencies]
bsql-build = "1.0.0-alpha.2"
```
