...
| `bsql-common` | [`crates/common/`](crates/common/) | **Zero-dependency leaf.** Hosts the pure (no-I/O) logic both the PostgreSQL core and the embedded SQLite driver depend on — the migration checksum / ordering / drift authority + source loader, and the N+1 detector (behind an `n1` feature) — as ONE compiled source, so the cross-backend behaviour cannot silently diverge. Reaches no `sqlparser` / `bsql-build`. |
...
- **Test functions: 2331** — every `#[test]` / `#[tokio::test]` attribute in the
  ...
  ```
- **`#[ignore]` live suites (need a running database): 322**:
  ```bash
  git ls-files -z -- '*.rs' \
