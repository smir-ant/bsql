...
```toml
[dependencies]
bsql = { version = "0.24", features = ["time", "uuid"] }
```
...
```toml
[dependencies]
bsql = { version = "0.24", features = ["sqlite"] }
```
...
```toml
bsql = { version = "0.24", features = ["time", "uuid", "decimal"] }
```
...
```toml
bsql = { version = "0.24", features = ["explain"] }
```
...
bsql = { version = "0.24", default-features = false }
```
...
```toml
bsql = { version = "0.24", features = ["detect-n-plus-one"] }
```
