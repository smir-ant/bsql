...
**Priority order (§1) — REVISED 2026-04-28 after DEF-208 ship + DEF-207 audit-rescope:**
1. **Decoder perf wins (bench-gated by DEF-197):**
   - **DEF-202** — simdutf8 for `<&str>::from_pg_text`. The actual UTF-8 validate hot point at ~7.2 ns/col. simdutf8 5-10× on x86/aarch64. **Highest-leverage decoder perf win** (DEF-207's "skip UTF-8 validate on i32" framing was invalid — DEF-184 already eliminated that path).
   - **DEF-200** — per-state dispatch LUT (A7 adjacent shape). Branch-predictor-friendly bucket split; orthogonal to DEF-202 (touches dispatch, not column decode).
...
4. **Zero-cost micro-wins:** DEF-195 / DEF-203 (small scope).
