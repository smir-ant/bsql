...
| DEF-275 | **`column_decode/iter_5cols_decode_text_long_ascii` +8.3% regression-hunt** observed in DEF-244 closure bench-stable compare vs `post-def248-subb` baseline (load 1.32-2.47). Suspect: LLVM codegen drift from DEF-258's `from_static_oids_text_format` synthetic-RowDesc helper added in the prior cycle (touches `decode.rs` text-format path — not changed by DEF-244 itself, ruled out as direct cause). Confirm reproducibility under multiple clean-load samples; if real, asm-diff `from_pg_text<&str>` lowering pre/post-DEF-258 to isolate the perturbation. | EXPLORATORY |
