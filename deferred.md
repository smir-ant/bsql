...
| 2 | DEF-280 | **Safety Bundle Group — sub-bundles E + F + `.unwrap_or(&[])` sweep + push_within_fanout_budget residue** (memo `/tmp/def-rethink-safety-memo.md` 1100 LoC). 5+1+1 of originally-scoped ship-bundles **SHIPPED 2026-05-18** in sequence with asm-diff + bench-stable verification at each commit: **Bundle G** `1cf6041` (debug_assert!(false,…) glass pattern, 4 production sites + 1 tier-1 elevation), **Bundle H** `7b7a8c4` (RowStream !Send), **Bundle J** `0286840` (distinct CRATE_BUG_REPLY_ID_SENTINEL=NonZeroU64::MAX), **Bundle K** `b66dc4a` (enter_partial_mode typed-Err), **Bundle I/D''** `e02eed7` (Zeroizing<i32> stack-slot scrub for secret_key at 2 sites), **Bundle K-mirror** `83d5a09` (exit_partial_mode typed-Err + upstream-discipline removal via Approach B single source of truth), **DEF-281 Sites A/B/C/D** `9c25c1e` + `df42ab0` (silent row-truncation class closure — 4 sites in `collect_tuple`). Total +1095 / −167 LoC across 7 commits; 606 → 617 tests (+11 spec/drift pins); 1 new hostile probe (P-D280H-1); 3 new CrateBugLocus variants (PushEmittedDeliverReply, PartialModeReentry, PartialModeExitUndrained); 2 new cluster δ leaf submodules; 2 new error types (AlreadyInPartialMode, PartialModeExitUndrained). **Remaining work**: (a) **Sub-bundle E** (~80 LoC, BREAKING API): closure-scope ALL secret accessors — `Sensitive::get`, `Password::as_bytes`, `SecretBoundedStr::as_str`, `ScramSession::password_bytes` — same retention-attack pattern as Bundle D' closed for CancelRequest; pairs with DEF-279 Foundation rethink. (b) **Sub-bundle F** (~150 LoC, BREAKING): migrate sealed-supertraits → concrete-tokens — `PostStateProof` / `DescribeName` tier-1 external, tier-2 within-crate; any in-crate module can `impl Sealed + impl PostStateProof for HostileWitness` and write any `ProtoState` variant via `install_into` (mirror DEF-272 cluster δ pattern); pairs with DEF-279. (c) **`.unwrap_or(&[])` sweep continuation** (~50 LoC, no API break): pending sites at `dispatch.rs:2405/2409` (error-response field parsing), `partial_assembly.rs:470`, `action.rs:466`, `protocol.rs:3312`; row_stream sites 721/1362/1383 already have classified Err catch downstream — verified not glass. Fundamental ReadBuf primitives at `buf.rs:174/627` defer to DEF-279 (refactoring to Result returns cascades through every caller). (d) **`push_within_fanout_budget` Bundle G-residue** — Bundle G removed the dev-loud debug_assert but the underlying tier-2 structural (const-asserted-architecturally-dead) classification persists; true tier-1 elevation would require either `unsafe push_unchecked` (forbid'd) or unstable `generic_const_exprs` (capability witness on OutActions). Defer until rust-version unlocks the feature OR an architectural rethink moves to a typed-witness API. | 2 sub-bundles (E/F) + 2 follow-up sweeps remaining; SHIPPED 7/9 work items (G/H/J/K/I/K-mirror/DEF-281) closing the no-API-break tier-neutral half AND the user-visible silent-truncation class | E breaking on 5 secret accessors; F breaking on PostStateProof/DescribeName impl callers; sweep tier-neutral; push_within_fanout_budget residue tier-neutral |
