...
- Pedantic-audit Tier 4 sweep batch 4 (Cluster A pilot + 3 small-batch addenda + 2 mid-size pairs + buffer-substrate trio + type-foundation pair) — 2026-05-19 — **5 pilot files + 3× small-batch (17 files) + 2 mid-size pairs (4 files) + buffer-substrate trio (3 files) + type-foundation pair (2 files); 31 files total. All small files closed; two audit-anchor-bearing mid-size files closed; second mid-size pair (push_command.rs + session_params.rs) closed; buffer-substrate trio (buf.rs + wire.rs + write_buf.rs) closed; type-foundation pair (state.rs + ident.rs) closed with 11 audit anchors preserved.**
  (1) **command.rs** `6bfc074` — 17 historical refs → 0; net −51 LoC (161 → 110). Deleted: `DEF-246 Phase 2` Startup-variant removal narrative; `1c-3a/1c-3b/1c-3c/1c-1b/1c-1b-MVP/1c-1-multi/1c-3e/1c-5/1c-6/Phase 1a` phase markers; F83-pass-#6 pre/post `u32 → FetchRows` migration history with date. Replaced: `DEF-112: typed ...` attribution prefixes with bare current-state explanations. Kept: PG-spec section refs (`§55.2.2`, `§55.2.3` — external); CREDO §B classification rationale on the `#[allow]` vs `#[expect]` choice; the `const _: () = assert!(...)` drift-pin verbatim.
  (2) **guard.rs** `2bc5b8b` — 17 historical refs → 0; net −35 LoC. Deleted: DEF-198/DEF-160 Z2/DEF-244/DEF-269 v2 (T)/DEF-211 SAFE-07/DEF-272 cluster γ attribution prefixes; all date stamps; the standalone "IdleStateProof deleted (cluster γ)" removal narrative. Replaced: "Pre-DEF-X did Y / Post-DEF-X does Z" pre/post pairs with current-state-only descriptions; "Memo §6.2 D4-b" internal-doc cross-reference. Kept: all 4 `compile_fail` doctests (load-bearing tier-1 closure proofs); CREDO §0 wildcard-arm rationale on `#[non_exhaustive]`; "belt-and-braces" Idle re-check rationale (current-state explainer).
  (3) **password.rs** `88d063f` — 29 historical refs → 0; net −40 LoC (62 ins / 102 del). Deleted: DEF-051/DEF-093/DEF-095/DEF-154 (O) P1-5/DEF-154 (S) P1-1/DEF-185 P2-E/DEF-244/DEF-256/DEF-259/DEF-048/DEF-215/DEF-216 prefix attributions and date stamps; `MAX_PASSWORD_LEN` docstring's "shrunk 1024 → 512 B" pre/post narrative replaced with current-state 512 B justification; `# DEF-185 P2-E (audit 2026-04-24): doc sync` block (post-mortem of fixed drift bug); the `// DEF-154 (O): originally #[expect(...)]` → `// DEF-215: downgraded to #[allow]` → `// DEF-244 modernisation audit: attribute removed` historical lifecycle commentary on a deleted `clippy::large_enum_variant` annotation. Kept: MAX_PASSWORD_LEN size cascade explanation (architectural invariant); `core::error::Error` impl (additive); `try_from_bytes` bound-narrowing comment (forbid-bundle rationale, load-bearing); all `tests/startup_spec.rs` test-pin cross-references (Debug-redaction invariants); Drop-witness tests verbatim.
  (4) **error_arena.rs** `7448c22` — 35 historical refs → 0; net −93 LoC (171 ins / 264 del). Deleted: DEF-184 audit-cycle prefixes (#3 A-02/A-04/A-06/A-07/A-09/A-13; #4 P0-1/P0-2/P1-3/P1-6/P2-7/P2-8/P2-11); DEF-185 P2-G; DEF-188 schema-arena removal narrative; DEF-205 (2026-04-27) tier-1 staleness pre/post; DEF-244 modernisation prefix on `core::error::Error` impl for `ArenaError`; DEF-259 (2026-05-08) drop-witness Pre-/Post- narrative; "Tier-3 #30" prefix on `Option<Severity>` rationale; "Pre-DEF-188 SchemaArena" / "1c-5" phase markers in Single-slot design section; "Pre-(audit #3 A-06)" narrative in Staleness classification; full `# DEF-184 (A1+A13) rationale` section with Action/OutActions/StreamItem cascade narrative — replaced with "Why an arena instead of inline strings" current-state explainer; orphan history-block at ex-lines 141-155 (ErrorPayload::empty / dead_for_test removals); orphan "// DEF-184: Hash removed" comment — folded into ErrorRef type-doc as one "Hash is deliberately not derived" sentence with current-state rationale. Replaced section headers with neutral `─── … ───` form. Kept: all module-level architectural explainers (single-slot, generation wrap-safety, staleness classification); ErrorPayload Drop chain + ZeroizeOnDrop tier-1 enforcement rationale; ArenaError Empty-vs-Stale variant-shape doc; niche-pin explanations (Option<ErrorRef> 8 B / sentinel-bit / forge_for_test test-hook contract); all 4 compile_fail / drift-pin / drop-witness doctests + tests (only inline DEF-N prefixes stripped where present).
  (5) **state_setter.rs** `43e301e` — 31 historical refs → 3 (Tier-3 audit-#26 anchors retained as load-bearing CREDO classification records); net −29 LoC (109 ins / 138 del). Deleted: DEF-270 cluster (N-D letter, Phase 2 2026-05-10) module-title prefix; "# Pre-DEF-270 N-D" + "# Post-DEF-270 N-D" pre/post section headers; "DEF-270 N-D follow-up (rust-version 1.78)" attribution before the `#[diagnostic::on_unimplemented]`; "DEF-280 Bundle F Phase 1:" inline prefix on PostStateProof marker comment; "**DEF-272 cluster γ (2026-05-10)**:" prefix on `StateSetter::new` doc; "# Pre-γ (DEF-271 cluster A)" + "# Post-γ" pre/post narrative sections; "pre-DEF-186 compute_push_*" perf-recovery audit cross-reference; DEF-271 cluster A / DEF-272 cluster δ section header DEF-N prefixes; "**DEF-272 cluster δ (2026-05-10)**:" prefix on FeedStateSetter::new doc; "DEF-272 cluster δ leaf entry point:" / "DEF-280 Bundle K (2026-05-18) leaf entry point:" / "DEF-280 Bundle K-mirror" / "DEF-248 Sub-A (2026-05-12)" attribution prefixes from 7 free-fn docstrings; Pre-DEF-271 / Post-DEF-271 pre/post pair in `drain_and_install_errored` docstring; "DEF-280 Bundle F Phase 1 (2026-05-18) — InstallBody seal pin anchor" doc-comment prefix; "(DEF-270 N-D pattern)" + "(DEF-280 Bundle F)" attribution parentheticals from the `#[diagnostic::on_unimplemented]` note string; ═══ section header DEF-N prefixes stripped (5 headers; all ═══ visual separators preserved verbatim). Fixed dangling test-name cross-reference at `install_body_seal_pin_anchor`: `bundle_f_hostile_witness_install_body_absent` → `hostile_witness_install_body_absent_anchor` (matches actual test name in push_command.rs:895). Kept: 3 Tier-3 audit #26 anchors (Bundle F closure, cluster δ closure, cluster γ closure) with verbatim "Tier-3 audit #26, 2026-05-19, verified" classification — these are explicit audit-record anchors preserving the closure proof from future re-audit; the closure-mechanism descriptor inside each anchor body lifted from DEF-N labels ("Bundle F") to architecture-name labels ("InstallBodySealed private supertrait", "per-leaf token gating", "lifetime-bound typestate") — current-state-only. Kept: every architectural rationale block (tier-1 by-construction proofs); all 7 install body impls; both compile_fail-anchor tests.
  (6) **Small-batch addendum #1** `b215b11` — 7 files, 31 historical refs → 0; net −22 LoC (101 ins / 123 del). Files: partial_assembly.rs (3→0); pristine.rs (3→0); scram/crypto.rs (3→0); scram/mod.rs (5→0); secret_zeroize.rs (5→0); frame.rs (6→0); sensitive.rs (6→0). Deleted: DEF-248 Sub-B / DEF-2XX / DEF-280 sweep + Tier-4 Cluster D #58 (partial_assembly); BS-11 / DEF-210 / DEF-211 INNO-01 + Pre-INNO-01 / Post-INNO-01 pre/post narrative + `deferred.md` internal-memo cross-refs (pristine); 3× "DEF-185 P0-D (audit 2026-04-24)" Zeroizing attribution prefixes (scram/crypto); DEF-META-01 expert-domain attribution + DEF-039 / DEF-040 type-pointer attributions + audit-A2 typestate attribution + DEF-184 audit-P0-1 hygiene attribution + DEF-053 channel-binding phase-deferral (scram/mod); DEF-211 SAFE-06 module-title attribution + BS-01 reference + 2× Phase 1d phase markers + DEF-211 SAFE-06 prefix on `#[expect(reason = ...)]` rationale (secret_zeroize); Phase 1a phase marker + DEF-211 FAKE-11 const-assert attribution + DEF-154 (G) pre-(G) `total_len: usize` pre/post narrative on field doc + DEF-154 (G) inline narrowing prefix + DEF-144 + pre-DEF-144 + audit-A015 historical-removal attribution (frame); Per-DEF-048 / DEF-093 / DEF-280 Bundle E Pre-Bundle E/Post-Bundle E pre/post narrative on `with_inner` + DEF-259 Pre-DEF-259/Post-DEF-259 narrative on drop_witness_tests module doc (sensitive). Kept: every architectural rationale block, all test bodies, all `#[expect(...)]` / `#[must_use = "..."]` attributes (with attribution prefixes stripped from reason strings where present but the rationale text preserved); the `protocol.rs` cross-ref in pristine.rs (points to actual code, not internal-memo); SCRAM exchange-flow diagram + security-discipline policy bullets in scram/mod.rs.
  (7) **Small-batch addendum #2** `a5be3d4` — 5 files, 54 historical refs → 0; net −48 LoC (171 ins / 219 del). Files: scram/types.rs (11→0); scram/session.rs (12→0); md5.rs (12→0); cancel.rs (12→0); bounded.rs (7→0). Deleted: DEF-039 / DEF-040 / DEF-093 attribution parentheticals + DEF-244 modernisation prefix on `core::error::Error` impl + DEF-040 regression-guard test-doc attribution + DEF-259 pre/post narrative on drop_witness_tests doc (scram/types); "audit 2026-04-19 finding A2, tightened by DEF-097" pre/post narrative on module doc + F-026 pass-#8 + Pre-F-026 narrative on Zeroize/ZeroizeOnDrop derive doc + DEF-210 PERF-02 + Pre-PERF-02/Post-PERF-02 pre/post pair on single-Box handshake state + DEF-205 step 4 / DEF-206 audit / DEF-097 attribution on `#[zeroize(skip)]` rationale + DEF-280 Bundle E + Pre-Bundle E/Post-Bundle E pre/post narrative on `with_password_bytes` + DEF-259 Pre-/Post- narrative on ScramSession drop_witness_tests doc (scram/session); DEF-216 (2026-05-05) module-title + 3× DEF-216 audit (2026-05-07) prefixes on Md5HandshakeState / MD5_RESPONSE_BODY_LEN / hex_lowercase / nibble_to_lowercase_hex / write_hex_lowercase / Tier-1 ZeroizeOnDrop enforcement docs + DEF-META-01 policy 9 attribution + Phase 1e driver-wrapper phase marker + DEF-244 let-chains modernisation attribution + DEF-259 Pre-/Post- narrative on Md5 drop_witness_tests doc (md5); DEF-278 Bundle D' module-title + Bundle D' elevation pre/post narrative + Phase 1e phase marker + DEF-185 P0-A panic-abort attribution + 6× DEF-278 Bundle D / D' / DEF-272 cluster α/β/Sub-B attribution prefixes on BackendKey / BackendKeyCell / empty / install_via_token / as_inner / CANCEL_REQUEST_LEN docs + Per-DEF-048 attribution on Sensitive<T>-Debug-redaction rule (cancel); DEF-203 module-title + DEF-210 CF-02 build-fail section header + DEF-244 modernisation attribution + DEF-210 ML-01 historical-removal attribution + DEF-210 CF-02 (audit 2026-04-28) + Pre-CF-02 narrative on BoundedU8 trait impl + DEF-210 ML-01 / CF-02 sister-comment references in BoundedU16 mirror block (bounded). Kept across all 5 files: every architectural rationale block, all test bodies, all `#[diagnostic::on_unimplemented]` notes (with DEF-N attribution removed), all `compile_fail` doctests on BoundedU8/U16 carriers, the SCRAM-handshake exchange-flow diagram, the cancel.rs lifecycle table.
  (8) **Small-batch addendum #3** `e59819e` — 5 files, 49 historical refs → 1 audit anchor; net −44 LoC (162 ins / 206 del). Files: schema_slot.rs (8→0); session_params_slot.rs (8→0); drop_witness.rs (10→0); params.rs (11→0); prepared.rs (12→1 audit anchor retained).
  (9) **Mid-size audit-anchor pair** `0fbc0ab` — 2 files, 60 historical refs → 2 audit anchors; net −78 LoC (279 ins / 357 del). Files: row_stream.rs (25→1 Tier-4 audit #59 anchor on POST_ERROR_SENTINEL_REPLY_ID); reply_id.rs (35→1 Tier-3 audit #17 anchor on MintSaturated / mint_or_saturate).
  (10) **Mid-size pair #2** `aadd19f` — 2 files, 103 historical refs → 0; net −75 LoC (302 ins / 377 del). Files: push_command.rs (48→0), session_params.rs (55→0). No audit anchors in either. Pattern at this scale: per-comment narrative rewrites are still the dominant cost (Pre-/Post- pairs → current-state-only with naive-alternative hypotheticals); attribution-prefix stripping is bulk replace via `replace_all` where the same comment shape appears 5-6 times (e.g. the 6× "DEF-280 Bundle F Phase 1: install body moved" → "Install body lives in `state_setter::InstallBody` impl" collapse). Also: **renamed `mod bundle_f_seal_probe` → `mod hostile_witness_seal_probe`** in push_command.rs to drop the DEF-N+Bundle-letter name and align with Cluster A's grep-friendly naming convention; the cross-reference at `state_setter.rs:710` was updated in the same commit. Deleted: DEF-269 v2 (T) module-title + Pre-/Post-DEF-269-v2 pre/post (replaced with "Why per-command structs and not a runtime enum"); DEF-272 cluster γ / pre-γ IdleStateProof attributions; DEF-112 kind-parameterisation attribution; DEF-246 Phase 2 import-removal comment + Phase 2 cross-refs in 3 sites; DEF-244 macro Phase 2 attribution on `Output`; DEF-270 N-D (Phase 2) attributions on PostState + execute doc; DEF-280 Bundle F Phase 1 attribution on `: InstallBody` bound; DEF-160 Z2 (2026-05-11) attributions on `'sql` lifetime + `sql: &'a str` fields (×2); DEF-246 Phase 2 attribution on Startup-not-here comment; DEF-270 Phase 2 (N-D, 2026-05-10) section header + Pre-DEF-270-N-D PgCommand backwards-compat-removed history; 6× DEF-280 Bundle F Phase 1 install-body-moved comments; DEF-272 cluster α + DEF-270 + deferred.md DEF-270 internal-memo cross-ref; DEF-244 (2026-05-13) BindPrepared section header + DEF-198 / DEF-270 N-D / memo §5.3 / §10.5 internal-memo cross-refs + DEF-270 N-D PostState attribution; DEF-269 v2 (T) Size pins section header + DEF-160 (Z2) / pre-DEF-160 / post-DEF-160 / DEF-269 v2 (T) pre/post on parse_size pin; DEF-280 Bundle F Phase 1 (2026-05-18) hostile-witness section divider + Post-Bundle-F attributions on the (2) and (4) seal-probe comments (push_command); DEF-042 / DEF-114 / DEF-106 / DEF-153 / DEF-185 P2-B / DEF-185 P2-3 / DEF-186 P1-5 / DEF-189 Q8-C3 / DEF-203 / DEF-211 INNO-01 + Pre-INNO-01/Post-INNO-01 + DEF-210 BS-11 + REC-08 origin block + F55 (pass #6 audit) / F22 cross-ref + F-073 (pass-#8) Phase 1e driver-wrapper attribution + F-074 (pass-#8) attribution + architect finding #66 (2026-04-21) attribution + DEF-096 / DEF-099 historical-derivation refs + DEF-154 (S) P1-1 pre-(S) attribution — all replaced with current-state explainers + naive-alternative hypotheticals (session_params). Pattern at mid-size scale: the outer audit-anchor wrap stays verbatim; the inside-body pre/post narrative gets rewritten to current-state language with the hypothetical alternative form ("A naive `X` form would silently…") replacing the historical "Pre-audit, the call site used X…" framing. Also fixed a stale doc-bug in reply_id.rs: the "Consume discipline" section referenced "Dropping a ReplyId is a runtime failure (DEF-101)" but the Drop impl had been deleted (panic-in-Drop is a footgun under integration-test unwind); rewrote to point at `#[must_use]` as the enforcement mechanism. Deleted: DEF-248 Sub-A module-title + Pre-DEF-248 pre/post + memo §3.2/§3.3/§3.4 broken external-file refs + Sub-A scope/Sub-B planning + DEF-280 Bundle H/K/K-mirror attributions + DEF-281 Site B/A/A-mirror/A-mirror chunk/E+F/G/H/I attribution prefixes + DEF-244 (2026-05-13) on `collect_tuple` + DEF-244 modernisation audits (1.82 is_none_or + 1.88 let-chains) + DEF-189-style fused-classification attribution + DEF-272 cluster δ "Mirror of" cross-ref + DEF-244 modernisation on `_row_stream_partial_leaf` mod + DEF-280 Bundle K spec-tests pre/post (also renamed `bundle_k_spec_tests` mod → `partial_mode_tier1_spec`) + 4× "Bundle K"/"Bundle K-mirror" assertion-message prefixes (row_stream); "(DEF-112)" module-title parenthetical + Pre-DEF-112 / DEF-112-elevates pre/post + DEF-270 cluster / Pre-/Post- with reforge.md §7.5 internal-memo cross-ref + DEF-280 Bundle J module-title + Pre-/Post-Bundle J pre/post on CRATE_BUG_REPLY_ID_SENTINEL + Sealed (DEF-112) attribution + DEF-112 follow-up modernisation + DEF-119/DEF-188/DEF-210 SR-01 staged-vs-public section header + DEF-210 SR-01 Path C + Phase 1c sub-phase markers + DEF-101 cross-ref on consume discipline + DEF-163 A006 not-zeroized header + 3× DEF-154 (K) historical-deletion comment blocks + DEF-270 U-letter Pre-/Post- visibility narrative + DEF-088/DEF-112 attribution on NOT-PartialEq comment + reforge.md §4.11 internal-memo cross-ref (reply_id). Deleted: DEF-272 cluster α/β module-title prefixes + Pre-/Post- pre/post sections on both slot cells (replaced with current-state "Architecture" sections + sealed-trait-alternative analysis); architect-hostile-probe attribution; DEF-272 P6 closure attribution on empty-cell docs (×2); DEF-279 follow-up attribution on schema_slot Cell `pub` doc + `#[allow]` comment; pre-DEF-272 cost-comparison reference; "Pre-β"/"Post-β" pre/post on session_params_slot lazy-init + bench-cost sections; DEF-185 P2-B / P2-3 operator-canary attributions on admit_at_* docs; DEF-189 Q8-C3 + DEF-205 step 3 attribution on clear_at_residue; DEF-259 module-title + Pre-DEF-259/Post-DEF-259 narrative on drop_witness module doc + DEF-259 prefix on the `#[allow(dead_code, reason)]` rationale + DEF-259 manifest section divider + DEF-265 Idea-38 ReadBuf attribution; Phase 1c-3b + "# Scope (1c-3b)" + DEF-135 + 1c-5 + DEF-143 + deferred.md §21 F-068 internal-memo cross-refs on params; DEF-115-class seal attribution + 2× DEF-244 follow-up modernisation prefixes on params `#[diagnostic::on_unimplemented]` blocks + DEF-244 memo §7 P8 closure references in note strings + DEF-154 (B) Phase B4-W P0-3 + P2 attribution on OverflowParams test-only struct + "in 1c-3b" phase marker on FORMATS doc (params); DEF-244 module-title + broken `/tmp/def244-design-memo.md` external ref + memo §7 P8 / DEF-248 Sub-A precedent / memo §5.4 / DEF-228 / "memo §7 P8-equivalent" / DEF-035 stmt-cache + 1c-6 portal-Close phase-marker chain + "memo §7 P3-P12" internal-memo references throughout prepared.rs (the audit-anchor-internal "memo §7 P3-P12" sentence was tidied to remove the internal-memo cross-ref while preserving the audit anchor itself). Kept: 1 Tier-3 audit #1+#27+#28 (2026-05-19) DEFERRED-with-rationale anchor at prepared.rs:476 — preserved verbatim per the established audit-anchor pattern (load-bearing CREDO classification record); every architectural rationale block; all sealed-trait + token-gating mechanism explainers; both `#[diagnostic::on_unimplemented]` attributes; all `compile_fail` / `compile`-anchor doctests; drop_witness manifest impls.
  (11) **Buffer-substrate trio** `83110e3` — 3 files, ~199 historical refs → 4 audit anchors (Tier-3 audit #34 anchors on `enter_partial_mode` / `exit_partial_mode` / `subtract_partial_remaining` / `partial_remaining` field, all in buf.rs, all preserved verbatim per the established audit-anchor pattern); net −221 LoC (651 ins / 872 del). Files: buf.rs (72→4 audit anchors), wire.rs (61→0), write_buf.rs (66→0). Pattern: buffer-substrate files are heavy on DEF-185 P0-A/B/C zeroize discipline narratives, DEF-204 staleness-leak closure narratives, DEF-265 Idea-38 two-tier design history, DEF-248 Sub-A partial-frame substrate scaffolding, DEF-280 Bundle K/K-mirror reentry-protection narratives, and DEF-154 (B) Phase B1 GhostCell-style brand scaffolding (write_buf.rs has the heaviest concentration: ~60 LoC of phase-doc on the BrandedWriteBuf/BrandedWriteReserved types whose `'brand` phantom was retired but the wrapper names retained for builder ergonomics — rewrote as a single-paragraph current-state explainer noting the retired generative-lifetime design). Deleted across the trio: DEF-185 P0-A/B/C audit prefixes + dates, DEF-204 / DEF-265 / DEF-248 / DEF-280 / DEF-244 / DEF-212 / DEF-215 / DEF-216 / DEF-184 / DEF-094 / DEF-057 / DEF-120 / DEF-199 / DEF-150 / DEF-058 / DEF-154 (G/H/K/M/N/O/Q/R/S/T/U/V/W) / DEF-160 Z2 / DEF-094 (R2) / DEF-111 / DEF-116 / DEF-115 / DEF-088 / DEF-094 / DEF-101 / DEF-119 / DEF-188 / DEF-189 / DEF-210 / DEF-211 / DEF-272 attribution prefixes; F-014 / F-015 / F-026 / F33 / F-046 / F-073 / F-074 / F61 pass-#N labels; 1c-1b / 1c-3a / 1c-3b / 1c-3c / 1c-3e / 1c-5 / 1c-6 / Phase 1a / Phase 1b / Phase 1c / Phase 1e / Phase B1 / B2 / B3 / B4 / B5 phase markers; 2026-04-19 / 2026-04-24 / 2026-04-27 / 2026-04-28 / 2026-05-04 / 2026-05-05 / 2026-05-07 / 2026-05-08 / 2026-05-10 / 2026-05-11 / 2026-05-12 / 2026-05-13 / 2026-05-17 / 2026-05-18 date stamps; "memo §1/§3.2/§3.3/§3.4/§5.3/§5.4/§7/§10.5" internal-memo cross-refs; Bundle K-mirror "Approach B" / Idea-38 design labels; "Pre-DEF-X did Y / Post-DEF-X does Z" pre/post pairs (rewritten as "A naive X would Y; instead Z does W" current-state with naive-alternative hypotheticals); DEF-154 (W) `BrandedBytes<'brand, 'a>` tombstone block + DEF-154 (W) `_brand: PhantomData<...>` DELETED inline tombstones (×2); DEF-160 Z2 `branded_reserve_as_bytes_len_mirrors_buf` test deletion tombstone; DEF-154 (W) `branded_bytes_empty_is_empty` test deletion tombstone; the full DEF-154 (B) Phase B1 generative-branding scaffolding doc-block (~60 LoC of "Phase B1 scope" / "Phase B1 does NOT" / "TEMPORARY through Phase B2" / "Invariance mechanism" / "Generativity via HRTB" — replaced with a 14-line current-state explainer). Renamed (none — all type/module names already grep-neutral). Stale doc cross-refs fixed: removed broken `write_buf.rs:673` / `buf.rs:382` line-number references inside drop_witness_tests modules. Kept across all 3 files: 4× Tier-3 audit #34 anchors on the partial-mode substrate (buf.rs `partial_remaining` field, `enter_partial_mode`, `exit_partial_mode`, `subtract_partial_remaining`); Tier-3 audit #19 saturation-classification anchor in `subtract_partial_remaining`; every architectural rationale block (single-tier-vs-two-tier ReadBuf perf-rationale, capacity drift-pin chains, zeroize-on-clear discipline narratives, manual-Drop rationale + panic-abort caveat); all PG-spec section refs (§55.2.2 / §55.7); all `const _: () = assert!(...)` drift-pins verbatim; SCRAM password-correlation security rationale (leak vector descriptions in `clear()` / `compact()` / `Drop` docstrings); the 2 BrandedWriteBuf/BrandedWriteReserved type docs (with current-state replacement); all 6 drop-witness tests; all 3 phase_b1_tests + the size drift-pin test; the DEF-248 Sub-A partial-frame substrate divider (rewritten to neutral current-state form). **Three clippy `doc_lazy_continuation` errors triggered post-rewrap where `+` ended up at line-start (markdown bullet)** — re-wrapped to keep `+` mid-line. Gates: clippy `--all-targets -D warnings` clean; lib 240/0/0; full-suite multi-file 700+/0/0; doctests compile.
  **Pattern established for future sweeps**: per-comment evaluation. KEEP iff (current-state explainer OR architectural invariant OR external-spec ref OR load-bearing test/doctest OR CREDO classification rule including Tier-3 audit-verification anchors). DELETE iff (pre-X/post-X narrative OR DEF-N attribution OR date stamp OR phase marker (`1c-*`, `Phase 1a`) OR future-roadmap reference OR internal-memo cross-ref). Stale `narrow_len_u16`-style references to deleted helpers also DELETE. **For audit-anchor blocks** (the 3 state_setter.rs audit-#26 anchors + the 4 buf.rs audit-#34 anchors are the prototype): KEEP the anchor wrap + verification timestamp + closure-state proof verbatim; REWRITE the inside descriptor to current-state architecture terms (drop DEF-N labels in favour of architecture names). **For tightly-scoped single-rationale comments** (small-batch files, ≤6 refs): per-comment edits work well; the DEF-N prefix gets stripped and the current-state explanation preserved as-is, no narrative restructuring needed. **For buffer-substrate-style files heavy on phase-doc** (Phase B1 / Phase 1c / Idea-38 scaffolding): replace the entire phase-doc section with a single-paragraph current-state explainer + naive-alternative hypothetical; do NOT preserve historical phase markers under any rewrap.
  **Clippy `doc_lazy_continuation` watch-out**: when rewrapping documentation paragraphs that contain `+` characters, ensure the `+` does NOT land at the start of a wrapped line — markdown parses leading `+` as a bullet. Three sites tripped during this batch (rewrapped to keep `+` mid-line). Same applies to leading `-` or `*` (less common in prose but possible).
  Gates per file: clippy `--all-targets -D warnings` clean; tests pass (567/0/8 workspace for command.rs/guard.rs/password.rs; 240/0/0 lib for all subsequent batches); doctests compile.
  (12) **Type-foundation pair** `4cafed2` — 2 files, 203 historical refs → 11 audit anchors; net −83 LoC (480 ins / 563 del). Files: state.rs (82→0 — no audit anchors), ident.rs (121→11 audit anchors). Pattern at the type-foundation tier: state.rs is heavy on DEF-279 Phase 1c Bundle Commit 1/3/7 + Phase 2 Commit 9 scaffolding doc (per-phase state enum split: `ConnectingState` / `ActiveState` / `ErroredState` + their `From`/`TryFrom` bijections + manual `Debug` impls); replaced with current-state explainer noting the per-phase Inner wrapping rationale. Deleted: DEF-279 Phase 1c (Commit 1/3/7/9) module-section + scaffolding-narrative attribution; DEF-001..DEF-004 / DEF-097 / DEF-187 / DEF-189 / DEF-210 SR-01/SR-03/SR-07/REC-06 / DEF-215 / DEF-216 audit prefixes throughout state.rs variant docs; DEF-211 FAKE-16 / DEF-205 step 4 / DEF-206 / DEF-061 / DEF-142 / DEF-138 / DEF-146 / DEF-178 / DEF-169 prefixes; PERF-02 attribution + audit dates 2026-04-25/-26/-28 + 2026-05-04/-08 + Phase 1b/1c/1c-1b/1c-3a/1c-3b/1c-3c/1c-5 phase markers; "audit2 A003/A005/A029/A038" / "A10/B22 revert 2026-04-24" / "Round-4 finding #2 (2026-04-20)" attribution; "H021 witness-guard session" / "B002" cross-refs; DEF-186 P1-6 audit prefix on the `Errored(_) => None` arm comment. Inside-doc Pre-DEF-X / Post-DEF-X narratives ("Pre-Path-C this variant carried..." / "Path C eliminates...") rewritten to current-state language with naive-alternative hypotheticals ("A naive shape would carry... Instead the single source of truth is..."). ident.rs: stripped DEF-096 / DEF-099 / DEF-115 / DEF-126 / DEF-154 (R) / DEF-154 (T) P1-2 / DEF-163 G016 / DEF-184 audit-2 / DEF-185 P2-D / DEF-203 ext / DEF-205 / DEF-244 / DEF-256 / DEF-259 attribution prefixes throughout; Phase 1c phase markers + "Round-4 finding #2" attribution + "1c-3c F12 (pass-#7 audit)" section header. The DEF-126 "Why `"…"` and not `"~"`" rationale block preserved verbatim (architectural decision record, current-state explainer). Audit-anchor preservation pattern proven again at scale — 11 anchors preserved verbatim in ident.rs (the per-anchor inside-body still retains the "Tier-3 audit #N" / "Tier-4 Cluster D #N" marker + verification timestamp; DEF-N prefixes inside the body of audit anchors stripped where present, e.g. "DEF-154 (T) P1-2 + Tier-4 Cluster D #56 (2026-05-19): narrow usize → LenT" → "Tier-4 Cluster D #56 (2026-05-19): narrow usize → LenT"). Two clippy `doc_lazy_continuation` errors triggered post-rewrap where `+` ended up at line-start — rewrapped to keep `+` mid-line. Gates: clippy `--all-targets -D warnings` clean; lib 240/0/0; all-targets multi-file 700+/0/0; doctests compile.
  **Pattern established for future sweeps**: per-comment evaluation. KEEP iff (current-state explainer OR architectural invariant OR external-spec ref OR load-bearing test/doctest OR CREDO classification rule including Tier-3 audit-verification anchors). DELETE iff (pre-X/post-X narrative OR DEF-N attribution OR date stamp OR phase marker (`1c-*`, `Phase 1a`) OR future-roadmap reference OR internal-memo cross-ref). Stale `narrow_len_u16`-style references to deleted helpers also DELETE. **For audit-anchor blocks** (state_setter.rs audit-#26, buf.rs audit-#34, ident.rs audit-#46/#48 + Tier-4 Cluster D #54/#55/#56 anchors are the prototype): KEEP the anchor wrap + verification timestamp + closure-state proof verbatim; REWRITE the inside descriptor to current-state architecture terms (drop DEF-N labels in favour of architecture names). **For tightly-scoped single-rationale comments** (small-batch files, ≤6 refs): per-comment edits work well. **For buffer-substrate-style files heavy on phase-doc** (Phase B1 / Phase 1c / Idea-38 scaffolding): replace the entire phase-doc section with a single-paragraph current-state explainer + naive-alternative hypothetical; do NOT preserve historical phase markers under any rewrap.
  **Clippy `doc_lazy_continuation` watch-out**: when rewrapping documentation paragraphs that contain `+` characters, ensure the `+` does NOT land at the start of a wrapped line — markdown parses leading `+` as a bullet. Five sites tripped across batches 11+12 (rewrapped each time). Same applies to leading `-` or `*` (less common in prose but possible).
  Gates per file: clippy `--all-targets -D warnings` clean; tests pass (567/0/8 workspace for command.rs/guard.rs/password.rs; 240/0/0 lib for all subsequent batches); doctests compile.
  **Remaining Cluster A scope** — **mid-size & protocol.rs only; 9 mid-size files closed (row_stream, reply_id, push_command, session_params, buf, wire, write_buf, state, ident)**:
  - protocol.rs (554 refs, ~9000 LoC) — single largest target; could be staged as section-by-section sub-batches.
  - dispatch.rs (145), action.rs (130), decode.rs (125), lib.rs (120), error.rs (100) — top mid-size cluster.
  Total remaining: ~1174 refs across 6 files. Recommended: ≤3 files per mid-size session, ≤1 file per session for protocol.rs sub-batches.
- Pedantic-audit Tier 4 sweep batch 3 (Cluster B partial) — 2026-05-19 — **1 item partial closure**.
  (1) **Cluster B** `fb5325e` — `Staged*Payload` field visibility tightened from `pub` (with `#[doc(hidden)]`) to `pub(crate)` across 3 types (StagedQueryCompletePayload command_tag+tx_status, StagedDescribeStatementCompletePayload param_oids+tx_status, StagedDescribePortalCompletePayload tx_status). Pre-Cluster-B: external code could write `Staged…Payload { … }` struct literal and build a `StagedReply` via the public `From` impls. Post-Cluster-B: struct-literal path closed; only in-crate `From` impls construct. The audit's full `__macro_internals` umbrella migration is BLOCKED — `pub trait ReplyKind: sealed::Sealed { type StagedPayload: Into<crate::action::StagedReply> + … }` forces `StagedReply` to be `pub`, and a `pub mod __macro_internals` re-export can't raise `pub(crate)` to `pub` (only reduce). The trait-bound redesign is BREAKING-API territory not in scope for v1.0 polish. 5 field-level `#[doc(hidden)]` attributes removed (now redundant with `pub(crate)`). `PongPayload` / `StartupCompletePayload` / `ParseCompletePayload` / `CloseCompletePayload` retained `pub` fields — those are legitimate USER-FACING types, not macro plumbing. Type-level `#[doc(hidden)]` retained on the Staged* sums.
  Gates: clippy `--all-targets -D warnings` clean; 567/0/8 tests pass.
- Pedantic-audit Tier 4 sweep batch 2 (Cluster D + #56 doc) — 2026-05-19 — **6 items closed by code + 3 documented as audit-recommendation-not-applicable, across 3 commits**.
  (1) **#54+#58+#82+#83 + #55 doc** `e1a6aaa` — per-site `unwrap_or` lift batch. (#54) `FixedStr::as_bytes`: `self.buf.get(..self.len()).unwrap_or(&[])` → `match self.buf.split_at_checked(self.len()) { Some((h, _)) => h, None => &[] }`. Same dead-arm sink, explicit match form, matches PodBytes::as_slice mirror site. asm-diff `as_bytes`: 0 codegen delta. (#58) `PartialAssemblyCell::absorb`: layered `if copy_take > bytes.len()` + `bytes.get(..copy_take).unwrap_or(&[])` collapsed into single `match bytes.split_at_checked(copy_take)` form — one fewer defensive check, same fail-closed semantic on dead None arm. asm-diff `absorb`: 0 codegen delta. (#82) decode.rs test fixture `from_utf8(input).unwrap_or("?")` → `alloc::string::String::from_utf8_lossy(input)` (Cow Borrowed for valid UTF-8, no alloc; Owned U+FFFD substitution stronger than `"?"`). (#83) decode.rs test fixture `(10_u64).pow(u32::try_from(len).unwrap_or(0)).saturating_sub(1)` three-layer dead-fallback chain → explicit `(len, all_nines_value)` tuple table; no pow / try_from / unwrap_or. (#55) `FixedStr::as_str`: audit's `Utf8Bytes<'a>` newtype lift DOES NOT FIT a self-borrowed accessor (no input bytes parameter to type); `unwrap_or` not in forbid list (only `unwrap`/`expect` are) — annotated rationale, kept current form.
  (2) **#85** `a34a4f4` — `scram::crypto::tests::rfc7677_appendix_a_full_proof` had two three-layer fallback chains (`base64_encode_to_buf(...).unwrap_or(0)` → `buf.get(..len).unwrap_or(&[])` → `from_utf8(...).unwrap_or("")`) for base64 verification. Migrated to direct `base64ct::Base64::encode(input, &mut buf)` which returns `Result<&str, InvalidLengthError>` borrowing into the caller buf. Single `unwrap_or("")` per verification instead of three layered fallbacks. Net −4 LoC, same RFC 7677 test coverage.
  (3) **#56 doc** `837ca9f` — annotated 4 `LenT::try_new_usize(value).unwrap_or_default()` sites in ident.rs. Audit's recommended structural lift (surface overflow as `Result<(), IdentError::TooLong>`) is BREAKING-API blocked — these are inside the INFALLIBLE `from_str_truncating` / `from_bytes_lossy` constructors whose API contract is "always succeed, truncate with marker on overflow"; Result return would ripple through 40+ call sites. A `LenT::saturating_new_usize` helper also blocked under forbid-bundle (usize → u8/u16 requires `as` or `try_from`). Centralised audit-#56 rationale block at the first site, brief invariant notes at the other 3, stale `narrow_len_u16` helper references cleaned up.
  (4) **#84+#95** — `must_parse` test fixture pattern (assert!(is_ok) + unwrap_or(default)) verified optimal under forbid-bundle (panic / unwrap / expect / unreachable / assert!(false) all forbidden, including in test code). No further migration possible; existing doc-comment at decode.rs:3260 already documents the pattern. No code change needed.
  Gates: clippy `--all-targets -D warnings` clean; 567/0/8 tests pass; asm-diff zero codegen delta on `as_bytes`, `absorb`, `col_next`.
- Pedantic-audit Tier 4 sweep batch 1 — 2026-05-19 — **2 items addressed (1 closed by code, 1 DEFERRED with rationale)**.
  (1) **#59** `3511997` — introduced `pub(crate) const POST_ERROR_SENTINEL_REPLY_ID: NonZeroU64 = NonZeroU64::MAX` in `row_stream.rs` with anchored docstring explaining the sentinel rationale (wrapper matches on `outcome::Err` cause not id equality; `NonZeroU64::MAX` cannot collide with monotonic-fetch_add-from-1 real ids; BREAKING `Option<NonZeroU64>` lift in `ColEvent::EndQuery` deferred to post-1.0 API redesign). All 17 `cached.unwrap_or(NonZeroU64::MAX)` occurrences migrated to use the named constant. asm-diff `col_next`: 0 codegen delta. Scattered sentinel literal → single source of truth.
  (2) **Cluster C** `649fed8` — **DEFERRED**. Audit proposed migrating `_token: TokenType` leaf-token consumer parameters to `token: &TokenType` (by-ref) across ~14 production sites + 2 macro patterns, motivated by "drop the underscore prefix; non-underscore name is fine with `&T`". Investigation found the proposal weakens tier-1: all leaf tokens EXCEPT `ProtoInitToken` are non-Copy ZST. Today, by-value parameters give compile-checked single-use enforcement — a refactor that double-passes the token to two state-mutating methods fails the move-checker. Switching to `&TokenType` erases that single-use shield. The audit's stated win is purely aesthetic (`_` prefix is the canonical Rust idiom for "intentionally unused parameter", not a code smell). Documented analysis as anchored comment block in protocol.rs above `_bind_execute_select_install_leaf` so future audits don't re-derive the impossibility proof. Tier-1 single-use proof beats stylistic cleanup.
  Gates: clippy `--all-targets -D warnings` clean; 567/0/8 tests pass; asm-diff zero codegen delta.
- Pedantic-audit Tier 3 sweep batch 3 — 2026-05-19 — **10 audit findings closed (5 by code + 5 by historical-verification annotation) + 1 DEFERRED with rationale, across 8 commits**.
  (1) **#77** `fd3b5bd` — `validate_empty_body(payload, tag)` helper extracted in `dispatch.rs`; 6 inline `match payload { [] => OK, other => UnexpectedFrameBody }` arms migrated to call it (SimpleQuery+TAG_EMPTY_QUERY_RESPONSE, Parse+TAG_PARSE_COMPLETE, BindExecute DML+TAG_BIND_COMPLETE, BindExecute Select+TAG_BIND_COMPLETE, DescribeStatement+TAG_NO_DATA, DescribePortal+TAG_NO_DATA). Audit's "EmptyBody ZST" suggestion discarded as cosmetic — no caller takes a typed witness; the helper-fn shape gives identical bundle-compliance with a smaller surface. `#[inline(always)]` verified ASM-neutral on `feed_bytes` hot path (asm-diff: 0 codegen delta). +1.3% growth in `dispatch` cold-path error-construction is structural unification cost — all 6 zero-body arms now share one source of truth for wire-tag + observed-len classification. Net −16 LoC.
  (2) **#46** `4ca54ed` — `LossyText<'a>` typed boundary at wire-decode. `#[repr(transparent)]` newtype over `&'a [u8]` exposes `from_bytes_lossy` (zero-cost wrap), `raw_bytes()` (escape hatch for byte-fidelity callers), `display()` (zero-alloc `fmt::Display` via `LossyDisplay<'a>`), `to_bounded::<N>()` / `to_secret_bounded::<N>()` (storage commitment, lossy coercion applied here). 3 `parse_error_response` text-field sites (M/D/H per PG §55.7) migrated to funnel through it. The audit's `as_str(&self) -> &str` shape deliberately omitted — incompatible with deferred coercion in safe Rust. 11 new contract tests in `lossy_text_tests`. asm-diff `parse_error_response`: 0 codegen delta.
  (3) **#25** `731c7ca` — `OutActions<'w, 'r>` 2-lifetime form documented as load-bearing rather than collapsible. Audit hypothesised both lifetimes originate from `&mut PgProtocol`; investigation found `'w` binds the SEPARATE `&'w mut WriteBuf` parameter and `'r` binds the shared `&'r PgProtocol` reborrow for arena slices in `Reply<'r>`. Unification would compile via covariance but erase type-level documentation of the two distinct borrow sources, and erase push-path expressivity (`OutActions<'w, 'static>` returns in 16 push sites that emit no arena-borrowed Reply variants). Module-doc claim "OutActions does NOT borrow PgProtocol" also fixed — `'r` IS a shared reborrow of PgProtocol; only `&mut self` reentry is blocked. Doc-only.
  (4) **#26+#29** `24e0e10` — annotated 4 historical comment blocks at state_setter.rs:75/405/547 + dispatch.rs:48 with explicit `(HISTORICAL — closed by ...; Tier-3 audit #N, 2026-05-19, verified)` prefixes. All four sites' `tier-2-by-discipline within-crate` markers are PRE-CLOSURE: state_setter.rs:75 pre-Bundle F (now tier-1 via private `mod install_body_seal` supertrait), state_setter.rs:405 pre-cluster δ (now tier-1 via 4 concrete-type-token leaves), state_setter.rs:547 pre-cluster γ (now tier-1 via IdleState lifetime-bound typestate), dispatch.rs:48 pre-cluster α (now tier-1 via concrete `TDispatchToken`). #28 (prepared.rs:489 RowDecode sealed trait) verified ALREADY tier-1 in-mod via `mod sealed` (no visibility keyword → private to parent); audit reference at line 489 was mis-located inside the new_prepared_query comment block.
  (5) **#1+#27+#28** `8a4bc98` — **DEFERRED** with detailed rationale. Audit's Phase A (sealed-token parameter on `new_prepared_query`) investigated and found unsound under stable-Rust proc-macro hygiene + `#![forbid(unsafe_code)]`. Macro expansion runs in caller's hygiene context with no privileged access; a private-field seal constructable only inside `mod prepared` is unreachable from both user crate AND macro's emitted user-crate code. Phase B (compile-time `(file!(), line!(), column!())` const-generic fingerprint) requires unstable `feature(adt_const_params)`. SQL-injection class for users-who-follow-the-macro-contract remains tier-1-by-construction via the proc-macro's lex + cast-annotation validation (memo §7 P3-P12). The prepared.rs doc-comment block at `new_prepared_query` updated to record the full investigation (~50 lines) so future audits don't re-derive the impossibility proof.
  (6) **#17** `3c9ab67` — typed `MintSaturated` ZST + `mint_or_saturate(raw: u64) -> Result<NonZeroU64, MintSaturated>` helper in `reply_id.rs`. Single production call site `<DisconnectedPhase>::next_reply_id` (protocol.rs:1807) migrated from `NonZeroU64::new(raw).unwrap_or(MIN)` to `mint_or_saturate(raw).unwrap_or(MIN)`. `saturating_add(1)` floor is `1`, so `Err(MintSaturated)` arm is architecturally unreachable; the typed classifier captures the contract at type level. Test-helper sites (5 in `#[cfg(test)]` mods + 1 in test_fixtures.rs + 1 in reply_id.rs tests) intentionally retain assert-then-fallback for loud test failure. asm-diff `next_reply_id`: 0 codegen delta.
  (7) **#34** `d0ebe0d` — `ReadBuf::partial_remaining: u32` → `Option<NonZeroU32>` type-state encoding. Pre-audit, "in partial mode" was value-level `partial_remaining > 0` (tier-2 by-discipline). Post-audit, type-level `Option::is_some()` — writing `partial_remaining = 0` no longer typechecks; only explicit `None` (exit) or `NonZeroU32::new(_)` (entry/decrement). Subtract reaching 0 auto-transitions Some → None. Error type fields (`AlreadyInPartialMode::prev_remaining`, `PartialModeExitUndrained::remaining`) preserved as u32 for diagnostic display. asm-diff: 0 codegen delta on `subtract_partial_remaining`, `is_in_partial_mode`, `feed_bytes`.
  (8) **#53** `70704ab` — annotated error.rs:1306 and 1461 historical comment blocks documenting the pre-DEF-154 (I) `state_kind().unwrap_or_else(|| { debug_assert!(false, ...); INTERNAL_FALLBACK })` pattern. Verified already-closed: current `state_kind() -> StateErrorKind` is total (no Option), with `AlreadyClosed → Internal` as honest projection. Tier-4 debug-loud-release-silent → tier-2 typed-total already achieved.
  Gates across all 8 commits: clippy `--all-targets -D warnings` clean; **669/0/9** full workspace test suite (was 658 — +11 LossyText contract tests); asm-diff on every touched hot path showed 0 codegen delta.
- Pedantic-audit Tier 3 sweep batch 2 — 2026-05-19 — **8 audit findings closed across 5 commits + 3 deferred**.
  (1) **#32** `4a07d03` — **closed cross-instance reply-id collision bug**. `<ActivePhase>::next_reply_id` at protocol.rs:2957 maintained a SEPARATE function-local `static COUNTER` distinct from `PROCESS_REPLY_ID_COUNTER` (line 961) that every other mint site uses (Disconnected/Connecting/ActiveInner). Two PgProtocol instances on the same process could mint overlapping ids across phase boundaries. Fix: collapse `<ActivePhase>::next_reply_id` to a 1-line delegate `self.inner.next_reply_id::<K>()`; delete the now-dead `<ActivePhase>::install_errored_replyid_saturation` helper. Process-global uniqueness now tier-1 by-construction.
  (2) **#75+#76+#31+#49** `2a65810` — four comment-level cleanups. (#75) error.rs ServerErrorResponse Display: removed historical "tier-4 silent-degradation" reference, tightened comment block to document CURRENT tier-3 classified behavior (loud `[use PgProtocol::display_error ...]` advisory + `[severity absent]` rendering for None). (#76) protocol.rs `malformed_counter.saturating_add(1)`: annotated that the saturation policing happens at `MALFORMED_STORM_THRESHOLD = 10_000` (tier-3 classified) far below u32::MAX — the u32::MAX arm is architecturally dead (post-Errored connection short-circuits, counter no longer read). (#31) dispatch.rs:933/2855: rewrote both comment lead-ins to reflect current tier-3 classified state (parse_rfq_payload Err arm at 933, `?`-marker coercion at 2855); removed historical "pre-fix tier-4" framing. (#49) 24 `#[expect(...)]` sites verified — all have load-bearing structural reasons; clippy `-D warnings` gates dead expects via the self-cleaning property.
  (3) **#73** `29eb9aa` — added `scripts/bench-stable.sh` quiet-system gate. Pre-flight aborts `save`/`compare` modes if 1-minute load_avg > 4.0. Rationale: --measurement-time 30 sample windows are noise-amplifying on contended hardware (deferred.md §A prior incident: contended host produced symmetric +400%/-73% deltas on unrelated benches). Override via `BSQL_BENCH_FORCE=1` (loudly logged). `list` mode excluded from the gate.
  (4) **#48+#86** `5420f06` — (#48) BoundedStr<N> `was_lossy()` now trips on length-overflow truncation in addition to byte-coercion. Pre-audit, the slow path through `from_str_truncating` and the bytes-route through `from_bytes_lossy` appended the `OVERFLOW_MARKER` ("…") but left was_lossy_flag at 0 — programmatic truncation detection required parsing the tail for the marker. Now structurally surfaceable: consumer calls `command_tag.was_lossy()` and gets a true/false answer. The audit's option (c) "CommandTag newtype with truncation_marker bool" was rejected as needless API expansion — BoundedStr already carries the flag at the same 1-byte cost. (#86) session_params.rs:217 `try_from_bytes(src).unwrap_or_else(Self::empty)` annotated — the bound shield at line 215 (`src.len() <= MAX_ENCODING_NAME_LEN`) makes the Err arm architecturally dead; this is NOT the silent-empty production path the audit suspected.
  **DEFERRED with rationale** (3 items): **#16** (ArenaError::Empty + forge_for_test elimination — architect-cycle decision between Phase A vs B trade-offs; neither satisfies "no discipline bets"). **#92** (as_write_buf_mut escape-hatch retirement — 5 production call sites: SCRAM auth, Bind+Execute, wire builders, params writer; structural refactor required to route each site through the branded buffer-identity-proof API). **#93** (current_row_desc typed RowDescAvailability return — public API change with no load-bearing caller use case at the moment).
  Gates across all 5 commits: clippy `--all-targets -D warnings` clean; **658/0/9** full test suite; `asm-diff next_reply_id`: zero codegen change (LLVM was already inlining the chain).
- Pedantic-audit Tier 3 sweep batch 1 — 2026-05-19 — **6 audit findings closed across 4 commits**.
  (1) **Tier 3 #15+#14+#12+#13** `97bf2f0` — three small items in one commit. (#15) `NonEmptyRange::DEAD_FALLBACK` → `test_unit()` constructor: typed unit-length range (`start=0, len=NonZeroU16::MIN`), no `unwrap_or` route; 2 call sites lifted; 3 doc-comment refs updated. (#14) `ErrorPayload::dead_for_test()` deleted; single call site at `dispatch.rs::parse_and_resolve` inlines the empty-field fallback via explicit `match r.cloned() { Ok | Err(_) => ... }`; the audit's preferred `or_test_panic<T>(r) -> T { panic!(...) }` helper isn't buildable here because `clippy::panic` is `forbid`. (#12+#13) `dispatch.rs::parse_error_response` `payload.get(start..).unwrap_or(&[])` + sibling `tail.get(..n).unwrap_or(&[])` → `split_at_checked(_)` + match + `cold_path()` on the None arm; natural single-failure-mode form.
  (2) **Tier 3 #19+#30** `d77af5d` — comment-only annotations on the `usize::try_from(...).unwrap_or(_)` saturation cluster (#19): buf.rs:963/964 marked as tier-3-diagnostic (saturating display fields, audit "keep"); partial_assembly.rs:512 annotated as architecturally-dead (structural lift to branded narrowing helper deferred — would change `absorb()` signature to Result, rippling through dispatch.rs). + (#30) BREAKING API: `ProtocolError::ServerErrorResponse::severity: Severity` → `Option<Severity>`. Public API consumer can now distinguish "server didn't send S/V field" (`None`) from "server sent unrecognised severity string" (`Some(Severity::Unknown)`). Sites touched: error.rs variant + Display impl, dispatch.rs ParsedServerError + parser + test fixture mk_err helper (empty severity string maps to None), error_arena.rs DisplayError + 4 test fixtures wrapped in `Some(_)`, tests/startup_spec.rs integration test asserts `Some(s).as_str() == "FATAL"`. asm-diff `parse_error_response`: 48 insertions / 46 deletions — register reshuffle + 1 byte for the niche discriminant; no hot-path concern. Per project's "absolute safety без компромисов" — breaking API acceptable for diagnostic-fidelity tier-3-by-discipline closure.
  (3) **Tier 3 #33** `82d07ca` — new trybuild probe **P-E-10** pinning `DispatchContext`'s `pub(in crate::protocol)` visibility. External code that attempts `use bsql_pg_proto::protocol::DispatchContext` trips E0603 (private struct). Complements P-E-1..P-E-9 family of consume-self phase-typed protocol closure probes; specifically pins the within-crate closure of dispatch-context construction. If a future refactor accidentally widens visibility to `pub`, the probe stops failing → CI catches the regression at gate time.
  **#16 DEFERRED** — `ArenaError::Empty + forge_for_test` elimination requires architect-cycle analysis: Phase A (delete variant, accept 2^32 generation-wraparound diagnostic loss) trades drift coverage for cleaner API; Phase B (keep variant, delete forgery hook + drift-shield tests) loses the unreachable-arm test. Neither path satisfies "no discipline bets" without further structural work.
  Gates across all 4 commits: clippy `--all-targets -D warnings` clean; **229/0/0** lib tests (no count change for first 3 commits; +6 for #30 + #33); **658/0/9** full suite after all 4 commits.
- Pedantic-audit Tier 2 sweep batch 2 — 2026-05-19 — **4 audit-driven cleanups across 9 files (+~50/-~50 LoC)** spanning three independent commits.
  (1) **Tier 2 #2+#7** `65772a7` — 5 `let _name = expr` discard sites in protocol.rs replaced with explicit `match { Some(_) | None => {} }` / two-arm enum match. Sites: 4190 (ParamStatusRecordOutcome admit — exhaustive two-arm match documents intentional consumption; Phase 1d may surface MalformedPayload via wrapper-advisory action), 4284 (partial-mode absorb — `_absorbed_n` → `absorbed_n` since `debug_assert_eq!` keeps the binding "used" by macro expansion even in release), 4818/5022 (Connecting+Active `install_errored_replyid_saturation` lift+lower drains — match-discard on Option<NonZeroU64> Some/None), 5882 (PgProtocol<ActivePhase>::install_errored_stream_dropped_mid_stream — match-discard on `self.drain_via_leaf`). Companion: must_use messages on `_replyid_saturation_drain_leaf::drain` and `_stream_dropped_mid_stream_drain_leaf::drain` updated to teach the bundle-compliant match-discard form instead of pointing at the banned `_drained_*` binding name.
  (2) **Tier 2 Cluster C** `9e1ec8e` — 18 token-gated entry points renamed `_t:` → `_token:`. Sites: cancel.rs:195 (BackendKeyCell::empty), partial_assembly.rs:644/666/690/709 (4 PartialAssemblyCell methods), session_params_slot.rs:149/185/207 (3 SessionParamsCell methods), schema_slot.rs:169/181/191 (3 RowDescSlotCell methods + docstring example), state_setter.rs:423/439/456/473/488/503/534 (7 cluster δ drain leaf entry points — audit had grepped only 11 sites; the cluster pattern was present at 7 more state_setter sites missed in the audit). **Why not by-ref `_token: &Token`** (the audit's primary recommendation): experimentally verified that `unused_variables` lint fires identically on unused fn params regardless of by-ref vs by-value form under `#![deny(unused_variables)]`; the audit's claim that "with `&T` a non-underscore name is fine" was incorrect. The two by-ref workarounds (`let _ = token;` to silence; `core::mem::drop(token)`) each trip a different forbid-bundle lint. The cleanest path is to preserve the canonical-Rust underscore-prefix-on-unused-fn-param idiom while extending the name from `_t` to `_token` for readability.
  (3) **Tier 2 #9** `3e82058` — new `DropCounter::scoped<R>(value, probe, body: impl FnOnce() -> R) -> R` closure-form API in drop_witness.rs (cfg(test)-only); 24 of 32 `let _w = DropCounter::new(value, probe.clone())` test sites migrated to the scoped form. Files touched: buf.rs, write_buf.rs, error_arena.rs, sensitive.rs, md5.rs, scram/session.rs, scram/types.rs, ident.rs, password.rs, drop_witness.rs. 8 sites NOT migrated (explicit-bind required by test semantics): error_arena.rs:676/695 mid-life overwrite test (requires mutable rebind), drop_witness.rs:423–425 three-wrappers-alive probe-share test (requires concurrent liveness), drop_witness.rs:445 explicit `drop(_w)` probe-clones test (requires manual drop sequencing). Companion: module docstring + DropProbe tier-1 example rewritten to point at `DropCounter::scoped` as the idiomatic test pattern. **DEF-259 zeroize-on-drop discipline preserved unchanged**: `scoped` is a pure wrapper over `new + drop`; the CrateZeroizeSecret manifest, the per-secret Drop chains, and the integration-test exhaustiveness gate are byte-identical pre- and post-migration.
  Gates across all three commits: build clean; clippy `--all-targets -D warnings` clean; **229/0/0** lib tests passing (no count change); **657/0/9** across all `cargo test` binaries; `scripts/asm-diff.sh feed_bytes` / `install_errored` / `empty` / `park_at`: zero codegen change on every commit. Production-codegen impact: zero (Cluster C is param-rename only; #9 is cfg(test)-only; #2+#7 is lexical-only — LLVM compiles `let _name = expr` and `match expr { _ => () }` to identical assembly).
- Pedantic-audit Tier 2 sweep batch 1 — 2026-05-19 — **5 audit-driven cleanups + 1 latent-doctest fix across 7 files (+test_fixtures.rs new, ~140 LoC delta)**. Continuation of the audit-accepted Tier-2 plan. Closed: (#8) `partial_assembly.rs::tests::inner_*` — `let _ = inner.absorb(...)` → `assert_eq!(inner.absorb(...), N)` (turns silent discard into invariant pin); (#18) new `pub(crate) mod test_fixtures` houses `fixture_i16(n: usize) -> i16` and `fixture_i32(n: usize) -> i32` — `#[track_caller]` assert-bounded narrowing for hand-built wire-frame fixtures; replaces 4 silent `i16/i32::try_from(...).unwrap_or(0)` sites in decode.rs (3056/3142/3242/3247) + the `return-on-overflow` form at 3739; (#43) state.rs two duplicate `nz()` test helpers → shared `crate::test_fixtures::fixture_nz_u64 as nz` import (single source for the assert+unwrap_or(MIN) NonZeroU64 promotion idiom); (#69) guard.rs four `compile_fail` doctest `let _ = guard.push_command(...)` sites → idiomatic `match ... { Ok(_) | Err(_) => {} }` form (doctest API examples now teach the bundle-compliant pattern); (latent) decode.rs FromPgText doctest at line 1607 referenced nonexistent `bsql_pg_proto::StreamItem` type — Tier 1 #41 rewrite couldn't have compiled but `cargo test --lib` (used in 042ddc9's verification) doesn't run doctests, so the breakage went silent until this commit ran full `cargo test`; rewritten to take `row_bytes: &[u8]` directly (the actual `DataRowRef::parse` signature) returning `Result<Option<(Option<i32>, Option<&'a str>)>, DecodeError>` — no `String`/alloc dependency, no fictional API surface. Also incidentally: `p_e_8_struct_literal_field_private.stderr` trybuild expectation updated to mention the `extras` field added by DEF-279 Phase 3 (compile_fail probe now exercises 3 private-field rejection arms, stronger tier-1 closure). **Verification**: build clean; clippy `--all-targets -D warnings` clean; **229/0/0** lib tests passing (was 223 baseline; +6 = 4 self-tests on `fixture_i16`/`fixture_i32` + 2 on `fixture_nz_u64`); full `cargo test` (lib + integration + doc + compile-fail + trybuild) aggregates 657/0/9 across all binaries — no remaining latent failures. Production-codegen impact: zero (every change lives behind `#[cfg(test)]` or in doctest blocks).
- Pedantic-audit Tier 1 sweep — 2026-05-18 — `042ddc9` — **11 audit-driven cleanups across 10 files (+101/-62 LoC)**. Audit-driven follow-up to the pedantic global audit (110 findings; audit.txt + audit_accepted.txt local-only, gitignored). Tier 1 = "quick wins ≤1-2 days". Closed: (#5) `let _ = self;` delete in push_startup; (#4) prepared! macro for PhantomData fields emits `true` directly; (#10) `let _ = PhantomData::<W>;` → `const _: PhantomData<W> = PhantomData;` (3 sites: push_command.rs:905/909 + row_stream.rs:1643); (#6) `Err(_architecturally_dead)` → `Err(_)` + cold_path comment; (#61) `#[must_use]` message on ErrorRef switched from banned `let _ = ref;` to `match r { _ => () }` / `core::mem::drop(r)` idiom; (#62) `architect.txt Part V` reference → `CREDO` in reply_id docstring; (#42) params.rs macros `($_t:ident)` → `($tok:ident)` (drops underscore-prefix on macro param names); (#20) `usize::BITS >= 32` const-assert added at crate root (rejects 16-bit targets at build time; documents the dead `unwrap_or(0)` in partial_assembly.rs:464 as architecturally-unreachable post-assert); (#22) `let _old = mem::replace(...)` in error_arena test → `core::mem::drop(core::mem::replace(...))`; (#23) test `unwrap_or_else(|_| debug_assert!(false, ...))` glass pattern → explicit `assert!(push_result.is_ok())`; (#41) decode.rs FromPgText doctest example rewritten (drops `_id`/`_name` underscore-prefixed bindings; returns the decoded tuple via structural `match (id, name)`); (#91) 3 runtime `assert_eq!` on `const` ParamsWriter trait values in params.rs → `const _: () = assert!(...)` build-time pins. **Deferred from Tier 1** with audit_accepted.txt rationale: #57 i32_from_hex helper (premature single-site abstraction), #88 BoundedStr<N> N>=1 centralised const-assert (need to verify whether BoundedLen<N> already enforces it), #89/#90 test type-aliases R0/R1/R2 (pure cosmetic; defer to UFCS sweep). **Tier 2-5** (~70 remaining audit work-items + 7 REJECT root-cause structural infra-fixes) tracked in audit_accepted.txt. **Verification**: build clean; clippy --all-targets -D warnings clean; **534/0/0** integration + **223/0/0** lib tests passing; existing StreamItem doctest failure (pre-existing on `main`) unchanged.
- DEF-279 Phase 3 — 2026-05-18 — `8931f28` — **outer Extras field-narrow + closure-parameterized dispatch (architect Interpretation B + Option Y)**. Closes the Phase 1c-deferred row_desc_slot narrow from Commits 2/8b. **Mechanism**: (a) new associated type `SealedPhase::Extras` mapped per phase — `DisconnectedPhase`/`ConnectingPhase`/`ClosedPhase` → `()` (ZST), `ActivePhase` → `crate::schema_slot::RowDescSlotCell`; (b) `PgProtocol<P>` gains `extras: <P as SealedPhase>::Extras` field (drops `#[repr(transparent)]` — Active monomorphisation has two non-ZST fields, incompatible with transparent); (c) `ConnectingInner` + `ActiveInner` each drop their `row_desc_slot` field (Inner narrow); (d) `ConnectingDispatchContext` drops its `row_desc_slot` field (per-phase dispatch context narrow); (e) `feed_bytes_dispatch` parameterized by a `materialise_fn: M` closure — Connecting wrapper passes a closure returning `OutActions<'w, 'static>` (no schema reachable; transient stack-local slot threads through dispatch.rs::dispatch for arm-internal &mut access, but materialise hardcodes `None::<&'static RowDesc>` → 'static is widening-safe via covariance), Active wrapper passes closure returning `OutActions<'w, 'r>` sourced from `&mut self.extras`; (f) Active's into_active mints fresh `RowDescSlotCell` via new `_proto_init_leaf::fresh_active_row_desc_slot()` (P6 closure intact, leaf-private mint); (g) `ActiveInner` methods (feed_bytes_impl / feed_bytes_bounded / advance_one_frame / clear_session_residue_for_class) take `row_desc_slot: &mut RowDescSlotCell` as param, sourced by outer's disjoint-field borrow `&mut self.extras`; (h) `RowDescSlotCell` visibility raised to `pub` (associated-type-of-pub-trait requirement; field-private tuple-struct on inner — token-gated constructor stays pub(crate) so external code cannot mint cells). **Layout impact**: `PgProtocol<ConnectingPhase>` **504 → 368 B (-136 B)** — Connecting handshake gets leaner; `PgProtocol<ActivePhase>` 536 B **(neutral)** — cell moved from Inner to outer Extras; `PgProtocol<DisconnectedPhase>` 0 B (unchanged); `PgProtocol<ClosedPhase>` 16 B (unchanged). **Tier-1 closure**: `PgProtocol<ConnectingPhase>` has NO `row_desc_slot` field at ANY level (outer, inner, dispatch context) — verified by `compile_fail` doctest probe on `SealedPhase::Extras` (E0609 on `&mut p.row_desc_slot`). The architect's Option Y prevailed over Z' (lifetime hole on transient slot escape; `OutActions` covariant in 'r → can't widen local 'tmp to outer 'r) and Option A (slot on outer for all phases; CREDO §0 "manufactured tier-1" — field carried by phases that semantically reject it). **Architect cycle**: Interpretation B confirmed Extras associated type structurally cleaner than literal "outer for all phases"; Option Y confirmed closure parameterization over dispatch-loop split (X) and Option<&mut> tier-2 (Y). **Verification**: build clean; clippy `--all-targets -D warnings` clean; **534/0/0** integration + **223/0/0** lib tests passing (no regression); compile_fail doctest probe passes; asm-diff `feed_bytes_dispatch` vs HEAD `2ba47ee`: 1380 → 236 lines / 25622 → 4926 bytes — closure parameterization let LLVM share code between with-schema and no-schema paths (code-size reduction is structural; per-call lift+lower still ~5-10 ns per architect estimate). Layout pins updated in `lib.rs`. 3 files, +361 / −138 LoC (protocol.rs +437 / −138, schema_slot.rs +17 / −0, lib.rs +45 / −0).
- DEF-279 Phase 2 Bundle bench-stable measurement — 2026-05-18 — **`pre-def279-phase2-flip` baseline vs post-Commit-13**. `bench-stable.sh compare` flagged 17 regressions, BUT the result is **statistically invalid due to system-contention noise**: the symmetric +400-500% / -73% delta pattern on **column-decode SWAR functions that have ZERO architectural connection to Commit 12's Active flip** (`parse_pg_bool_swar_false +479.32%`, `validate_utf8_swar_multibyte_miss +357.95%`, mirrored -71-73% improvements on `iter_rows_via_*`) cannot reflect real regression — those functions don't touch state, dispatch, or any per-phase Inner type. Diagnosis: `bench-stable.sh`'s `taskpolicy -c utility` QoS demotion + 30s/10s measurement windows are noise-amplifying on contended hardware (documented caveat in `bench_ping_round_trip` docstring). The architect-predicted ~5-10 ns/call lift+lower overhead for `<ActivePhase>::feed_bytes` stands as the structural estimate; valid bench measurement requires a quiet system + cargo bench at normal QoS (deferred to a separate measurement run). **No production regression suspected** — the Phase 1c Connecting bench (Commit 8b) showed the same noise pattern on Active-only benches that Commit 8b doesn't modify; the Commit 12 measurement reproduces the noise floor.
- DEF-279 Phase 2 Bundle Commit 13 — 2026-05-18 — `93e7ad6` — **Delete PgProtocolInner + orphans (pure cleanup, -789 LoC)**. With Commit 12 every SealedPhase points at a per-phase Inner; PgProtocolInner became fully vestigial. **Deleted from protocol.rs** (-826 LoC): `pub struct PgProtocolInner` + `impl PgProtocolInner { ... }` (7 methods with no live callers) + `_proto_init_leaf::fresh_inner` const fn (last caller migrated to `fresh_active_inner` in Commit 12) + `advance_one_frame_dispatch` free function (replaced by per-phase variants) + `impl core::fmt::Debug for PgProtocolInner`. **Updated lib.rs** (+9/-25 LoC): deleted `size_of::<PgProtocolInner>() == 536` pin; updated surrounding docstring to enumerate the 4 per-phase Inner sizes (0/504/536/16); ActivePhase pin docstring re-references ActiveInner as the layout source. **Tier-1 closure now structurally complete across every phase** — state-variant-in-wrong-phase write impossible at the type level; only bridges between per-phase enums and shared ProtoState are the exhaustive `From`/`TryFrom` impls (adding a ProtoState variant fails build at projection sites). **Verification**: build clean; clippy `-D warnings` clean; 534/0/0 tests passing; `asm-diff feed_bytes` vs HEAD `8fb2d33`: 1915 lines / 36526 bytes BEFORE = AFTER → **byte-identical production codegen** (LLVM already DCE'd the deleted symbols; deletion is pure source-level cleanup). 2 files, +37/-834 LoC.
- DEF-279 Phase 2 Bundle Commit 12 — 2026-05-18 — `0597dae` — **SealedPhase flip for ActivePhase + into_active rebuild + body migrations**. `impl SealedPhase for ActivePhase { type Inner = ActiveInner }` (was PgProtocolInner). With this commit, **every phase has a per-phase Inner** — DisconnectedPhase→DisconnectedInner (ZST), ConnectingPhase→ConnectingInner, ActivePhase→ActiveInner, ClosedPhase→ClosedInner. PgProtocolInner is fully vestigial (struct + impl preserved transiently via `#[allow(dead_code)]`; deletion in Commit 13). **Migrations**: (1) `_proto_init_leaf::fresh_active_inner` constructor; (2) `<ConnectingPhase>::into_active` HandshakeReady success arm rebuilds `ActiveInner` (was PgProtocolInner) via destructure+rebuild + `state: ActiveState::Idle`; (3) `<ActivePhase>::into_closed_if_errored` matches `ActiveState::Errored(state_kind)`; (4) `<ActivePhase>::push_command_internal` lift+lower for IdleState setter machinery via `mem::replace(&mut self.inner.state, ActiveState::Idle)`; (5) `<ActivePhase>::state()` return type `&ProtoState → &ActiveState`; (6) `<ActivePhase>::classify_for_iter_rows` matches `ActiveState::*`; (7) New `drain_via_leaf<F>` helper on `impl PgProtocol<ActivePhase>` refactors 5 `install_errored_*` methods (RowStream cold paths + Drop) — same cluster δ token-gated drain leaves preserved, single lift+lower per drain. **Test sweep** (13 test files): `proto.state()` method name unchanged; return type change drives variant matches `ProtoState::* → ActiveState::*`; `use ActiveState` added; helpers `expect_awaiting_ping_reply` / `state_is_valid` typed for ActiveState; protocol-internal test mod `residue_policy_per_class_tests` rebound `SealedPhase<Inner = PgProtocolInner> → SealedPhase<Inner = ActiveInner>` + Connecting-class test rewritten to use `StatePushClass::Connecting` constant (Connecting state unreachable from ActiveState). **Verification**: build clean; clippy `-D warnings` clean; **534/0/0** tests passing (no regression); `asm-diff feed_bytes` vs HEAD `3de97ef`: 1703→1915 lines / 32354→36526 bytes — expected new emission for now-live Active lift+lower wrappers. Per-call overhead ~5-10 ns architect-estimate (parallel to Phase 1c Commit 8b). 14 files, +353/-182 LoC.
- DEF-279 Phase 2 Bundle Commit 11 — 2026-05-18 — `3de97ef` — **ActiveInner methods + manual Debug for ActiveInner**. 7 methods + manual Debug. Same shape as Phase 1c Commit 7 for Connecting. `install_errored_replyid_saturation` (lift+lower drain), `next_reply_id` (shared static counter), `feed_bytes_impl::<BOUNDED>` (delegate to `feed_bytes_dispatch_active`), `feed_bytes_bounded` (1-line wrapper), `feed_inbound` (append bytes, Errored arm matches `ActiveState::Errored`), `advance_one_frame` (delegate), `clear_session_residue_for_class` (state-agnostic helper). **Manual `Debug` for ActiveInner** — same `finish_non_exhaustive()` 3-field shape as PgProtocolInner. `#[allow(dead_code)]` until Commit 12 SealedPhase flip. **Verification**: 534/0/0 tests; clippy clean; production codegen unchanged (DCE). 1 file, +206 LoC.
- DEF-279 Phase 2 Bundle Commit 10 — 2026-05-18 — `7ab62ee` — **`feed_bytes_dispatch_active` + `advance_one_frame_dispatch_active` (scaffolding)**. Three `#[allow(dead_code)]` items: (a) `ActiveDispatchContext<'state, 'r>` — per-phase mirror of `DispatchContext` with `state: &'state mut ActiveState`; (b) `feed_bytes_dispatch_active::<BOUNDED>` lift+lower wrapper — same architectural pattern as Phase 1c Commit 6's `feed_bytes_dispatch_connecting`, NO transition-signal translation needed (Active doesn't have a HandshakeReady analogue; Active's lower is direct projection); (c) `advance_one_frame_dispatch_active` — Active-specific fast paths (StreamingRows→FeedEvent::StreamingRows, Errored→Close, Idle+empty→Idle) + delegate to `feed_bytes_dispatch_active::<true>`. **Same architect-endorsed Alt I** (single source of truth for dispatch loop preserved; lift+lower amortises ~5-10 ns per call). **Verification**: 534/0/0 tests; clippy clean; production codegen unchanged (DCE). 1 file, +176 LoC.
- DEF-279 Phase 2 Bundle Commit 9 — 2026-05-18 — `28ff685` — **ActiveInner struct + ActiveState classifier methods + ActiveState Debug**. (1) `ActiveInner` struct (protocol.rs) — 8 fields mirror `PgProtocolInner` exactly except `state: ActiveState` (~80 B, same as ProtoState). Field set unchanged from PgProtocolInner because every Active variant needs every cell. (2) `ActiveState::take_inflight_reply_raw_id` — exhaustive over every variant; Idle/DrainRfqAfterError/Errored return None. (3) `ActiveState::push_class` — Idle/PingAwaiting/Errored/BusyQuery; Connecting-only `StatePushClass::Connecting` unreachable by storage absence. (4) `ActiveState` manual `Debug` — mirror of `ProtoState`'s post-handshake field rendering; no SCRAM/MD5/Cleartext secret material in Active variants. **`#[allow(dead_code)]`** until Commit 10. **Verification**: 534/0/0 tests; clippy clean; production codegen unchanged (DCE). 2 files, +222/-3 LoC (protocol.rs +61, state.rs +164/-3).
- DEF-279 Phase 1c Bundle bench-stable measurement — 2026-05-18 — **`pre-def279-phase1c-flip` baseline vs post-Commit-8b**. `bench-stable.sh compare`: **0 regressions** flagged by criterion's confidence-bound classifier (PASS). 10 unchanged / 2 improvements. **Top mean-change movers** within criterion's noise envelope (alignment lottery + bench-stable QoS demotion noise per `bench_ping_round_trip` docstring caveat): +9.78% iter_batched_floor_probe/ref_proto_plus_wb, +9.50% prepared_query_push/execute_prepared_dml, +7.88% push_bind_execute_one_int_param/bind_execute, +6.42% ping_round_trip/push_then_feed, +5.18% iter_batched_floor_probe/drop_proto_plus_wb. **All five top regressions are on Active-phase hot paths that Commit 8b does NOT modify** — Active still uses PgProtocolInner; the lift+lower wrapper is wired only into `<ConnectingPhase>`. Codegen perturbation (alignment shifts from new ConnectingInner symbols emitting nearby in the binary) is the structural cause. Symmetric -73% improvements on iter_rows variants + push_command/ping_amortised mirror the noise (baseline measurement was system-contended). No directly Connecting-touching hot path is benched (hot_paths.rs targets post-handshake Active flow); the architect-predicted ~5-10 ns/call lift+lower overhead for `<ConnectingPhase>::feed_bytes` stands as the structural estimate. **Verdict**: no statistically significant regression on the measured Active hot paths; Connecting-path overhead bounded by structural reasoning. Bundle 8b cleared for production.
- DEF-279 Phase 1c Bundle Commit 8b — 2026-05-18 — `5d59b64` — **SealedPhase flip + body migrations**. `impl SealedPhase for ConnectingPhase { type Inner = ConnectingInner }` (was PgProtocolInner). **The flip materialises tier-1 closure on state-variant-in-wrong-phase by storage absence** — `ConnectingInner.state: ConnectingState` has no Active variants. **Migrations**: (1) `<DisconnectedPhase>::push_startup` constructs ConnectingInner via `_proto_init_leaf::fresh_connecting_inner` (Commit 8a), uses lift+lower (`proto_state: ProtoState::Idle` local for IdleState setter machinery, then `ConnectingState::try_from(proto_state)` lower-assigns `new_inner.state`); (2) `<ConnectingPhase>::into_active` observes `ConnectingState::HandshakeReady` for success arm (field-destructures ConnectingInner + rebuilds PgProtocolInner for ActivePhase, which still uses PgProtocolInner — Active migration deferred), Errored arm matches `ConnectingState::Errored(k)` → ClosedInner; (3) `<ConnectingPhase>::state()` return type `&ProtoState → &ConnectingState` — **architect-endorsed Alt D** (per-phase tailored API): plus new typed predicates `is_handshake_ready()` and `is_errored()` (stronger invariants than open pattern matching); (4) 10 `<ConnectingPhase>::*` method bodies' `self.inner.X` now resolve to `ConnectingInner::X` (Commit 7); (5) `PgProtocolInner::next_reply_id` DELETED (sole non-Active caller migrated to `ConnectingInner::next_reply_id`); (6) `_proto_init_leaf::fresh_inner` gated to `#[cfg(test)]` (only test-helper callers remain). **Public re-exports** (lib.rs): `ConnectingState` / `ActiveState` / `ErroredState`. **Size pin updated**: `size_of::<PgProtocol<ConnectingPhase>>() == 504` (was 536) — 32 B saving from `ProtoState 80 B → ConnectingState 48 B`. Field-narrow drops of `row_desc_slot` (140 B) + `backend_key` (12 B) DEFERRED — the lift+lower wrapper needs both slots borrowable during shared dispatch match-arm evaluation. **Test sweep** (5 files: startup_spec, scram_fuzz_spec, tier_seams_spec, def198_guard_closure_spec, audit_coverage_spec): `proto.state()` continues to compile (method name unchanged); return type change drives variant matches — `ProtoState::Connecting*` → `ConnectingState::*` (unprefixed, via sed), `ProtoState::Idle` (on Connecting) → `ConnectingState::HandshakeReady`, `ProtoState::Errored(_)` (on Connecting) → `ConnectingState::Errored(_)`; `use ConnectingState` added to each test file. **Verification**: build clean; clippy `-D warnings` clean; **534/0/0** tests passing across 25 crates (no regression). Production codegen DID change (expected — Connecting path now uses lift+lower wrapper). Per-call overhead estimate ~5-10 ns from `mem::replace` + `Into` + `TryFrom` per feed_bytes call (amortised over up to MAX_DISPATCHES=8 frames). Bench-stable measurement deferred to follow-up. 7 files, +281/-156 LoC.
- DEF-279 Phase 1c Bundle Commit 8a — 2026-05-18 — `e7cf19f` — **`_proto_init_leaf::fresh_connecting_inner` constructor**. Mirror of `fresh_inner` for `ConnectingInner` — same `ProtoInitToken::mint()`-gated cell construction (DEF-272 P6 closure preserved). Initial state `ConnectingState::Errored(Framing)` is a transient placeholder (push_startup's setter overwrites it immediately via lift+lower). `#[allow(dead_code)]` until Commit 8b. **Verification**: build clean; clippy clean; 58+12+3+3 tests passing; production codegen unchanged (DCE). 1 file, +44 LoC.
- DEF-279 Phase 1c Bundle Commit 7 — 2026-05-18 — `1b44c2c` — **ConnectingInner methods + Debug + ConnectingState Debug (scaffolding)**. Adds `#[allow(dead_code)]` per-phase Inner API surface for `<ConnectingPhase>` substrate (dead until Commit 8 SealedPhase flip). **5 methods on `ConnectingInner`**: (1) `install_errored_replyid_saturation` — saturation classifier routes through same cluster δ token-gated `_replyid_saturation_drain_leaf::drain` via lift+lower (tier-1 closure on FeedStateSetter construction preserved — same leaf submodule, same token); (2) `next_reply_id<K>` — shared `super::PROCESS_REPLY_ID_COUNTER` atomic counter + cold saturation route; (3) `feed_bytes_impl::<BOUNDED>` — thin delegate building `ConnectingDispatchContext` via disjoint-field-borrow from `&mut self`, forwards to `feed_bytes_dispatch_connecting`; (4) `feed_bytes_bounded` — 1-line wrapper; (5) `feed_inbound` — append bytes (Errored arm reads `ConnectingState::Errored(k) => *k`, partial-mode routing identical). + thin delegate `advance_one_frame` → `advance_one_frame_dispatch_connecting`. **Manual `Debug` for `ConnectingInner`** — same field set (state / read_buf / session_params) + same `finish_non_exhaustive()` as `PgProtocolInner`'s Debug. **Manual `Debug` for `ConnectingState`** (state.rs) — **Sensitive-redaction parity** with `ProtoState`'s manual Debug: variants carrying SCRAM `ScramSession`/`SecretDigest`, MD5 `Md5HandshakeState`, cleartext `Sensitive<Password>`, or post-auth `Sensitive<i32>` secret keys use `finish_non_exhaustive()` to elide secrets; non-sensitive variants print all fields via `finish()`/`write!`. `HandshakeReady` is a unit variant (no payload — per Bundle D' tier preservation). ActiveState + ErroredState manual Debug deferred (neither yet on any Debug-printing path). **Verification**: build clean; clippy `-D warnings` clean; 58+12+3+3 tests passing; `asm-diff feed_bytes` vs HEAD `e14ce3a`: 783 lines / 14797 bytes BEFORE = AFTER → byte-identical production codegen (DCE drops the dead methods); grep `ConnectingInner::feed_bytes_impl\|ConnectingInner::feed_inbound\|ConnectingInner::advance_one_frame\|ConnectingInner::install_errored` in release `.s`: 0 matches per emit. 2 files, +287/-4 LoC (protocol.rs +220, state.rs +71/-4).
