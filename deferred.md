...
| DEF-094 | Registered (rejected) | **`Action<'buf>` lifetime-bound zero-copy SendBytes — rejected.** Phase 1c batch 1 attempted the DEF-089-follow-up refactor `Action<'buf>::SendBytes(&'buf [u8])` (Sync 5-byte static-ref + StartupMessage zero-copy against `self.write_buf`). Fundamental borrow-checker conflict: `OutActions<'buf>` captures the lifetime of `&mut self.write_buf` across `feed_bytes`'s multi-frame drain loop. Each loop iteration calls `dispatch(..., write_buf)` which re-borrows `write_buf`; the previous iteration's emitted `Action<'buf>::SendBytes(&'buf [u8])` is still sitting in the `out: OutActions<'buf>` accumulator holding the borrow. Rust tracks lifetimes by type, not by runtime state — "break after emit" does not help because the `out` vector's type already carries `'buf`. Resolution would require restructuring `OutActions` to carry ranges into an internal byte buffer (wrapper copies on `as_bytes()`), which defeats the zero-copy goal (the memcpy just moves from owned-in-Action to owned-in-OutActions). SendBuf's owned memcpy is therefore the **terminal shape** of the send-path, not a temporary step. Tier-3 seam was already closed at tier-1 structural by DEF-089 v1 (single-shape newtype — no enum arms to swap). Perf impact of keeping memcpy: 5 bytes per Ping (rare) + ~300 bytes per startup handshake (once per connection lifetime) — negligible. Do not re-open without a fundamentally different architecture for multi-frame Action accumulation. |
