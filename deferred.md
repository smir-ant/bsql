...
| **DEF-285** | **Phase 1c session perf regression investigation** | OPEN 2026-05-23. `scripts/bench-stable.sh compare post-def248-subb` after the 2026-05-23 session work (20 commits: DEF-282 boxing in `7bb3346` + DEF-220 LISTEN/NOTIFY + DEF-219 COPY + DEF-226 multi-statement + DEF-225 PortalSuspended + DEF-243 read_buf eager-clear + DEF-165 + audit #100 + DEF-248 Sub-B follow-up) shows: **`push_command/ping` 13 → 78 ns (+492%, p=0.00)**; **`ping_round_trip/push_then_feed` 82 → 224 ns (+173%, p=0.00)**; `parse_header/rfq_header` 2.46 ns (unchanged — pure function). **Likely contributors**: (a) DispatchContext gained 2 new fields (notifications_arena + copy_chunks_arena slots) → +16 B per dispatch call frame, +2 ref-loads per advance_one_frame; (b) dispatch() fn signature gained `copy_chunks_arena_slot` param; (c) DEF-243 eager-clear adds one `matches!()` per feed_bytes; (d) DEF-220 pre-dispatch filter for TAG_NOTIFICATION_RESPONSE adds one tag-compare per frame; (e) ActiveInner gained 16 B → PgProtocol 504 → 520 B (alignment-affected layout). Bench fixture uses `iter_batched_ref` with fresh proto per iter → full handshake = 5+ advance_one_frame calls = overhead ×5. **Baseline saved as `post-session-2026-05-23` for delta tracking.** **Investigation plan** (next focused session): (1) bisect using `git worktree` to identify the largest-contributor commit (DEF-282 alone vs cumulative session); (2) `asm-diff.sh` on `feed_bytes_dispatch` against pre-DEF-220 HEAD to identify codegen pessimization; (3) explore «universal» alternatives — possibly Box the DispatchContext, mark dispatch() `#[inline(always)]`, lazy-init pattern for arena slots via thread-local or static cache. **Memory-note discipline applies**: per [[feedback_regression_on_safety_change]], tier-elevation work that introduces perf regression triggers INVESTIGATION (alternative architectures), not rollback. The DEF-220 / DEF-219 functional surfaces are SHIPPED; the perf-recovery work is its own DEF. |
