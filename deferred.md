...
- DEF-248 Sub-A — 2026-05-12 — `3a55d89` — closure-scoped `iter_rows` API + `ColEvent` (Got/Null/EndRow/Chunk/ChunkEnd/NeedMore/EndQuery) + ReadBuf partial-frame substrate + Drop install-Errored for D-tag DataRow streaming. **`mem::forget(RowStream)` structurally closed** (E0507 on `*stream` deref). Multi-MB JSONB/TEXT/BYTEA cells now stream at ~2.7 GiB/s on the partial-frame path. v1.0 production blocker resolved (most real workloads). Sub-B (non-D tag per-tag arena sinks) remains for universal coverage promise.
...
| 5 | DEF-248 Sub-B | **Universal-coverage streaming for non-D tags** — Sub-A SHIPPED 2026-05-12 (`3a55d89`); D-tag DataRow > READ_BUF_CAP now streams correctly via `ColEvent::Chunk`/`ChunkEnd`. **Sub-B remaining scope** (~5500-7000 LoC, closes universal coverage per v2 memo §4 / principal directive «если возможен, не опускаем»): per-tag streaming sinks for T (`SchemaArena` incremental column-def parser) + E/N (`ErrorArena` incremental TLV parser) + S (`SessionParamsSlot` incremental CSTR-pair parser) + R (per-sub-code dispatch into existing SCRAM state) + A (new `NotificationArena` lazy `Option<Box<>>` on PgProtocol) + C (new `CmdTagArena` lazy `Option<Box<>>`) + v (one-shot box for oversized NegotiateProtocolVersion); `select_sink(tag, state)` exhaustive `match` on sealed `InboundTag` in dispatch.rs (tier-1 by construction — new PG tag = compile error); lazy-box `session_params_slot` (`Option<Box<SessionParamsSlot>>`) as footprint compensation (-248 B vs +56 B new = -192 B net, preserves 520 B pin). After Sub-B: every PG wire-legal frame size handled for every tag; universal coverage per principal directive. **Practical note**: all current PG drivers cap non-D frame bodies well below 4 KB (RowDescription typically < 1 KB; ErrorResponse usually < 1 KB; CommandComplete < 64 B); Sub-B closes the universal-coverage promise rather than a concrete v1.0 blocker. Reuses Sub-A's `partial_remaining: u32` substrate + leaf-token discipline. | Universal: every wire-legal frame size handled; closes universal-coverage residue | NON-BREAKING (additive sinks; existing callers unaffected on small frames) |
