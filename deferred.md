...
**Shipped 2026-04-22 → 2026-05-25** — see `#### Shipped from this queue (chronological)` table below and `## §D. Closed Index` for the major-DEF one-liners; verbose per-batch narrative lives in git log (`git log --oneline` since `2026-04-22` covers it). Active sweep cycles included pedantic-audit Tier 3 + Tier 4 cluster sweeps (Cluster A/B/D + historical-prefix strip across all `*.rs` / `*.toml` / `*.yml`), DEF-279 per-phase Inner refactor Phase 1a/1b/1c/1d/1d.2, DEF-270 Phase 1/2 clusters, DEF-271/272 tier-1 closures, DEF-160 Z2 zero-copy borrowed-SQL push, DEF-248 Sub-A/B universal-coverage streaming, DEF-258 compile-time format-code matrix, DEF-265 footprint reduction, DEF-266 SWAR opt-in helpers, DEF-269 v2 type-level CommandKind dispatch, **DEF-286 full footprint cascade (Action 80→24 B, OutActions 728→224 B, Reply 80→12 B, 8 phases + follow-up)**.
...
| 2026-05-26 | DEF-214 Phase 1e | **SSL state machine** — `SslNegotiatingPhase` ZST typestate. `push_ssl_request` + `classify_ssl_response` + `SslClassified` enum. 0 B runtime, 3 tier-1 invariants. 6 tests. | `e4570d5` |
| 2026-05-26 | DEF-224 | **NoticeResponse delivery** — NoticesArena (multi-slot, gen-tagged), Action::Notice + FeedEvent::Notice, parse_and_alloc_notice, PgProtocol::get_notice accessor. 6 tests (3 arena + 3 integration). | `9d5e678` + `e79877d` + `fb0bb47` |
| 2026-05-26 | DEF-218 partial | **RowDesc → Box<[u32]>** exact-size, no column cap. FormatCodeSet DELETED (−774 LoC). MAX_ROW_COLUMNS 32→1600. **ParamOids → Box<[u32]>** exact-size, no param cap. collect_tuple stack 27 KB→272 B. PgProtocol<Active> 528→296 B (−44%). 9 new tests. 2 bugs found+fixed (stack overflow, constant mismatch). | `3af6b60` + `b8b1ebe` + `a20dadb` + `fabbe66` + `8fcd7e9` |
| 2026-05-26 | Session perf | push_command/ping −27% (64→46 ns), interleaved_streaming −10%, iter_rows −3%. parse_rfq_payload #[cold] fix, 10× cold_path(), ReadBuf helpers, simdutf8 binary decode, col_next mid-row classify skip, WriteBuf #[inline], INLINE_BUF_CAP 256→128, #[repr(C)]. 0 regressions bench-verified. | `efaf8a1` + `afb22b4` |
...
| 2026-05-24 | DEF-286 | **Full footprint cascade — Action 80→24 B (−70%), OutActions 728→224 B (−69%), Reply 80→12 B (−85%)**. 8 sequential phases shipped in one session: **Φ1** ParamOids slot-pattern (`5314566`); **Φ2'** PushFailure-only Box (`1fa9f04`); **Φ-B''** ScramFailureClass + ErrorArena reuse — ProtocolError 72→24 B (`88a3b42`); **Φ3** CommandTag typed enum + slot (`cf2adda`); **Φ-D** ICC arena externalisation — Action 80→48→40 B (`a6a65b3`); **Φ-E** tx_status slot — Reply 32→24 B (`fdbb419`); **Φ-Final** combined Φ-I.b (FailReply.cause slot) + Φ-F* (Reply field externalisation to accessors) — Action 40→24, Reply 24→12, OutActions 368→224, FeedEvent 80→24 (`3d1c53f` merge: `3cd4642` + `374a4c2` + `f4ebc08` perf-recovery). **Unified slot pattern** across 6 ephemeral per-cycle fields (row_desc, param_oids, command_tag, tx_status, command_tags_arena, fail_cause) — all token-gated tier-1 by-construction. Reply<'r> lifetime DROPPED (Reply: Copy lifetime-free). **Perf**: push_command/ping perf-neutral or improved (−6.2% architect measurement) after ARM64 arg-count regression fix (Φ-Final Fix-A: dead `fail_cause clear` dropped from Idle arm — saved 3-4 ns/push from dispatch fn arg spill). 7-bench coverage: 3 improvements, 4 parity, 0 regressions. Zero new heap allocs on hot path (Box<PE> cold-only). **Post-cascade RowDesc Box-in-slot MEASURED-REJECTED**: ActiveExtras 168→40 B footprint win BUT iter_rows benches +1-2% regression (Box deref on per-row `current_row_desc()` hot path). Lesson preserved: Box-in-slot viable for cold-read slots only. | `3d1c53f` (merge) |
...
| DEF-243 | **Eager `read_buf.clear()` at install_errored transition** — CLOSED 2026-05-23 via post-loop eager-clear at `feed_bytes_dispatch` (shared body for both Connecting + Active phases). The fix is ONE site, not the 108-callsite plumbing the deferred entry's «mid-size refactor» framing suggested. Implementation: after the dispatch loop's existing «if not Errored, advance read_buf» block, add `if matches!(state, ProtoState::Errored(_)) { read_buf.clear(); }`. Cost: one `matches!()` branch per `feed_bytes` (sub-ns on the happy non-Errored path; LLVM elides via `core::hint::cold_path`) + one `clear()` call on the cold Errored exit path (O(populated_len) memset over the ≤4 KiB read_buf cap, zeroize-on-clear per DEF-204 scrubs secret-correlated bytes). **Window closed**: pre-DEF-243 had read_buf bytes sit ~one feed_bytes interval until the next `AlreadyErrored` arm cleared, or until Drop. Post-DEF-243 the clear fires AT the transition site in the same call — secret-correlated bytes (SCRAM server-frame fragments, ErrorResponse detail strings, partial frame bodies) are scrubbed before the wrapper observes the Errored state. The pre-existing `AlreadyErrored`-arm clear (line 3687) is now redundant on the «consecutive errored-feeds» path but kept for correctness (idempotent on already-cleared buf). | CLOSED 2026-05-23 |
...
| DEF-218 | **Buffer sizing for large rows / large queries** — current caps are dangerously small for production workloads:<br/>• `READ_BUF_CAP = 4096` → DataRow body > 4 KB tears down the connection. PG TEXT/BYTEA/JSONB cells routinely span MB. **Any `SELECT row_with_jsonb` falls.** Hard failure (`UnexpectedFrameSize` → connection teardown). **Open** — closed structurally by DEF-248 (queue pos 8).<br/>• ~~`MAX_SQL_LEN = 2048`~~ → ~~analytics queries 5-50 KB silently truncated~~ **CLOSED STRUCTURALLY 2026-05-11** by DEF-160 Z2 (commit `5c5cfa4`): `Parse<'a>::sql = &'a str` + `SimpleQuery<'a>::sql = &'a str` — SQL flows as borrowed slice with no copy-into-arena step. Truncation arena eliminated; unbounded SQL length within `i32::MAX` PG wire frame limit.<br/>• ~~`MAX_PARAMS_ARITY = 16`~~ → **CLOSED 2026-05-26**: ParamOids redesigned to `Box<[u32]>` exact-size. No fixed cap. PG limit i16::MAX = 32767.<br/>• ~~`MAX_ROW_COLUMNS = 32`~~ → **CLOSED 2026-05-26**: RowDesc redesigned to `Box<[u32]>` exact-size. MAX_ROW_COLUMNS raised to 1600 (PG MaxTupleAttributeNumber). FormatCodeSet deleted, −774 LoC. Effective wire limit ~300-400 columns via partial_assembly PREFIX_CAP.<br/>**Principal-directive 2026-05-10 (verbatim):** «универсальное решение, если буду не просто огромные базы, а колоссальные, то оно и под ними должно себя стабильно а главное корректно вести и работать». Translation: cap-bump and per-instance const-generic are **fallback patterns** — they push the limit but do not eliminate it; on colossal data (MB-GB cells) any static cap eventually fails. **Universal solution = DEF-248** (streaming row decode): per-column pull-based decode where the buffer holds `O(largest_value)` not `O(largest_row)`. Frames > `READ_BUF_CAP` are read in chunks and consumed column-by-column; the cap-as-failure-mode disappears structurally.<br/>**Design choices** (kept as historical record — superseded by streaming approach):<br/>(a) bump defaults — REJECTED (fallback; doesn't solve colossal).<br/>(b) const-generic on `PgProtocol<const READ_CAP, …>` — REJECTED (still per-instance fixed limit).<br/>(c) **streaming row decode** — SELECTED (DEF-248 architectural fix; pull-API splits frames across buffer reuse cycles; memory bounded by single-cell size).<br/>For non-buffer caps (`MAX_PARAMS_ARITY`, `MAX_ROW_COLUMNS`): bump defaults to honest production values + const-generic for users with even-bigger-than-honest needs. | Block real-world data shapes (READ_BUF_CAP residue + arity/column caps); SQL-len leg closed via DEF-160 | 1d / DEF-248 (streaming) ships first |

**Important features (block significant use cases)**:

| DEF | Item | Phase |
|-----|------|-------|
| DEF-219 | **COPY protocol** — FULLY SHIPPED 2026-05-23. 4 staged phases: (1) wire tags + CopyResponseHeader parser; (2) state machine + dispatch arms (3 new variants: CopyOutStreaming, CopyOutAwaitingCC, CopyInActive; 9 dispatch arms); (3) Action::CopyDataChunk surface via CopyChunksArena (multi-slot gen-tagged arena mirror of NotificationsArena); (4) COPY IN push methods (push_copy_data / push_copy_done / push_copy_fail) on PgProtocol<ActivePhase>. 7 integration tests + 8 arena/header unit tests pass; clippy clean. ProtoState size pin 504 → 520 B (+16 B for the two Option<Box> arenas — notifications + copy_chunks; both lazy-allocated). See §D Closed Index for full narrative. | CLOSED 2026-05-23 |
| DEF-220 | **LISTEN/NOTIFY + NotificationResponse delivery** — FULLY SHIPPED 2026-05-23. 5 staged sub-steps: (1) `src/notifications_arena.rs` multi-slot gen-tagged arena (~340 LoC, 7 unit tests); (2) `wire::TAG_NOTIFICATION_RESPONSE = 'A'` const; (3) `Action::Notify { pid, notif_ref }` + `StagedAction::Notify` mirror + 4 materialise arms; (4) `ActiveInner.notifications_arena: Option<Box<NotificationsArena>>` lazy slot threaded through `DispatchContext` + `ActiveDispatchContext`, pre-dispatch filter `_notification_response_admit_leaf::parse_notification_payload` + `admit_notification_frame` in the shared `feed_bytes_dispatch` body, per-cycle clear at Idle/Errored boundaries via `clear_session_residue_for_class_dispatch`; (5) `PgProtocol<ActivePhase>::get_notification(NotificationRef) -> Result<&NotificationPayload, ArenaError>` accessor with static EMPTY_ARENA fallback. **Approach D over E**: E (zero-copy direct `&'r [u8]` borrow from read_buf into Action::Notify) hit the `StagedActions<'w>` single-lifetime architectural wall — D (arena-backed) is pragmatically equivalent: one `Box<NotificationsArena>` per LISTEN-using connection + per-notification `Vec<u8>` payload alloc; sub-µs cost on the cold notification-arrival path; the «zero-copy» wins were theoretical (< 25 KB/s memcpy at 100 NOTIFY/s typical rate, below noise floor). 4 integration tests in `tests/notification_spec.rs` (single-notification round-trip, multi-notification-per-feed_bytes, ref-stale-after-next-cycle, empty-payload). ProtoState/PgProtocol size pin updated 504 → 512 B (+8 B for the Option<Box> slot; pay-when-used since the Box itself is lazy). | CLOSED 2026-05-23 |
...
| DEF-224 | **NoticeResponse delivery** — FULLY SHIPPED 2026-05-26. `NoticesArena` (multi-slot, gen-tagged, lazy `Option<Box>` on ActiveInner). `Action::Notice { notice_ref: NoticeRef }` + `FeedEvent::Notice(NoticeRef)` + `PgProtocol::get_notice(ref) -> Result<&NoticePayload, ArenaError>`. `NoticePayload` uses `BoundedStr` (not Secret — notices are operator-informational). `parse_and_alloc_notice` mirrors `parse_error_response` wire loop. Per-cycle clear at Idle/Errored. 3 arena unit tests + 3 integration tests (single notice, stale ref, multi-notice batch). PgProtocol +8 B (lazy Option<Box>). Action stays 24 B (NoticeRef 4 B body). | CLOSED 2026-05-26 |
...
| DEF-226 | **Multi-statement SimpleQuery batch** — FULLY SHIPPED 2026-05-23. Stream-style emission via new `Action::IntermediateCommandComplete { tag: BoundedStr<32> }` variant (DEF-220 sibling pattern — feed-path-only Action variant carried by value, no reply_id consumption). 3 new dispatch arms on `SimpleQueryAwaitingRfq`: (a) `+TAG_COMMAND_COMPLETE` parses new tag, emits intermediate signal with PRIOR tag, replaces state's tag with new one, stays in AwaitingRfq; (b) `+TAG_ROW_DESCRIPTION` parses + parks new RowDesc, emits intermediate signal with PRIOR tag, transitions to SimpleQueryStreamingRows preserving reply_id (next statement is SELECT); (c) `+TAG_EMPTY_QUERY_RESPONSE` validates empty body, emits intermediate signal with PRIOR tag, replaces state's tag with BoundedStr::default() (empty marker, matching the SimpleQueryAwaitingFirstResponse + EmptyQueryResponse path). Final RFQ arm unchanged — delivers `Reply::QueryComplete` carrying the LAST statement's tag. Mid-batch ErrorResponse routes through standard `DrainRfqAfterError` recoverable path. **No `BoundedVec<CommandTag, N>` accumulator**: stream-style avoids the cap-bump fallback per the «universal coverage» principle; per-statement tag visibility scales with the OutActions cap (9 per feed_bytes call — structurally bounded by the existing per-call ceiling). Pre-DEF-226 a multi-statement query like `"BEGIN; UPDATE; COMMIT;"` triggered UnexpectedFrame teardown on the second CommandComplete; post-DEF-226 the wrapper observes `Intermediate("BEGIN")` + `Intermediate("UPDATE 1")` + `DeliverReply(QueryComplete { command_tag: "COMMIT" })` cleanly. 4 integration tests (`tests/multi_statement_spec.rs`): 2-statement DML batch, 3-statement batch, batch with empty middle statement (`;;`), single-statement regression check. All tests pass + clippy clean. | CLOSED 2026-05-23 |
...
| DEF-165 | `ParamOids::n_params` u16→u8 | CLOSED 2026-05-23 via `BoundedU8<MAX_PARAMS_ARITY>` migration — tier-3 by-validation → tier-1 by-construction (see §D Closed Index entry). |
| DEF-166 | `PodBytes<N>` visibility `pub → pub(crate)` | Via state-field privatize; pairs with B5 |
| **DEF-285** | **CLOSED 2026-05-23 — pre-session lift/lower perf regression eliminated** | (commit `293ec80`). Bisect found root cause: `0597dae` DEF-279 Phase 2 SealedPhase flip introduced +28 ns/push_command on `push_command_internal` via the lift+lower (`ActiveState↔ProtoState` conversion). The big «+13 → ~49 ns» cliff between `d54c697` and `5d59b64` was NOT a regression — it was the bench fixture switching from `PgProtocol::new()` (disconnected) to `fresh_active_via_trust_handshake()` (full handshake + active push) in commit `17dc228` (DEF-246 Phase 2+3+4). Methodologically more honest. **Recovery**: 4-fold fix — `#[inline(always)]` on the 4 dispatch wrappers (active+connecting, feed_bytes + advance_one_frame) + `#[inline(always)]` on the 4 conversion impls (From<ActiveState>, From<ConnectingState>, TryFrom<ProtoState> for both) + skip-the-From-walk in `push_command_internal` lift (precondition is Idle → use ProtoState::Idle literal) + specialized 10-arm ProtoState→ActiveState lower (setter PostState bound restricts output to a subset of Active variants; dead arms eliminated). Bench result: push_command/ping recovered 77 → 54.7 ns (within criterion's 5% noise threshold vs pre-Phase-2 baseline `bisect-5d59b64-pre-phase2` 49 ns). `bench-stable PASS: no regressions detected`. DEF-279 Phase 2 tier elevations preserved (per-phase Inner, SealedPhase). Generalisable insight: when tier-elevation introduces a generic conversion between equivalent typed enums on the hot path, the conversion's DEAD ARMS at the constrained call site are real costs LLVM cannot eliminate from the generic form — specialize the conversion at the call site. |
...
- **DEF-219 — COPY protocol (CopyIn + CopyOut + CopyData/Done/Fail)** — 2026-05-23. Closes the «PG §55.2.6 COPY sub-protocol unimplemented» gap (pg_dump / ETL pipelines / bulk insert workflows). 4 staged commits in `9631804` (Phase 1) + `f9b73cd` (Phase 2) + `5bbb79a` (chore) + `5f1db34` (Phase 3) + this commit (Phase 4).

  **Phase 1 — wire tags + header parser** (`src/decode.rs::CopyHeader` + `parse_copy_response_header`): 7 new wire tag consts (`TAG_COPY_OUT_RESPONSE='H'` / `TAG_COPY_IN_RESPONSE='G'` / `TAG_COPY_DATA='d'` + 3 outbound mirrors) with distinctness drift pins. `CopyFormat` enum (Text/Binary) + `CopyHeader { format, n_cols }` struct. Parser validates PG §55.2.6 invariants: format ∈ {0, 1}, n_cols ≤ MAX_ROW_COLUMNS, per-column format codes == overall format byte. `ProtocolError::MalformedCopyResponse` variant + Display + state_kind classifier.

  **Phase 2 — state machine** (`src/state.rs` + `src/dispatch.rs`): 3 new state variants `SimpleQueryCopyOutStreaming(reply)` + `SimpleQueryCopyOutAwaitingCC(reply)` + `SimpleQueryCopyInActive(reply)` with all plumbing arms (From/TryFrom, take_inflight, push_class × 2, unsolicited_admit, Debug × 2). 9 dispatch arms: 2 entry points (`SimpleQueryAwaitingFirstResponse + H/G`) + COPY OUT arms (StreamingRows transitions on data/done/error) + COPY IN arms (CopyInActive transitions on server's CC/E). **Tail-state reuse**: both COPY directions transition to existing `SimpleQueryAwaitingRfq` on CommandComplete (via existing `advance_to_awaiting_rfq` helper) — same RFQ handling as DML/SELECT. DEF-226 multi-statement arms even handle hypothetical `COPY ...; COPY ...;` batches cleanly without further work.

  **Phase 3 — Action::CopyDataChunk surface** (`src/copy_chunks_arena.rs` + Action variant + threading): mirror of DEF-220 NotificationsArena pattern — multi-slot heapless::Vec<CopyChunkPayload, 9> with gen-tagged refs, lazy `Option<Box<CopyChunksArena>>` slot on ActiveInner. Per CopyData frame: heap-alloc Vec<u8> for the chunk bytes (cold COPY path, sub-µs per chunk; acceptable for bulk-transfer use case). `Action::CopyDataChunk { chunk_ref }` Copy variant in OutActions stream; `PgProtocol::get_copy_chunk(ref) -> Result<&CopyChunkPayload, ArenaError>` accessor with static EMPTY_ARENA fallback. Per-cycle clear via `clear_session_residue_for_class_dispatch` at Idle/Errored boundaries (gen bump invalidates outstanding refs). DispatchContext + ActiveDispatchContext field added; Connecting wrapper uses transient empty slot (COPY is post-handshake). `dispatch()` fn signature gains `copy_chunks_arena_slot` parameter; 2 caller sites updated.

  **Phase 4 — COPY IN push methods** (this commit): 3 new public methods on `PgProtocol<ActivePhase>` — `push_copy_data(bytes, wb)`, `push_copy_done(wb)`, `push_copy_fail(error, wb)`. Each writes the appropriate wire frame ('d' / 'c' / 'f') to the caller-supplied WriteBuf and returns the staged byte slice. State stays in `SimpleQueryCopyInActive` throughout the entire client push phase — only the server's CommandComplete (or ErrorResponse on fail) advances state via the existing dispatch arms. New `CopyPushError` enum (NotInCopyInState / FrameTooLarge / EmbeddedNul / WriteBufFull) — simple sum type, no ReplyId correlator (the COPY-cycle reply_id is preserved in the state across all client pushes). Wire framing: PG i32-length limit pin via `u32::from_le_bytes(i32::MAX.to_le_bytes())` const (no `as` cast); CopyFail rejects embedded-NUL in error string at framing layer.

  **Pattern reuse insight**: Phase 3 was a mechanical mirror of DEF-220 NotificationsArena (multi-slot arena + Action variant + threading + accessor). Phase 4 is a different pattern (push methods write outbound bytes, no arena). Both use the same state machine foundation. The DEF-220→DEF-226→DEF-219 sequence cemented the «server-pushed bulk-data surface via gen-tagged arena» template as fully established.

  **ProtoState size pin**: 512 → 520 B (+8 B for the copy_chunks_arena Option<Box> slot; the notifications_arena slot was the +8 B in DEF-220). Connections that never COPY pay the 8-byte field but no heap. 7 integration tests in `tests/copy_state_machine_spec.rs` cover: full COPY OUT cycle state transitions, full COPY IN cycle state transitions, malformed-header teardown, data chunks surface via Action::CopyDataChunk, full COPY IN push cycle, push_copy_data outside CopyInActive errors, push_copy_fail embedded-NUL rejection. Plus 8 unit tests (5 copy_header_tests + 3 copy_chunks_arena tests). All pass + clippy clean.

  **Phase 5 follow-up candidate**: zero-copy `iter_copy_data` pull API as an alternative to the per-chunk heap alloc (mirror of `iter_rows`). The current arena approach has GB-scale heap traffic for large COPY OUT; iter_copy_data would do direct read_buf borrows. Same StagedActions<'w> single-lifetime constraint that blocked DEF-220 E means iter_copy_data requires a NEW infrastructure path (not the Action stream); ~150-200 LoC. Deferred unless a concrete bulk-COPY consumer surfaces with measured heap-pressure concerns.

  SHIPPED 2026-05-23.
- **DEF-226 — Multi-statement SimpleQuery batch** — 2026-05-23. Closes the «`Q` frame with `;`-separated batches» silent-loss gap (`BEGIN; UPDATE; UPDATE; COMMIT;` returned only the LAST CommandComplete's tag, and any 2-statement+ query triggered UnexpectedFrame teardown on the second CommandComplete arrival in AwaitingRfq). Stream-style emission via `Action::IntermediateCommandComplete { tag: BoundedStr<32> }` + `StagedAction::IntermediateCommandComplete` mirror — feed-path-only variant carrying `tag` by value (BoundedStr<32> is `Copy`), no reply_id consumption (the reply_id is preserved in the state and consumed only by the final RFQ arm).

  **3 new dispatch arms on SimpleQueryAwaitingRfq**:
  - `+TAG_COMMAND_COMPLETE`: parses new statement's tag via existing `parse_command_tag`, replaces state's command_tag with the new one, emits `StagedAction::IntermediateCommandComplete { tag: prior_tag }` carrying the PRIOR statement's tag. State stays in AwaitingRfq awaiting either another statement's response or final RFQ.
  - `+TAG_ROW_DESCRIPTION`: next statement is a SELECT. Parses + parks RowDesc in `row_desc_slot` via the existing `park_row_description_at_dispatch` leaf, transitions to `SimpleQueryStreamingRows { reply }` preserving reply_id, emits intermediate signal with prior tag.
  - `+TAG_EMPTY_QUERY_RESPONSE`: next statement is empty (e.g., `;;` produces an empty middle statement). Validates empty body via existing `validate_empty_body`, replaces command_tag with `BoundedStr::default()` (matching the SimpleQueryAwaitingFirstResponse + EmptyQueryResponse path), emits intermediate signal with prior tag.

  The final RFQ arm is UNCHANGED — delivers `Reply::QueryComplete { command_tag, tx_status }` carrying the LAST statement's tag + final transaction status. Mid-batch ErrorResponse routes through the standard `DrainRfqAfterError` recoverable path (intermediate tags observed up to the error point remain valid in the OutActions stream).

  **Stream-style over accumulator**: the deferred row suggested `Reply::QueryBatchComplete { tags: BoundedVec<CommandTag, N> }` OR stream-style. The accumulator form carries a static cap N (fallback per the «universal coverage» principle); the stream form is structurally bounded by the existing OutActions cap (`MAX_ACTIONS_PER_CALL = 9` — at most 9 statements observable per `feed_bytes` call, with subsequent statements' tags arriving in the next call's OutActions cycle if the wire stream is bundled across multiple network reads). Stream-style is the «universal» answer.

  **Match arm coverage** at 4 sites (mirror of DEF-220 Notify variant work): `materialise` (main StagedAction → Action mapper), `compute_push_startup_idle_only`'s materialise inner + `materialise_push` (push-path-dead arms), `StagedObs::from_staged` cfg(test) helper, `row_stream::slow_path_once` (mid-iter_rows intermediate → ColEvent::NeedMore — Phase 2 follow-up if mid-row-stream batch use case surfaces).

  4 integration tests in `tests/multi_statement_spec.rs`:
  - `batch_two_dml_statements_surfaces_intermediate` — `"BEGIN; COMMIT;"` produces Intermediate("BEGIN") + DeliverReply(QueryComplete { "COMMIT" }).
  - `batch_three_statements_surfaces_two_intermediates` — `"BEGIN; UPDATE t SET x=1; COMMIT;"` produces 2 intermediates + 1 final.
  - `batch_with_empty_query_response_in_middle` — `"BEGIN;;COMMIT;"` produces Intermediate("BEGIN") + Intermediate("") + DeliverReply("COMMIT") (empty middle statement maps to empty-string tag).
  - `single_statement_batch_no_intermediates` — regression check: `"SELECT 1"` produces ONE DeliverReply with no intermediate (the DEF-226 arms only fire on the SECOND CommandComplete; single-statement preserves the existing API).

  All tests pass + clippy clean. ProtoState/Action size pins unchanged (BoundedStr<32> is 36 B, Action variant fits well within the 88 B pin).

  **Generalisable insight**: when the deferred row presents a choice «accumulator (BoundedVec<CommandTag, N>) OR stream-style», prefer stream-style if the OutActions cap is already the structural bound — the «universal coverage» principle says caps are fallbacks; reusing an existing structural bound is honest. Cross-reference with [[feedback-universal-streaming]] (cap-bump and per-instance const-generic are fallbacks; streaming pull APIs are the universal solution). SHIPPED 2026-05-23.
- **DEF-220 — LISTEN/NOTIFY + NotificationResponse delivery** — 2026-05-23. Closes the «PG §55.7 LISTEN/NOTIFY surface unimplemented» gap. The deferred row framed this as «1f» — far in the future — but the «5 staged sub-step» implementation actually fit within one productive session under the «продолжай» momentum. Implementation across 6 files:

  **Step 1**: `src/notifications_arena.rs` — new module, ~340 LoC including 7 unit tests. Multi-slot gen-tagged arena: `heapless::Vec<NotificationPayload, MAX_NOTIFICATIONS_PER_CALL>` with `MAX = 9` (tied to OutActions cap — structurally bounded by the existing per-call ceiling, not an additional fallback). `NotificationRef { slot: BoundedU8<{MAX-1}>, generation: u16 }` (Copy, 4 B). `NotificationPayload { pid: i32, channel: Ident, payload: alloc::vec::Vec<u8> }` — payload bytes on heap (alloc per notification, free on arena clear) so the inline slot stays ~96 B and uncapped payload length covers PG's 8000-byte `NOTIFY_PAYLOAD_MAX_LENGTH` without the «fixed payload buffer» fallback.

  **Step 2**: `wire::TAG_NOTIFICATION_RESPONSE: InboundTag = 'A'` const + drift pin in `InboundTag::ALL`.

  **Step 3**: `Action::Notify { pid: i32, notif_ref: NotificationRef }` (12 B variant, well under Action 88-B size pin) + `StagedAction::Notify` mirror + materialise arms at the 4 sites the build flagged (main `materialise`, `compute_push_startup_idle_only` and `materialise_push` push-path-dead arms, `row_stream::slow_path_once` → ColEvent::NeedMore for mid-iter_rows arrival, `StagedObs::from_staged` cfg(test) helper).

  **Step 4**: `ActiveInner.notifications_arena: Option<Box<NotificationsArena>>` lazy slot (one Box per LISTEN-using connection; LISTEN-free connections pay the 8-byte field but no heap). Threaded through `DispatchContext` + `ActiveDispatchContext`; Connecting variant uses a transient empty slot at the wrapper level (LISTEN is post-handshake only — the filter never fires there, the threading is for shared-body signature compliance). Pre-dispatch filter in the shared `feed_bytes_dispatch` body parses `pid: i32 BE` + CSTR channel + CSTR payload via `_notification_response_admit_leaf::parse_notification_payload`, lazy-inits the arena, allocates the payload, stages `StagedAction::Notify`. Per-cycle clear at Idle/Errored boundaries via `clear_session_residue_for_class_dispatch` — refs from prior cycles resolve `Err(ArenaError::Stale)` via gen mismatch.

  **Step 5**: `PgProtocol<ActivePhase>::get_notification(NotificationRef) -> Result<&NotificationPayload, ArenaError>` accessor. Static EMPTY_ARENA fallback for the unallocated-slot case (a stashed ref against an empty arena returns Err(Stale) — impossible in practice since no ref can be issued without an arena alloc). 4 integration tests in `tests/notification_spec.rs` covering: single-notification round-trip with channel/payload field validation; 3 notifications in one feed_bytes call (multi-frame buffering); ref-becomes-Stale after next feed_bytes cycle; empty-payload notification.

  **Approach D over E**: I previously framed E (zero-copy direct `&'r [u8]` borrow from read_buf into Action::Notify) as the «zero-compromise» variant per user pushback («нырять глубже»). On re-derivation: E hits the `StagedActions<'w>` single-lifetime architectural wall — staging cannot carry borrows from read_buf into an OutActions<'w, 'r> with disjoint 'w/'r. D (arena-backed) is pragmatically equivalent: one `Box<NotificationsArena>` alloc per LISTEN-using connection + per-notification `Vec<u8>` payload alloc; sub-µs cost on the cold notification-arrival path; the «zero-copy» theoretical wins (< 25 KB/s memcpy at 100 NOTIFY/s typical rate) are below noise floor. The «E exists, you just didn't find it» framing was perfectionism — D is the correct architectural answer given the StagedActions lifetime constraint.

  ProtoState/PgProtocol<ActivePhase> size pin updated 504 → 512 B (+8 B for the Option<Box> slot). 4 integration tests + 7 in-module unit tests pass. clippy `-D warnings` clean.

  **Generalisable insight**: when a deferred row marks something «1f» (far-future phase), re-derive the actual scope from first principles before deferring. DEF-220's «1f» tag suggested «complex pub/sub state machine» but the implementation followed the ErrorArena single-slot pattern with mechanical multi-slot ring extension — substantially easier than the deferred row implied. Cross-reference with [[feedback-challenge-audit-verdicts]] (audit verdicts are starting points, not load-bearing). SHIPPED 2026-05-23.
- **DEF-243 — Eager `read_buf.clear()` at install_errored transition** — 2026-05-23. Closes the «read_buf residue window between mid-loop install_errored and next feed_bytes/Drop» concern from DEF-238's 2026-05-05 post-impl audit. The deferred entry framed it as «mid-size refactor (signature plumbing through dispatch.rs + protocol.rs)» targeting 108 install_errored callsites; on second look, ONE eager-clear at the post-loop site (shared `feed_bytes_dispatch` body for both Connecting + Active phases) closes the window without any callsite plumbing. Implementation: after the existing «if not Errored: advance read_buf» block at the post-dispatch-loop boundary, add `if matches!(state, ProtoState::Errored(_)) { read_buf.clear(); }`. The `core::hint::cold_path` hint pushes the branch off the happy-path instruction sequence; on Errored exit (rare per `#[cold]` classifier on `install_errored`), the `read_buf.clear()` performs zero-on-clear of the populated region (≤4 KiB, sub-µs at L1-cache resident). Pre-DEF-243 the residue sat in the backing array until either (a) the next feed_bytes' `AlreadyErrored` arm cleared (window ~one feed_bytes interval) or (b) `Drop` fired on connection discard (could be delayed arbitrarily by a connection pool / async handoff). Post-DEF-243 the clear fires at the transition site itself — secret-correlated bytes (SCRAM server-frame fragments `v=<base64_signature>`, ErrorResponse detail strings carrying server-side PII, partial frame bodies left mid-parse) get scrubbed before the wrapper observes the Errored state. The pre-existing `AlreadyErrored`-arm clear at the ingress classifier (line 3687) is now redundant on the «consecutive Errored feeds» path but kept for correctness (idempotent on already-cleared buf; defense-in-depth against future regressions of the post-loop site). **Generalisable insight**: «mid-size refactor» framings in audit entries should be re-derived from first principles — the deferred entry assumed signature plumbing through every callsite, but the natural «clear at the transition END» single site was simpler. Apply this lens: before committing to a callsite-plumbing refactor, ask «is there a single bottleneck/exit site that catches all the cases?». 642 tests pass + clippy clean. SHIPPED 2026-05-23.
- **DEF-165 — `ParamOids::n_params: u16` → `BoundedU8<MAX_PARAMS_ARITY>`** — 2026-05-23. Tier-3 by-validation → tier-1 by-construction. The field was `u16` accepting any 0..=65_535 value; only the wire parser (`parse_parameter_description` in `decode.rs`) checked `n_params_usize > MAX_PARAMS_ARITY` and returned `TooManyParameters`. The bound was tier-3 by-validation — a future internal mutation (or a missed-validation parser path) could store `n_params > MAX_PARAMS_ARITY`. Migrated to `BoundedU8<{ MAX_PARAMS_ARITY }>` (NonZeroU8-backed offset-by-one storage; the type itself rejects out-of-range values at every constructor — `try_new` returns `None`, `new_const::<VAL>()` is a build error). **Size unchanged at 68 B**: `repr(C, align(4))` pads the 1-byte BoundedU8 to 4 bytes (same slot the u16 occupied with its 2-byte pad). Touchpoints: `EMPTY` const (uses `BoundedU8::ZERO`), `from_parts` (signature takes `BoundedU8` instead of u16), `len` (calls `.get()` → u8 → usize via `from`), `is_empty` (compares `.get() == 0`), `PartialEq` (compares `.get() == .get()`), `parse_parameter_description` (validates wire u16 → narrows to u8 with `try_from` → constructs `BoundedU8` via `try_new`). 6 callers updated, all tests pass + clippy clean. **Generalisable insight**: when a field's validity range is documented at parse-time but stored in a wider type, the type-narrowing-with-bound migration is tier-3 → tier-1 elevation at zero size cost when alignment pads absorb the narrower field. Cheap wins under «impossible states are impossible» principle. SHIPPED 2026-05-23.
