...
- Pedantic-audit Tier 4 sweep batch 4 (Cluster A pilot + 3 small-batch addenda + 2 mid-size pairs + buffer-substrate trio) — 2026-05-19 — **5 pilot files + 3× small-batch (17 files) + 2 mid-size pairs (4 files) + buffer-substrate trio (3 files); 29 files total. All small files closed; two audit-anchor-bearing mid-size files closed; second mid-size pair (push_command.rs + session_params.rs) closed; buffer-substrate trio (buf.rs + wire.rs + write_buf.rs) closed.**
  ...
  **Remaining Cluster A scope** — **mid-size & protocol.rs only; 7 mid-size files closed (row_stream, reply_id, push_command, session_params, buf, wire, write_buf)**:
  - protocol.rs (554 refs, ~9000 LoC) — single largest target; could be staged as section-by-section sub-batches.
  - dispatch.rs (145), decode.rs (125), action.rs (130), lib.rs (120), error.rs (100), ident.rs (69) — top mid-size cluster.
  - state.rs (76) — remaining mid-size cluster (no known audit anchors).
  Total remaining: ~1180 refs across 8 files. Recommended: ≤3 files per mid-size session, ≤1 file per session for protocol.rs sub-batches.
