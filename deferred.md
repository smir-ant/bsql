...
| DEF-094 | **Re-opened** (2026-04-19) | **Initial rejection was premature.** Registered as rejected after a naive attempt that placed `Action<'buf>::SendBytes(&'buf [u8])` directly in `OutActions<'buf>`. The borrow checker correctly rejected that — `OutActions<'buf>` captured the `&'buf mut write_buf` borrow across `feed_bytes`'s multi-frame drain loop, preventing subsequent re-borrows. A paranoid 2026-04-19 audit (architect sub-agent, full-crate pass) identified a **staged-dispatch architecture** that sidesteps this: **Phase 1** (mut) — loop dispatches write bytes into `self.write_buf`, emitting `StagedAction` values carrying *ranges* (no refs). **Phase 2** (shared) — once the write loop terminates and the mutable borrow is released, iterate `StagedAction`s and materialise `Action<'buf>::SendBytes(&self.write_buf.as_bytes()[range])`. Multiple shared borrows into the same underlying storage coexist without conflict — standard borrow-checker semantics. Perf gain reframed by audit: shrinks `Action` from 864 bytes (512-byte inline `SendBuf` buffer) to ~32 bytes, shrinks `OutActions` from 3464 bytes to ~144 bytes. **~3.3 KB of stack allocation elided per `feed_bytes` call, even when zero actions are emitted.** This is the real win — not the 5-byte Sync memcpy. See §14 Phase-1c batch 2 for implementation commit. |
