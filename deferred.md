...
| DEF-208 | **`compute_push_*` Idle-only refactor** — surfaced by DEF-198 audit (surface 6, "это точно tier-1 полностью?" probe). Post-DEF-198 ship, public API surface (push via `ReadyGuard`) is tier-1 — the `pub(crate)` visibility on `push_command_internal` makes non-Idle push unreachable from outside the crate. **However**, internal `compute_push_<cmd>_*` functions still match exhaustively over `StatePushClass` (5 arms: Idle / Errored / Connecting / PingAwaiting / BusyQuery), with the four non-Idle arms emitting `FailReply` defensively. From the public API's perspective these arms are **dead code** (unreachable via guard); from the internal perspective they are tier-3 defensive fallbacks against future internal callers (none currently exist — `push_command_internal` is invoked only by `ReadyGuard::push_command`). Refactor: extract `compute_push_<cmd>_idle_only(cmd_payload, &mut ProtoState, ...)` per command (Ping / Startup / SimpleQuery / Parse / Bind+Execute / DescribeStmt / DescribePortal / Close), each with **single Idle-arm body** (no dispatch). `ReadyGuard::push_command` routes through these instead of via the dispatching `compute_push`. Optionally introduce `IdleStateProof: ZST` witness type, constructible only inside ReadyGuard, passed as parameter to `compute_push_<cmd>_idle_only` to make the "state is Idle" precondition tier-1 by-construction even at internal-fn boundary. Closes DEF-198 surface 6 in tier-1. | Tier-3 (internal defensive arms) → tier-1 (Idle-only sealed by `pub(crate)` + optional ZST witness). Surface 5 (runtime "is state Idle right now" via `Option<ReadyGuard>`) remains tier-3 — irreducible per Postgres server-driven state nature. | Tier elevation closure for DEF-198 internals + recovers ~+3% perf cost from `push_command/ping` bench (the 5-arm dispatch on `state.push_class()` becomes single Idle-arm). ~250-400 LoC: per-command Idle-only fns + optional `IdleStateProof` ZST + ReadyGuard routing change. Internal-only refactor (no public API change). | PROPOSED, tier-1 closure of DEF-198 surface 6 |
...
**Priority order (§1) — REVISED 2026-04-28 after DEF-198 ship + DEF-208 register:**
1. **Tier-1 closure (DEF-198 follow-up):** DEF-208 — `compute_push_*` Idle-only refactor. Closes DEF-198 internal tier-3 defensive arms (surface 6); restores +3% perf cost from `push_command/ping`. Per CREDO §1 priority pyramid (safety > tier > perf), tier-1 closure preferred over decoder perf wins.
2. **Decoder perf wins (bench-gated by DEF-197):**
