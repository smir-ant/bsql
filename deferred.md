...
|-----|------|--------------|-------|
| DEF-214 | **TLS / SSLRequest pre-startup state machine** — send 8-byte SSLRequest packet (length=8, version=80877103), parse `'S'` (start TLS) / `'N'` (plaintext) / ErrorResponse byte. New `ProtoState::ConnectingPreSslHandshake` variant. The actual TLS handshake (rustls) lives in `bsql-driver-postgres`, but the proto MUST track the pre-startup phase. Without this, **no cloud PG accessible** (RDS, Cloud SQL, Heroku, Supabase, Neon, Aiven all require TLS). Tier-1 closure: typed pre-handshake states with exhaustive dispatch arms. | Block all cloud PG | 1b ext / 1e |
| DEF-215 | **Cleartext password auth** (R/3) — server sends `AuthenticationCleartextPassword`; client sends `PasswordMessage` ('p') with NUL-terminated password. **FOUNDATION SHIPPED 2026-05-05**: `AUTH_CLEARTEXT_PASSWORD = 3` const, `AuthSubCode::CleartextPassword` variant, drift-pin asserts, `assert_all_distinct_raw!` updated. Both existing dispatchers (Trust path + SCRAM path) updated to classify the code as `KnownButWrong` rejection — neither carries cleartext-password capability. **Pending**: `Credentials::CleartextPassword(Sensitive<Password>)` variant + `ConnectingCleartextAwaitingAuthOk` ProtoState variant + new dispatcher arm + `PasswordMessage` builder + integration tests. Trivial protocol-wise (~50 LoC), but cascade through Credentials enum + Startup state split (mirror DEF-097 Trust/Scram split). | Block legacy on-prem | 1b ext |
| DEF-216 | **MD5 password auth** (R/5) — server sends `AuthenticationMD5Password` with 4-byte salt; client sends `PasswordMessage` containing `"md5" + md5_hex(md5_hex(password+username) + salt)`. Adds `md5` RustCrypto crate dependency (DEF-META-01: never hand-roll crypto). Many enterprise on-prem PG installations still on MD5 (PG ≤ 13 default). **FOUNDATION SHIPPED 2026-05-05** (sibling to DEF-215): `AUTH_MD5_PASSWORD = 5` const, `AuthSubCode::Md5Password` variant, drift-pin asserts. **Pending**: `md5` workspace dep, `Credentials::Md5Password` variant, ProtoState variant, dispatcher arm, salt extraction from auth payload, `PasswordMessage` builder, integration tests. | Block enterprise on-prem | 1b ext |
