...
| 2 | DEF-280 | **Safety Bundle Group — Bundle F + push_within_fanout_budget residue remaining** (memo `/tmp/def-rethink-safety-memo.md` 1100 LoC). 8 of originally-scoped 9 work items **SHIPPED 2026-05-18** in sequence with asm-diff + bench-stable verification at each commit: **G** `1cf6041` (debug_assert!(false,…) glass pattern), **H** `7b7a8c4` (RowStream !Send), **J** `0286840` (distinct CRATE_BUG_REPLY_ID_SENTINEL=NonZeroU64::MAX), **K** `b66dc4a` (enter_partial_mode typed-Err), **I/D''** `e02eed7` (Zeroizing<i32> stack-slot scrub), **K-mirror** `83d5a09` (exit_partial_mode typed-Err + upstream-discipline removal), **DEF-281 A/B/C/D** `9c25c1e` + `df42ab0` (silent row-truncation closure), **E Phase 1 + P-D280E-1/2 + sweep** `315c178` + `a7d6424` + `2041db8` (Sensitive::get → with_inner closure-scope + 2 hostile probes + 3 non-fundamental .unwrap_or(&[]) sites). Total +1334 / −175 LoC across 13 commits; 606 → 619 tests (+13 spec/drift/probe pins); 3 hostile probes (P-D280H-1, P-D280E-1, P-D280E-2); 3 new CrateBugLocus variants (PushEmittedDeliverReply, PartialModeReentry, PartialModeExitUndrained); 2 new cluster δ leaf submodules; 2 new error types (AlreadyInPartialMode, PartialModeExitUndrained); 1 new const (CRATE_BUG_REPLY_ID_SENTINEL); 1 public API break (Sensitive::get → with_inner). **Remaining work**: (a) **Sub-bundle F** (~150 LoC, BREAKING within-crate): migrate sealed-supertraits → concrete-tokens — `PostStateProof` / `DescribeName` tier-1 external (pub(crate) sealed), tier-2 within-crate (any in-crate module can `impl Sealed + impl PostStateProof for HostileWitness` and write any `ProtoState` variant via `install_into`). DEF-279 «Subsumes/closes ... partial of DEF-274» — Bundle F is partial DEF-274 (cell-encapsulation closure); deeper closure when DEF-279 lands per-phase Inner (some cells become `<Active>`-private inline). (b) **`push_within_fanout_budget` Bundle G-residue** — Bundle G removed the dev-loud debug_assert but the underlying tier-2 structural (const-asserted-architecturally-dead) classification persists; true tier-1 elevation requires either `unsafe push_unchecked` (forbid'd) or unstable `generic_const_exprs` (capability witness on OutActions). Defer until rust-version unlocks the feature OR an architectural rethink moves to a typed-witness API. (c) **Bundle E Phase 2** (SecretBoundedStr::as_str/as_bytes closure-scope): no docstring discipline rule on these accessors — Rust lifetime-checking is sound for &str/&[u8] borrows; closure-scope migration would be cosmetic with no real tier-elevation. Skip unless a specific use case surfaces a docstring rule worth eliminating. (d) **Fundamental ReadBuf primitives** (`buf.rs:174/627`): defer to DEF-279 — refactoring to Result-returning accessors cascades through every ReadBuf caller. | 1 sub-bundle (F) + 1 push_within residue remaining; SHIPPED 8/9 work items closing the no-API-break tier-neutral half AND the user-visible silent-truncation class AND the foundational secret-borrow retention class | F breaking on PostStateProof/DescribeName impl callers; push_within_fanout_budget residue tier-neutral |
