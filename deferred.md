...
|-----|-----|------|----------|-----------|
| 1 | DEF-265 | **α footprint reduction (PgProtocol shrink)** — **TWICE MEASURED-REJECTED 2026-05-08.** Two distinct designs both implemented and bench-verified; both regressed. **Attempt 1 — Box<ReadBuf>** (commit-then-revert): `read_buf: ReadBuf` → `Box<ReadBuf>`. Bench: `ping_round_trip` **+54.65%** (heap-alloc cost on every fresh PgProtocol::new()). **Attempt 2 — caller-supplied `&'buf mut ReadBuf` via lifetime parameter** (commit-then-revert): `pub struct PgProtocol<'buf> { read_buf: &'buf mut ReadBuf, ... }`. Stack-allocated buffer in benches (no heap alloc), full breaking refactor across 19 files (`PgProtocol<'buf>`, `ReadyGuard<'a, 'buf>`, `RowStream<'p, 'w, 'buf>`, ~70 callsite updates, 5 iter_batched→b.iter conversions for self-referential setup). All 447 tests passed, clippy clean, size pin 4352 B → **248 B exact**. Bench: `ping_round_trip` **+18.01%** (119→140 ns) even though zero heap alloc — **cache locality regression** (PgProtocol metadata at 248 B and ReadBuf at 4112 B now occupy separate cache lines vs pre-DEF-265 single-cache-line inline layout) + +1 deref cost on every `self.read_buf.X()` access. Both attempts confirm a fundamental physical trade-off: **the 4096-byte buffer storage MUST live somewhere; co-locating it with PgProtocol metadata gives single-cache-line hot path; separating it (heap or stack-via-borrow) trades cache locality for inline footprint reduction.** No design that preserves perf AND shrinks inline footprint exists within `#![forbid(unsafe_code)]` + stable Rust constraints — the buffer storage is the inherent cost. **Cache-footprint optimization for connection pools achievable by users wrapping in `Box<PgProtocol>` themselves** (no library API change required); per-conn pointer chase is the same cost as Idea-31 measured here, but applied only when caller opts in. **Conclusion:** no library-side action; document fundamental trade-off. Working tree reverted (both attempts). Architect note: PGO (DEF-255 deferred) might inform LLVM's per-callsite layout heuristics enough to extract a different point on the trade-off curve — revisit with PGO data if footprint becomes a real-world bottleneck. | Both designs measured to regress (+54% and +18% on ping_round_trip respectively). REJECTED. | None — caller-side `Box<PgProtocol>` available today |
