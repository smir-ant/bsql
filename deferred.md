...
- Pedantic-audit Tier 3 sweep batch 3 — 2026-05-19 — **2 audit findings closed across 2 commits**.
  ...
  (2) **#46** `4ca54ed` — `LossyText<'a>` typed boundary at wire-decode. `#[repr(transparent)]` newtype over `&'a [u8]` exposes `from_bytes_lossy` (zero-cost wrap), `raw_bytes()` (escape hatch for byte-fidelity callers), `display()` (zero-alloc `fmt::Display` via `LossyDisplay<'a>`), `to_bounded::<N>()` / `to_secret_bounded::<N>()` (storage commitment, lossy coercion applied here). 3 `parse_error_response` text-field sites (M/D/H per PG §55.7) migrated to funnel through it. The audit's `as_str(&self) -> &str` shape deliberately omitted — incompatible with deferred coercion in safe Rust (would require alloc-on-construct or silent-empty fallback); `display()` covers the zero-alloc rendering case, `to_bounded::<N>().as_str()` covers the owned-storage case. 11 new contract tests in `lossy_text_tests` pin the four invariants (raw_bytes verbatim, display matches slow-path substitution, to_secret_bounded byte-identical to direct `from_bytes_lossy`, repr(transparent) layout). asm-diff `parse_error_response`: 0 codegen delta — the wrapper inlines to identical machine code as the prior direct call.
  Gates across both commits: clippy `--all-targets -D warnings` clean; **669/0/9** full workspace test suite (was 658 — +11 LossyText contract tests).
