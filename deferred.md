...
| DEF-218 | **Buffer sizing for large rows / large queries** — current caps are dangerously small for production workloads:<br/>• `READ_BUF_CAP = 4096` → DataRow body > 4 KB tears down the connection. PG TEXT/BYTEA/JSONB cells routinely span MB. **Any `SELECT row_with_jsonb` falls.** Hard failure (`UnexpectedFrameSize` → connection teardown).<br/>• `MAX_SQL_LEN = 2048` → analytics queries 5-50 KB silently truncated with `"…"` marker (semantic corruption — server runs the truncated query, returns "results" for the wrong query, no error). **More dangerous than crash** — wrong numbers in reports.<br/>• `MAX_PARAMS_ARITY = 16` → UNNEST bulk insert blows past limit at 50-1000+ params.<br/>• `MAX_ROW_COLUMNS = 32` → wide-table SELECT * (>32 cols) emits TooManyColumns.<br/>**Principal-directive 2026-05-10 (verbatim):** «универсальное решение, если буду не просто огромные базы, а колоссальные, то оно и под ними должно себя стабильно а главное корректно вести и работать». Translation: cap-bump and per-instance const-generic are **fallback patterns** — they push the limit but do not eliminate it; on colossal data (MB-GB cells) any static cap eventually fails. **Universal solution shipped via DEF-248** (streaming row decode): per-column pull-based decode where the buffer holds `O(largest_value)` not `O(largest_row)`. Frames > `READ_BUF_CAP` are read in chunks and consumed column-by-column; the cap-as-failure-mode disappears structurally.<br/>**Design choices** (kept as historical record — superseded by streaming approach):<br/>(a) bump defaults — REJECTED (fallback; doesn't solve colossal).<br/>(b) const-generic on `PgProtocol<const READ_CAP, …>` — REJECTED (still per-instance fixed limit).<br/>(c) **streaming row decode** — SELECTED (DEF-248 architectural fix; pull-API splits frames across buffer reuse cycles; memory bounded by single-cell size).<br/>For non-buffer caps (`MAX_SQL_LEN`, `MAX_PARAMS_ARITY`, `MAX_ROW_COLUMNS`): bump defaults to honest production values + const-generic for users with even-bigger-than-honest needs. SQL-len truncation gets ALSO closed by switching from copy-into-arena to `&'a str` lifetime API (DEF-160 in queue, eliminates the truncation arena). | Block real-world data shapes | 1d / DEF-248 (streaming) ships first |
