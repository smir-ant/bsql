...
|-----|------|--------------|-------|
| DEF-214 | **TLS / SSLRequest pre-startup state machine** — send 8-byte SSLRequest packet (length=8, version=80877103), parse `'S'` (start TLS) / `'N'` (plaintext) / ErrorResponse byte. The actual TLS handshake (rustls) lives in `bsql-driver-postgres`, but the proto MUST track the pre-startup phase. Without this, **no cloud PG accessible** (RDS, Cloud SQL, Heroku, Supabase, Neon, Aiven all require TLS). **WIRE BYTES SHIPPED 2026-05-05**: `wire::SSL_REQUEST_VERSION = 80_877_103` const + `wire::SSL_REQUEST_WIRE_BYTES: [u8;8] = [0,0,0,8, 0x04,0xd2,0x16,0x2f]` (pub, top-level re-export `bsql_pg_proto::SSL_REQUEST_WIRE_BYTES`); 5 tier-1 const-asserts (length, length-field literal, version-bytes match `to_be_bytes()` formula, no drift between byte-literal and `SSL_REQUEST_VERSION` const); 7 integration tests in `tests/ssl_request_wire_spec.rs` pin public-API surface (top-level + module path agree, byte-by-byte spec match, length-field includes self, distinctness from TERMINATE/Sync/PROTOCOL_VERSION_3_0, magic-version decomposition (1234 << 16 | 5679)). **Pending Phase 1e**: `ProtoState::ConnectingPreSslAwaitingResponse` variant + 1-byte response feeder (the response byte is OOB — single byte, no length prefix, doesn't fit our tagged-frame parser; driver handles parsing) + transition logic to dispatch into normal `ConnectingStartup*` after TLS handshake completes. Drivers can already write `SSL_REQUEST_WIRE_BYTES` to socket and read the response byte today; state-machine envelope tracking awaits the driver crate. | Block all cloud PG | 1b ext (wire SHIPPED) / 1e (state machine) |
...
| DEF-221 | **CancelRequest send flow** — special startup variant (length=16, version=80877102, pid, secret_key) sent on a *parallel* TCP connection to cancel an in-flight query. Requires storing BackendKeyData (pid + secret_key, already collected in `ConnectingPostAuthHaveKey` variant). New API surface: `proto.cancel_request_bytes(pid, secret_key) -> [u8; 16]`. Without this, `statement_timeout` server-side is the only cancel mechanism — no graceful client-initiated cancel. | 1e |
...
- **DEF-214 (wire-bytes phase)**: SSLRequest pre-startup primitive. `wire::SSL_REQUEST_VERSION = 80_877_103` (PG magic-version sentinel: 1234 << 16 | 5679) + `wire::SSL_REQUEST_WIRE_BYTES: [u8;8] = [0,0,0,8, 0x04,0xd2,0x16,0x2f]` (length-prefixed StartupMessage-shape: 4-byte length=8 + 4-byte version code BE). Tier-1 closure: 5 `const _: () = assert!(...)` drift-pins (length=8, length-field literal, version-bytes match `SSL_REQUEST_VERSION.to_be_bytes()` formula — not just paired-literal pin, the formula itself is load-bearing). Top-level re-export `bsql_pg_proto::SSL_REQUEST_WIRE_BYTES` for driver ergonomics. `tests/ssl_request_wire_spec.rs` 7 runtime tests + 3 const-asserts pin the public-API surface from a downstream crate's POV (top-level re-export equals module path, byte-for-byte spec match per PG §55.10, length-field self-inclusion convention, distinctness from TERMINATE/Sync/PROTOCOL_VERSION_3_0, magic-version 1234<<16-decomposition). **State-machine integration pending Phase 1e** (`ProtoState::ConnectingPreSslAwaitingResponse` + 1-byte response feeder). The 1-byte server response (`'S'`/`'N'`/`'E'`) is OOB — it doesn't fit our tagged-frame parser (no tag, no length prefix); driver handles parsing in its own pre-TLS reader, then constructs `PgProtocol::new()` for the post-TLS phase. Drivers can write `SSL_REQUEST_WIRE_BYTES` and consume the response byte today. SHIPPED 2026-05-05.
