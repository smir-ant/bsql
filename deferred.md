...
| **DEF-285** | **Pre-session perf regression investigation (NOT session-attributable)** | OPEN 2026-05-23, RESCOPED. **Initial misdiagnosis**: `bench-stable compare post-def248-subb` after the session showed +492% / +173% regressions on `push_command/ping` and `ping_round_trip/push_then_feed`. I attributed this to session changes. **Bisect verdict 2026-05-23**: ran fresh baseline at pre-session HEAD `7bb3346` (DEF-282), then `compare`d current HEAD vs pre-session — **3/3 benches show «No change in performance detected» (p > 0.05 on all three)**. Session is CLEAN. The actual cliff is BETWEEN `d54c697` (post-def248-subb baseline, 2026-05-12) and `7bb3346` (DEF-282, pre-session HEAD): `push_command/ping` 13 → 74 ns (+469%); `ping_round_trip/push_then_feed` 82 → 210 ns (+156%); `parse_header/rfq_header` 2.49 ns (unchanged). **Suspect commits in the d54c697..7bb3346 window** (~20 commits): (a) `7bb3346` DEF-282 perf(state): box ParamOids — though headlined -30.95% on cancel_credentials, other benches uncovered; (b) `5d3af48` refactor(partial-assembly): lift absorb()'s usize-return to (consumed, leftover) — touches the dispatch hot path's partial-assembly entry; (c) `33d38e1` refactor(narrow): introduce typed narrowing/widening helper module — adds function-call indirection on frame headers; (d) `abc791e` refactor(narrow): elevate 4 «display-saturation» sites. **Investigation continues**: bisect with `git worktree` against `d54c697..7bb3346` range; expected ~4-5 bench runs to narrow. **Baselines**: `pre-session-clean-7bb3346` (2026-05-23 anchor) + `post-def248-subb` (May 12 anchor) saved. Session conclusion: 21 commits, ZERO perf regression introduced. DEF-285 is now pre-session investigation, not session-attribution work. |
