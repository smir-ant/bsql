...
- DEF-278 Bundle D — 2026-05-18 — `9f75194` — **CancelRequest production primitive — query cancellation via PG §55.2.7 side-channel**. Pool layer (Phase 3) needs this to interrupt long-running queries before `reset_to_idle` recycle; without it `reset_to_idle` blocks on natural query completion. Architecture: new `cancel.rs` module with `BackendKey { pid: i32, secret_key: Sensitive<i32> }` captured at handshake-complete, wrapped in `BackendKeyCell` (`#[repr(transparent)]` over `Option<BackendKey>`) installed via token-gated `_backend_key_install_leaf::BackendKeyInstallToken` at the `(ConnectingPostAuthHaveKey, ReadyForQuery)` dispatch arm. `<ActivePhase>::cancel_request_credentials(&self) -> CancelRequestCredentials` exposes pid + secret_key via direct-return (NOT Option — type invariant `<ActivePhase>` ⇒ backend_key installed). `CancelRequestCredentials`: `pub fn pid(&self) -> i32` (non-sensitive — matches `StartupCompletePayload.pid`), `pub fn encode(&self) -> [u8; 16]` (4 B length + 4 B magic CANCEL_REQUEST_VERSION + 4 B pid + 4 B secret_key, all BE), `Drop` zeroizes inner `Sensitive<i32>` via existing zeroize-on-drop chain, `Debug` redacts secret_key. `CANCEL_REQUEST_LEN: u32 = 16` const in `wire.rs` with 3-fold drift assertion (length pin + slice-length pin + encoded-length-field pin); `cancel_request_bytes()` references `CANCEL_REQUEST_LEN.to_be_bytes()` (single source of truth). **3 new tier-1 invariants on top of DEF-246's 4** (memo §6): #8 CancelRequest wire bytes const-asserted, #13 `cancel_request_credentials` method-absent on every pre-handshake / terminal phase (3 hostile probes pin E0599), #14 `Drop` zeroizes secret_key (defense-in-depth — note: tier-1 against Drop-firing retention; `mem::forget`/`Box::leak` structurally possible but signposted by `Debug` redaction). **Approach E preserved**: method-absent gating replaces runtime-classify; zero `pub fn __test_*` / `_for_test_*` surfaces. **5 hostile probes** at `crates/bsql-pg-proto-derive/tests/def278d_compile_fail/`: P-d278d-1 (E0599 method-absent on `<DisconnectedPhase>`), P-d278d-2 (E0599 on `<ConnectingPhase>`), P-d278d-3 (E0599 on `<ClosedPhase>`), P-d278d-4 (E0603 `_backend_key_install_leaf` module private — cannot mint token outside leaf), P-d278d-5 (E0616 `CancelRequestCredentials.secret_key` field private — cannot read outside the encode/Drop paths). **13 spec tests** at `tests/cancel_request_spec.rs`: 5 wire-format pins (16-byte length, BE pid, BE secret_key, magic version, length field), Debug-redaction smoke test, drop-glue `needs_drop::<CancelRequestCredentials>()` shield, post-handshake direct-return, `pid()` round-trip, encode idempotence, accessor-doesn't-consume. **Verification**: 605/0/0 tests (591 → +14 new); 5/5 hostile probes pass; 9/9 DEF-246 hostile probes still pass; clippy `-D warnings` clean; release build clean; `forbid(unsafe_code)` preserved. **Size pins**: 528 B → 536 B (+8 B for BackendKeyCell field, niche-optimized) — all 6 pins (`PgProtocol`, `PgProtocolInner`, 4 phase-typed wrappers) sync at 536 B. **Bundle D is orthogonal** to DEF-278 Bundles A (Active/Errored split), B (StateCell newtype), C (reset_to_idle) — those ship in subsequent commits as multi-commit atomic sequence (each commit independently tier-1, no tier-2 windows between). 9 files, +1641 / −12 LoC.
