...
- Pedantic-audit Tier 4 sweep batch 4 (Cluster A pilot) — 2026-05-19 — **4 pilot files cleaned, pattern established for the remaining sweep**.
  ...
  **Pattern established for future sweeps**: per-comment evaluation. KEEP iff (current-state explainer OR architectural invariant OR external-spec ref OR load-bearing test/doctest OR CREDO classification rule). DELETE iff (pre-X/post-X narrative OR DEF-N attribution OR date stamp OR phase marker (`1c-*`, `Phase 1a`) OR future-roadmap reference OR internal-memo cross-ref). Stale `narrow_len_u16`-style references to deleted helpers also DELETE.
  Gates per file: clippy `--all-targets -D warnings` clean; tests pass (567/0/8 workspace for command.rs/guard.rs/password.rs; 240/0/0 lib for error_arena.rs); doctests compile.
  **Remaining Cluster A scope** (sized for follow-up sessions, NOT closed):
  ...
  - state.rs (79), session_params.rs (73), buf.rs (64), wire.rs (61), write_buf.rs (52), push_command.rs (49), reply_id.rs (43), row_stream.rs (39), state_setter.rs (32) — mid-size cluster.
  - 14 smaller files with 17-29 refs each.
  Total remaining: ~1900 refs across ~28 files. Recommended: ≤5 files per session to keep per-comment judgement sharp.
