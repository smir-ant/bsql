...
| 6 | DEF-248 | **Pull-based per-column decode — v1.0 production blocker** — `stream.col_next() -> ColEvent::Got(bytes) / NeedBytes / EndRow / EndQuery`. Eliminates whole-frame buffering. Frames > `READ_BUF_CAP` no longer require bumping caps (solves DEF-218 large rows structurally). **Per principal-directive 2026-05-10**: this is the **universal** solution for colossal data — cap-bump and const-generic are fallback patterns that don't actually solve the limit, just push it. Streaming makes the cap as-failure-mode disappear: memory becomes `O(largest_value)` (single cell) not `O(largest_row)` (whole frame). Without DEF-248, any `SELECT` against a column with multi-MB JSONB / TEXT / BYTEA tears down the connection — crate is **not v1.0-ready** for real-world data shapes. **Phased ship plan 2026-05-11 (principal-approved):** Two atomic PRs total. **Sub-A** (~1500-2000 LoC, closes D-tag v1.0 blocker): cycle-2 v2 memo §2 ColEvent + RowProgress + ColPhase types in row_stream.rs (Phase 1); ReadBuf `partial_remaining: u32` + `_row_stream_partial_leaf` submodule with `PartialFrameToken` ZST + `enter_partial_mode` / `exit_partial_mode` / `partial_visible_chunk` in buf.rs (Phase 2); `parse_header_streaming(unread, in_streaming_state: bool)` extension in frame.rs that disables `FrameTooLarge` rejection specifically for `'D'` in streaming-state (Phase 3); `col_next` state-machine in row_stream.rs for D-tag (Phase 3); closure-scoped `iter_rows<R, F>(&mut self, wb: &mut WriteBuf, f: F) -> R` in protocol.rs (Phase 4); `impl Drop for RowStream` install-Errored on `!drained` + new `CrateBugLocus::StreamDroppedMidStream` (1-byte niche-packed inside existing `InternalCrateBug`, preserves `ProtocolError == 72` size pin) + `install_errored_stream_dropped_mid_stream` on PgProtocol (Phase 5); delete `next_row` / `next_row_bytes` / `consume_rows` / `for_each_row` / `next_event` + `struct Row` + `enum StreamItem` + `pub use row_stream::{StreamItem}` re-export (Phase 6); migrate `tests/row_stream_spec.rs` (12 tests A-L) + `tests/simple_query_spec.rs::dml_after_select_clears_row_desc` + `tests/def198_guard_closure_spec.rs` references + `benches/hot_paths.rs` 5 `iter_rows_via_*` groups + new `iter_10cols_large_5kb_row` + `iter_jsonb_1mb_streaming` + `col_next_per_event_cost` benches + `benches/alloc_counts.rs::scenario_iter_rows_100` (Phase 7). After Sub-A: D-tag DataRow > 4 KB streams correctly; non-D tags > 4 KB continue to tear down as today. **Sub-B** (~5500-7000 LoC, closes universal coverage per v2 memo §4): per-tag streaming sinks for T (`SchemaArena` incremental column-def parser) + E/N (`ErrorArena` incremental TLV parser) + S (`SessionParamsSlot` incremental CSTR-pair parser) + R (per-sub-code dispatch into existing SCRAM state) + A (new `NotificationArena` lazy `Option<Box<>>` on PgProtocol) + C (new `CmdTagArena` lazy `Option<Box<>>`) + v (one-shot box for oversized NegotiateProtocolVersion); `select_sink(tag, state)` exhaustive `match` on sealed `InboundTag` in dispatch.rs (tier-1 by construction — new PG tag = compile error); lazy-box `session_params_slot` (`Option<Box<SessionParamsSlot>>`) as footprint compensation (-248 B vs +56 B new = -192 B net, preserves 520 B pin). After Sub-B: every PG wire-legal frame size handled; universal coverage per principal directive. **Multi-session reality:** architect cycles 2026-05-11 (×2) determined that even Sub-A is not single-architect-call atomic in tier-1 quality at one shot — Sub-A needs ~2-3 hours focused work with `cargo check` between each phase; Sub-B is ~4-6 hours. Plan accommodates dedicated multi-session sprints. | Architectural — memory becomes O(largest_value) not O(largest_row); closes DEF-218 READ_BUF_CAP residue | BREAKING canonical iterator API |
| 7 | DEF-246 | **Const-generic typestate `ProtoPipeline`** — runtime `ProtoState` (80 B enum) replaced by type-state chain: `PgProtocol<Closed>` → `<ConnectingStartup>` → `<Idle>` → `<QueryAwaitingRfq>` etc. State transitions are `move` (zero-cost). Server-driven dispatch (Auth subcode, ErrorResponse mid-handshake) via enum-of-typestate-variants on return path (50-70% type-statifiable, rest stays runtime). | 80 B `ProtoState` eliminated in Idle case + tier-1 transitions | BREAKING (most ambitious refactor) |
