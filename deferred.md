...
| DEF-203 | Exhaustive niche audit sweep — narrow `len: u16` fields to `BoundedU8<MAX>` where MAX ≤ 254. **Wave 1 SHIPPED 2026-04-28**: `OtherEncoding::len` (u16 → BoundedU8<32>); OtherEncoding 34 → 33 B (-1 B), Option<OtherEncoding> 36 → 33 B (-3 B). Wave 2 (PROPOSED): `PodBytes<N>::len` u16 → BoundedU8<{N as u8}> for `N ≤ 254` instances (SCRAM `PodBytes<48>` and `PodBytes<128>`). Requires generic-over-MAX-fitting-u8 cleanup; tier-1 by `const { assert!(N <= 254) }` block in PodBytes. ~80 LoC. Wave 3 (PROPOSED): `FixedStr<N, Tag>::len` for instances with N ≤ 254 (Ident, DatabaseName, StmtName, PortalName — all 63 B). Requires FixedStr generic-over-LenT (defaulting to u16 for backward-compat large-N, BoundedU8 for small-N). Larger refactor — ~150-200 LoC. | Tier-2 by-construct + tier-1 niche on `Option<T>` per site. | Wave 1 SHIPPED: 1-3 B saved per OtherEncoding/Option<OtherEncoding>. Cumulative across waves: ~10-30 B saved per protocol instance once all narrowed. | WAVE 1 SHIPPED, waves 2/3 PROPOSED |
...
2. **Architectural enabler:** DEF-199 (READ_BUF_CAP const generic — fewer syscalls on large frames).
3. **Architectural pre-discussion:** DEF-201 before any code (massive refactor; ≥3 alternatives per architect.txt process).
4. **Zero-cost micro-wins:** ~~DEF-195~~ SHIPPED. ~~DEF-203 wave 1~~ SHIPPED (OtherEncoding). DEF-203 waves 2/3 (PodBytes::len, FixedStr::len) PROPOSED — small scope follow-ups.
5. **Low-severity heap scrub:** DEF-206 (Box<PodBytes> SCRAM client-message scrub — wire-public bytes, low priority).
6. **DEF-202 follow-ups (decoder amortisation, design-discussion-gated):**
...
- **DEF-203 (wave 1)**: Niche audit — `OtherEncoding::len` narrowed `u16 → BoundedU8<32>`. Tier-2 by-construct (`BoundedU8::try_new` rejects len > MAX_ENCODING_NAME_LEN); tier-1 niche on `Option<OtherEncoding>` (NonZeroU8 niche absorbs discriminant). Layout deltas: `OtherEncoding` 34 → 33 B (1 B saved); `Option<OtherEncoding>` 36 → 33 B (3 B saved per Option). Two new const-asserts pin both sizes exactly. `from_truncated_bytes` migrated from the `narrow_len_u16` helper to a `u8::try_from(...).and_then(BoundedU8::try_new)` chain (still has architecturally-dead Err paths with `unwrap_or(BoundedU8::ZERO)` empty-fallback for no-panic discipline). All 19 test crates pass. Waves 2 (PodBytes::len for N ≤ 254) and 3 (FixedStr::len for small-N instantiations) deferred to follow-ups in DEF-203 entry. SHIPPED 2026-04-28.
