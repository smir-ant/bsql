...
| DEF-282 | **`ProtoState` bit-packing** (audit_accepted #24) — `ProtoState` is 80 B dominated by `ParamOids` 68 B in the `DescribeStatement*` variants. Bit-packing or `Box`'ing the heavy variants could shrink the enum to ~24 B, propagating savings to per-phase `Inner` types. Measure-first: state-transition hot path is well-predicted; the Box indirection per Describe-flow might exceed the savings on per-row cache density. Pareto-better gate applies. | EXPLORATORY |
