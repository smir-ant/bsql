...
- Pedantic-audit Tier 4 sweep batch 4 (Cluster A pilot + small-batch sweep) — 2026-05-19 — **5 pilot files + 7-file small-batch addendum cleaned; pattern established for the remaining sweep**.
  ...
  **Pattern established for future sweeps**: per-comment evaluation. KEEP iff (current-state explainer OR architectural invariant OR external-spec ref OR load-bearing test/doctest OR CREDO classification rule including Tier-3 audit-verification anchors). DELETE iff (pre-X/post-X narrative OR DEF-N attribution OR date stamp OR phase marker (`1c-*`, `Phase 1a`) OR future-roadmap reference OR internal-memo cross-ref). Stale `narrow_len_u16`-style references to deleted helpers also DELETE. **For audit-anchor blocks** (the 3 state_setter.rs audit-#26 anchors are the prototype): KEEP the anchor wrap + verification timestamp + closure-state proof verbatim; REWRITE the inside descriptor to current-state architecture terms (drop DEF-N labels in favour of architecture names). **For tightly-scoped single-rationale comments** (small-batch files, ≤6 refs): per-comment edits work well; the DEF-N prefix gets stripped and the current-state explanation preserved as-is, no narrative restructuring needed.
  Gates per file: clippy `--all-targets -D warnings` clean; tests pass (567/0/8 workspace for command.rs/guard.rs/password.rs; 240/0/0 lib for error_arena.rs + state_setter.rs + small-batch); doctests compile.
  **Remaining Cluster A scope** (sized for follow-up sessions, NOT closed):
  ...
  - dispatch.rs (145), decode.rs (125), ident.rs (69), lib.rs (120), action.rs (130), error.rs (100) — top mid-size cluster.
  - state.rs (76), session_params.rs (55), buf.rs (59), wire.rs (61), write_buf.rs (66), push_command.rs (48), reply_id.rs (35), row_stream.rs (25) — mid-size cluster (reply_id contains a Tier-3 audit #17 anchor + row_stream contains a Tier-3 audit #59 anchor — preserve like state_setter.rs audit-#26 anchors).
  - 8 remaining smaller files: scram/session.rs (12), prepared.rs (12), md5.rs (12), cancel.rs (12), scram/types.rs (11), params.rs (11), drop_witness.rs (10), session_params_slot.rs (8), schema_slot.rs (8), bounded.rs (7) — fast-sweep candidates next session.
  Total remaining: ~1700 refs across ~24 files. Recommended: ≤7 files per small-batch session, ≤3 files per mid-size session, ≤1 file per session for protocol.rs sub-batches.
