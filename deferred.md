...
| 2 | DEF-280 | **Safety Bundle Group — 43 NEW findings from rethink audit** (memo `/tmp/def-rethink-safety-memo.md` 1100 LoC). Consolidates 7 ship-bundles closing tier-2-by-discipline gaps surfaced by Architect 3's surface audit. **Sub-bundle G** (~20 LoC, ship-ready, no API break): eliminate `debug_assert!(false, ...)` glass-architecture pattern (11 sites — release-mode silent fallthrough makes assert misleading dev-noise; classified fallback IS the safety net, assert redundant). **Sub-bundle H** (~5 LoC): `RowStream: !Send` marker via `PhantomData<*const ()>` — closure-scoped API enforces lifetime scope, but `&mut RowStream` can be `tokio::spawn`'d, racing with Drop. **Sub-bundle J** (~10 LoC): distinct ReplyId saturation sentinel ≠ first-mint MIN — current `unwrap_or(NonZeroU64::MIN)` saturation byte-identical with legitimate first ID, monitoring code checking `id == MIN` false-positives every connection's first command. **Sub-bundle K** (~20 LoC): `enter_partial_mode` typestate — `buf.rs:850-863` debug-asserts re-entry precondition but release silently overwrites `partial_remaining`, future refactor triggering re-entry causes invisible connection desync. **Sub-bundle I (Bundle D'')** (~20 LoC): `Zeroizing<i32>` wrap for `dispatch.rs:613-618` secret_key extraction — currently `let secret_key_inner: i32 = *secret_key.get()` lives unscrubbed on stack for the function-frame lifetime; promote from `protocol.rs:2335-2339` «Bundle D'' if ever needed» comment to commit. **Sub-bundle E** (~80 LoC, breaking API): closure-scope ALL secret accessors — `Sensitive::get`, `Password::as_bytes`, `SecretBoundedStr::as_str`, `ScramSession::password_bytes` — same retention-attack pattern as Bundle D' closed for CancelRequest. **Sub-bundle F** (~150 LoC, BREAKING): migrate sealed-supertraits → concrete-tokens — `PostStateProof` / `DescribeName` tier-1 external, tier-2 within-crate; any in-crate module can `impl Sealed + impl PostStateProof for HostileWitness` and write any `ProtoState` variant via `install_into` (mirror DEF-272 cluster δ pattern). **Plus 1 silent data-corruption fix**: `row_stream.rs:1028-1030` silent column truncation under adversarial server — `bytes_offset.saturating_add(col_len_usize)` clamps, `.unwrap_or(&[])` delivers zero-byte column to user, oversize column reads as empty value. Replace with error classification. **Total HIGH-priority in-tree work**: ~305 LoC + silent-truncation fix. Bundles G/H/J/K can ship immediately (no API break); Bundle D'' next; Bundle E during/after DEF-279 (closure-scope all secrets — pairs with foundation); Bundle F either before or as part of DEF-279. | 8 sub-bundles closing 16 HIGH-priority + 18 MED-priority gaps; data-corruption class closed | Mixed — G/H/J/K/I tier-neutral; E breaking on 5 secret accessors; F breaking on PostStateProof/DescribeName impl callers |
