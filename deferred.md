...
| DEF-210 | **Tier-1 audit findings (architect-driven 2026-04-28)** — full closure detail in §D Phase 1c entry below; this row stays in §A only as a pointer. **ALL CLOSED in §D**: SR-01 Path C/D, SR-02, SR-03, SR-04, SR-05, SR-06, SR-07, ML-01, ML-03, BS-11, CF-02 (initial batch, commit `d0b794e`); REC-06 SCRAM Box consolidation + NB-04 residue-policy per-class pin tests (follow-up batch). **Acknowledged-as-policy**: BS-01 `panic = "abort"` zeroize gap; BS-02 transitive `unsafe` in `simdutf8`/`heapless`/`sha2` deps per CREDO §11. | Tier-2 by-discipline → tier-1 by-construction at 14 sites; see §D for per-site detail. | Closure of "стеклянная архитектура" risks. | §D ALL CLOSED 2026-04-28. |
...
**REC-06 SCRAM Box consolidation + NB-04 residue-policy per-class pin tests (follow-up batch, audit 2026-04-28):** (m) **REC-06** introduces `state::ScramHandshakeState` struct holding the three SCRAM handshake fields previously each heap-boxed in `ConnectingScramAwaitingServerFirst` (`scram` + `client_first_bare` + `client_nonce_b64`). The variant now carries a single `Box<ScramHandshakeState>`. Per-handshake allocator ops at the SASL-continue arm: pre-REC-06 was 0 allocs + 3 Box-frees (the three separate Boxes drop on transition); post-REC-06 is 0 allocs + 1 Box-free + 1 stack `ScramSession::drop` with `ZeroizeOnDrop` scrub (cumulatively −2 allocator ops + same scrub coverage). Helper signature `dispatch_auth_sasl_continue` rewritten: `scram` parameter changed from `Box<ScramSession>` to `&ScramSession` (helper only reads `password_bytes()` once for HMAC composition; caller drops `scram` at arm scope exit, single source of `ZeroizeOnDrop` scrub firing). Variant size: 4 fields × 8 B → 2 fields × 8 B = 16 B inline (down from 32 B); ProtoState size pin holds at 80 B (DescribeStatementAwaitingRfq still dominant). SR-07 docstring restored to literal accuracy ("at any moment in a SCRAM handshake's lifecycle at most ONE `Box` is live"). Bench `push_command/ping_amortised` post-REC-06: 54.17 ns vs prior commit `d0b794e` 55.95 ns — net **−1.8 ns / −3% improvement** (smaller binary surface from variant shape change ripples positively into LTO inlining decisions on adjacent code).
(n) **NB-04** adds the `protocol::residue_policy_per_class_tests` lib unit-test module, pinning the 5-class residue policy of `clear_session_residue_if_idle_or_errored` against arm-body swaps. Five tests cover: `Idle` (clears `row_desc_slot`, preserves `session_params` content), `Errored(_)` (clears all three; verified via `SessionParams::is_pristine`), and `Connecting` / `PingAwaiting` / `BusyQuery` (preserve every observable residue field). The wildcard `_ => {}` form remains in production (chosen for register-pressure preservation per the путь-Z investigation); the test pin lifts the broad-scope tier-2-by-discipline to tier-2-by-test-pin (an arm-body swap that shifts semantics — e.g. Idle accidentally scrubs session_params — trips an immediate test failure). The residual tier-3 surface ("a new `StatePushClass` variant added without a matching test arm") closes via contributor discipline + code review; full integration-test-via-public-API closure is out of scope (requires server-frame fixtures that aren't justified for this risk class). Total tests: 297 → 302. Both REC-06 and NB-04 SHIPPED 2026-04-28 (follow-up commit, see git log).
