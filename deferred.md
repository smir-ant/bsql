...
|-----|-----|------|----------|-----------|
| 1 | DEF-279 | **Foundation Rethink — Phase 1a SHIPPED (`5b5b886`), Phases 1b/1c/1d/2/3/4 pending** (memo `/tmp/def-rethink-foundation-memo.md` 1368 LoC). Direction 1.B: `<P as SealedPhase>::Inner = DisconnectedInner | ConnectingInner | ActiveInner | ErroredInner | ResettingInner | ClosedInner`. Each phase has its own struct with phase-appropriate fields. **8-10 tier-1 elevations gained** at full bundle completion: per-phase methods (`<ActivePhase>::with_cancel_request() -> R` directly, no `Option<R>`; Bundle D' collapses); state variant in wrong phase = compile error; backend_key/session_params inline non-Option on `<Active>`; per-phase dispatch carries only legally-touchable slots; `<Disconnected>::feed_inbound` storage-absent not just method-absent. **Per-phase sizes** (Phase 1a shipped first row): `<Disconnected>` 536 → **0 B (ZST)** ✅ Phase 1a `5b5b886`; `<Closed>` 536 → **~1 B** Phase 1b; `<Errored>` 536 → ~32 B Phase 1b; `<Connecting>` 536 → ~296 B Phase 1c; `<Active>` 536 + 436 heap → ~952 B inline Phase 1d (−1 heap alloc, net −20 B). **Bundled with footprint memo Layout B** (`/tmp/def-rethink-footprint-memo.md` 5 wins): Box ProtoState heavies (state 80 → 16 B, −64 B); u16 OID interner in RowDesc (−64 B); Box<RowDesc> (−132 B, measure first +2-5 ns/row); Password 512 → 32 B inline + `LongPassword(Box)` for outliers (−486 B/alloc); retire `error_arena` (lifetime-bounded slices, −8 B struct + ~290 B heap + ~1100 LoC + 12 tier-3 surfaces). Combined `<Active>::Inner` ~200 B inline post-rethink (Box-heavies + per-phase tight fit). **Remaining cost**: 5-8 weeks (Phase 1a took ~430 LoC; remaining 1b/1c/1d/2/3/4 estimated +3500/-1500 LoC cumulative). **Rejected (with reasoning)**: enum-of-phase (1.C, loses type-level phase proof), union-storage (forbid(unsafe_code)), boxed Inner (heap indirection per DEF-187), bit-packed state discriminant (needs unsafe in Drop). **Subsumes/closes** (post-completion): DEF-273 (within-_proto_init_leaf — collapses via new per-phase storage layout); partial of DEF-274 (cell modules; per-phase Inner makes some cells `<Active>`-private inline, eliminates cross-module write surface); DEF-058 (ring-buffer ReadBuf becomes per-phase consideration); DEF-247-redesigned (typed-row projection layer integrates with `<Active>::Inner`'s narrower scope). **Phase 1a sequencing**: GAT bootstrap + DisconnectedInner ZST + push_startup rebuild + cascade fixes + size pins + asm-diff bit-equivalent verification. **Next**: Phase 1b (`ClosedInner` carrying only StateErrorKind cause — drop read_buf/state/row_desc_slot, ~504 B savings). | 8-10 tier-1 elevations + factor-536 reduction for ZST/1B phases + Active heap-alloc eliminated; Phase 1a delivered the first 536 B win | BREAKING — every API touching `PgProtocolInner` changes; consume-self phase transitions become real memcpy (~ns, once per connection); Phase 1a's promotion of `PgProtocolInner`/`DisconnectedInner` to pub is purely E0446 mitigation (fields stay private; no new external capability) |
