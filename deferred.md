...
| 4 | DEF-246 | **Branch-collapse typestate `PgProtocol<P: SealedPhase>`** — `#[repr(transparent)]` outer wrapper over `PgProtocolInner` + ZST `PhantomData<fn() -> P>` phase marker; 4 phases (`DisconnectedPhase`/`ConnectingPhase`/`ActivePhase`/`ClosedPhase`); inner `ProtoState` enum preserved. **Design memo 2026-05-16** (`/tmp/def246-design-memo.md`) rejected pure-typestate option as driver-side regression (26-variant enum same size as today's `ProtoState` + monomorphisation explosion); selected design (c) branch-collapse — byte-identical layout, layered phase-conditional methods. **Phase 1 SHIPPED 2026-05-16 `d97484d`**: scaffolding (PgProtocolInner + PgProtocol<P> wrapper + 4 markers + SealedPhase sealed-trait + Deref-less explicit `self.inner.<field>` access + 5 size const-asserts), 0 tier-elev, **zero public API breakage** via default `P = ActivePhase`; asm-diff bit-identical on materialise/parse_header/feed_inbound/dispatch; bench-stable 0 regressions. Phases 2-6 pending: Phase 2 push-before-Startup (+1 tier-elev), Phase 3 push-during-Connecting (+1), Phase 4 Errored-absorbs-input + `feed_inbound -> Result<(), ProtocolError>` (+2 per principal directive), Phase 5 hostile-probe closure, Phase 6 default-phase removal. | **4 tier-elevations**, revised from memo §1 — «80 B Idle elimination» premise was misframed (the 80 B is `ParamOids`-dominated, not Idle-specific); actual win is structural state-transition correctness by-construction + `?`-propagation on feed_inbound. **Primary value: architectural** | BREAKING in Phases 2-6; Phase 1 itself additive only |
