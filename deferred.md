...
|-----|-----|------|----------|-----------|
| 1 | DEF-265 | **α heap-box ReadBuf** — **MEASURED-REJECTED 2026-05-08.** Direct implementation: `read_buf: ReadBuf` → `read_buf: Box<ReadBuf>`, `pub const fn new()` → `pub fn new()` with `Box::new(ReadBuf::new())`. Compiled clean (Box::Deref/DerefMut covered all callsites without API churn). All 447 tests passed, clippy clean, `PgProtocol` size 4352 B → 248 B (94% inline footprint reduction confirmed). **BUT:** bench-stable compare vs `phase-c-clean-pre-heap-box` showed `ping_round_trip/push_then_feed` **+54.65%** (119.12 ns → 185.26 ns; +66 ns ≈ one Box::new alloc cost on Apple Silicon system allocator). The bench creates fresh `PgProtocol::new()` per iteration — the regression is the heap-alloc cost on cold connection setup. Other benches mixed: parse_header -2.40% (improvement, likely re-bench noise on unrelated path), iter_rows_via_next_event -3.30% (improvement, also re-bench noise), iter_rows_via_next_row +4.62% mean / +6.52% median (borderline, criterion classified within noise band but mean upper-CI exceeds 5%). **Trade-off summary:** cache footprint reduction is structural truth (-94% inline; pool of 100 conns: hot working-set 425 KB → 25 KB) but doesn't show as win on existing single-conn benches; alloc cost on cold cycle IS visible. Production reality: connection lifetime is thousands of queries, per-query alloc cost amortises to ~0; ping_round_trip bench measures cold-cycle which is non-representative of production. Per CREDO §3 strict measure-first: **measured regression > theoretical win → REJECT.** Working tree reverted. | Cache footprint -94% inline (theoretical, not bench-visible); +54% on cold-cycle bench (REJECTED) | None public; rejected at impl |
