...
fn resolve_cache_dir() -> Result<PathBuf, String> {
    ...
    // first online build created `.sasql/` at the right level.
    let mut search = dir.clone();
    loop {
        let candidate = search.join(".sasql");
        if candidate.is_dir() {
    ...
    // No existing .sasql/ found — create at CARGO_MANIFEST_DIR.
    // The user can move it to the workspace root if desired.
    Ok(dir.join(".sasql").join("queries"))
}
...
pub fn lookup_cached_validation(parsed: &ParsedQuery) -> Result<ValidationResult, String> {
    ...
    let envelope: CacheEnvelope = bitcode::decode(&bytes).map_err(|e| {
        format!(
            "failed to decode offline cache file {} (file may be corrupted \
             or from an incompatible sasql version — run `cargo build` with \
    ...
    if envelope.version != CACHE_FORMAT_VERSION {
        return Err(format!(
            "offline cache was generated by a different sasql version \
...
mod tests {
    ...
    fn validate_enum_string() {
        assert!(validate_cached_type("::sasql_core::types::EnumString").is_ok());
        assert!(validate_cached_type("Option<::sasql_core::types::EnumString>").is_ok());
    }
    ...
    #[test]
    fn walk_up_finds_existing_sasql_dir() {
        ...
        let tmp = TempDir::new().expect("tempdir");
        let sasql_dir = tmp.path().join(".sasql");
        std::fs::create_dir_all(&sasql_dir).expect("mkdir");
        ...
        assert_eq!(
            found,
            Some(sasql_dir.join("queries")),
            "walk should find .sasql at workspace root"
        );
    }

    #[test]
    fn walk_up_falls_back_to_start_dir() {
        // No .sasql/ exists anywhere — should fall back to the starting dir
        let tmp = TempDir::new().expect("tempdir");
        ...
        assert!(found.is_none(), "no .sasql/ should be found");
        // In production code, the fallback creates at CARGO_MANIFEST_DIR
        let fallback = start.join(".sasql").join("queries");
    ...
    fn write_cache_and_lookup_roundtrip() {
        let tmp = TempDir::new().expect("tempdir");
        let queries_dir = tmp.path().join(".sasql").join("queries");
        std::fs::create_dir_all(&queries_dir).expect("mkdir");
