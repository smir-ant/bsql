//! Implementation of the `#[sasql::pg_enum]` attribute macro.
//!
...
fn gen_from_sql(
    ...
    quote! {
        impl<'_pg> ::sasql_core::pg_types::FromSql<'_pg> for #enum_name {
            fn from_sql(
                _ty: &::sasql_core::pg_types::Type,
                raw: &'_pg [u8],
...
fn gen_to_sql(enum_name: &syn::Ident, variants: &[EnumVariant], pg_type_name: &str) -> TokenStream {
    ...
    quote! {
        impl ::sasql_core::pg_types::ToSql for #enum_name {
            fn to_sql(
                &self,
                _ty: &::sasql_core::pg_types::Type,
                out: &mut ::sasql_core::pg_types::private::BytesMut,
            ) -> ::std::result::Result<::sasql_core::pg_types::IsNull, ::std::boxed::Box<dyn ::std::error::Error + ::std::marker::Sync + ::std::marker::Send>> {
                match self {
                    #(#to_sql_arms)*
                }
                Ok(::sasql_core::pg_types::IsNull::No)
            }
            ...
            fn to_sql_checked(
                &self,
                ty: &::sasql_core::pg_types::Type,
                out: &mut ::sasql_core::pg_types::private::BytesMut,
            ) -> ::std::result::Result<::sasql_core::pg_types::IsNull, ::std::boxed::Box<dyn ::std::error::Error + ::std::marker::Sync + ::std::marker::Send>> {
                if !<Self as ::sasql_core::pg_types::ToSql>::accepts(ty) {
