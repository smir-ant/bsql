...
cargo test -p bsql-postgres-async --test streaming_scale -- --ignored  # live constant-memory scale gate: query_each_raw streams 5M rows with RSS O(1) in row count (coarse ps -o rss= delta vs a 10k stream, generous 48 MiB margin) + exact count/Gauss-sum/order — the live peer of the offline engine_query_break_alloc gate
...
fn creates_a_user_sync(conn: &mut bsql::pg_sync::Connection) {
    conn.query_raw("CREATE TABLE users (id int)").unwrap();  // same isolation, blocking driver
}
```
...
- **A PG `PreparedStatement` cannot be used on a FOREIGN connection (no silent
  wrong result).** A `PreparedStatement` is a free owned `'static` handle naming a
  server-side statement (`_bsql_<n>`) whose plan lives ONLY on the connection that
  `prepare`d it — and the name comes from a PER-CONNECTION counter, so `_bsql_0`
  exists on EVERY connection's first prepare, each a DIFFERENT plan. Running
  connection A's handle on connection B would bind B's like-named-but-unrelated
  statement: a SILENT wrong result (or a server `26000` if B has none). Closed BY
  CONSTRUCTION: each `Core` mints a PROCESS-UNIQUE identity at connect (a monotonic
  `u64` from a process-global `AtomicU64`, distinct for every live connection),
  stamped onto every statement it prepares; `query_prepared` / `execute_prepared` /
  `close_statement` check the handle's origin against the connection's identity
  FIRST — before `verify_params`, before ANY wire I/O — and reject a mismatch with
  the classified `DriverError::WrongConnection` (fieldless, 24 B pin untouched). The
  connection is UNTOUCHED (no round trip), so `is_disconnect()` is `false` (fix the
  call, retry on the connection that prepared the statement). This is a RUNTIME
  guard, not compile-time, because a PG prepared verb needs exclusive `&mut conn` AT
  CALL TIME (`conn.query_prepared(&stmt, …)`) — so the handle canNOT hold the
  connection borrow the way SQLite's connection-scoped `SqliteStatement<'conn>` does
  (whose verbs are ON the handle, making its cross-connection use a COMPILE error);
  tying an owned `'static` PG handle to a specific runtime connection value would
  need a generative lifetime brand on every `Connection` / verb signature (rejected
  as pervasive complexity). A loud zero-wire rejection is the best achievable and
  strictly better than a silent wrong result. The dynamic `DynStmtCache` has NO such
  hole — it is per-connection-internal, never handed out. Witnessed by the
  `--ignored` `prepared_statement_used_on_a_foreign_connection_is_rejected` live
  tests (both drivers) + the offline classification unit test.
...
- **Dynamic streaming — `conn.query_each_raw(sql, on_row)` /
  ...
  peer of the eager `query_raw` / `query_params`, and the PostgreSQL peer of the
  ...
  growing memory (the escape from `query_raw`, which materialises the whole
  ...
  drained, never swallowed. `query_each_raw` rides the SIMPLE-query wire (like
  `query_raw`); `query_each_params` rides the FUSED one-round-trip dynamic path
...
- **Homogeneous atomic bulk write — `conn.execute_batch::<Q>(params_iter)`**
  ...
  `deps_pin` is unchanged. **SQLite twin:** `sqlite_conn.execute_batch::<Q>(…)` runs
  ...
  in-process). The typed batch is `execute_batch` on BOTH backends (one cross-backend
  name); SQLite's DYNAMIC multi-statement raw-SQL executor (rusqlite's, a semantically
  DISTINCT operation) is the disambiguated `execute_batch_raw` — the `_raw` suffix marks
  the raw-SQL verb exactly as `query_raw` / `execute_raw` do, and as `copy_in` /
  `copy_in_typed` disambiguate their pair; so a consumer doing cross-backend bulk writes
  uses `execute_batch::<Q>` on BOTH backends, and `execute_batch_raw` is the SQLite-only
  raw-script verb. READ-ONLY under a conformance build (with `macros-sqlite`
...
- **Client-side read-liveness window (a black-holed in-flight query cannot hang) —
  ...
  (2) EVERY dynamic runtime-SQL verb — the raw-text (`simple_query`/`execute_raw`/
  `query_raw`/`query_one_raw`/`query_opt_raw`), param (`query_params*`/
...
- **Structured diagnostics — `DiagEvent` + the `Diagnostics` sink.** The dep-free
  ...
  dynamic verbs (`query_raw`/`execute_raw`/`query_params`/`execute_params`, timing
