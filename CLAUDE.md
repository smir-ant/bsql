...
- **Homogeneous atomic bulk write — `conn.execute_batch::<Q>(params_iter)`**
  ...
  DISTINCT operation) is the disambiguated `execute_batch_sql` — the `_sql` suffix marks
  the raw-SQL verb exactly as `query_sql` / `execute_sql` do, and as `copy_in` /
  `copy_in_typed` disambiguate their pair; so a consumer doing cross-backend bulk writes
  uses `execute_batch::<Q>` on BOTH backends, and `execute_batch_sql` is the SQLite-only
